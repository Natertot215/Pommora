...
- **Agenda:** the calendar layer — **Tasks** (reminder-shaped; keyed with `TaskID`) and **Events** (calendar-shaped; keyed with `EventID`) — each as Markdown files in its own root singleton folder (seeded as `/Tasks` and `/Events`, registered by sidecar id and renameable), distinguished via their key and validated against their folder placement.
...
- **Connections:** the shared name for both the `[[Title]]` and `[Label](Title)` link syntaxes inside MarkdownPM surfaces, resolved against an in-memory title map built from the page tree — connecting to another Page as the Content ↔ Content matrix. A connection may also be held as a Link property's value; Contexts are the only relation, and no relation-type property exists.
