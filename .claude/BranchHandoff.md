...
The branch is merged and gone. What survives here is the *reasoning* — the feature docs carry the live behaviour, and where the two ever disagree the feature doc wins.
...
- **The legacy `tierN` read path was defended, then deleted.** The claim that migration never rewrote page front-matter was wrong — `reconcileWriteRoot` resolved each `tierN` array, wrote the bracketed key and deleted the tier field, and migration step 4 ran it over every entity root. The conversion itself has since gone: both nexuses were verified converted, so the migration and every trace of the old key form came out rather than staying as dormant weight. A file still carrying one keeps it as an ordinary foreign key — preserved by value, never read.
...
- **There is no content index.** The one that existed had no query consumer and was deleted; its replacement gets written alongside the query layer that reads it, and gates Linked-From, backlinks and search.
