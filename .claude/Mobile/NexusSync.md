...
The desktop's file-watcher treats every write it did not make as the same event: it re-reads the tree and refreshes, without caring what wrote the file (its own writes are skipped through an echo record, since the app refetches those itself). It was built so external edits — Obsidian, Finder — flow in without a restart, and an iCloud change arriving from the phone is, to this code, exactly that: an external edit. The sync-ingestion path already exists, and the same holds in reverse on the phone.
...
- **Sync the whole nexus, minus a few per-machine display files.** Almost everything is shared and must sync — page files, sidecars, and nearly all of `.nexus/`, which holds the property registry, the Contexts, the Homepage, settings, and image assets. The one thing that must NOT sync is `.nexus/nexus.db` and its `-wal`/`-shm` siblings — every per-machine display fact now lives there, and a WAL database moving through a file syncer invites corruption. Excluding the `.nexus/` folder wholesale would strip the schema, Contexts, Homepage, and images from the phone — the exclusion is **per-file, never per-folder**. (`state.json` is fully canonical in this build — it holds only the top-level order keys — so it syncs whole, no special-casing.)
