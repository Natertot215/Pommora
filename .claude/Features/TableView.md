...
Ungrouped and loose rows sit at the loose-inset, tucked a touch left of the column inset so they land near the Title column, and property grouping's **no-value rows get the identical treatment: no "None" band**, a header-less flattened tail (→ `Views.md`). Collapse rides a Reveal on the shared disclosure motion, and collapsed rows leave the DOM. A collapsed band folds to the row rhythm, the cards view's seam law: any band seam it touches sheds the section gap and its dead head-to-row clearance folds away, the spacing morphing in step with the fold — sub-band buckets already sit at the row rhythm so their lead never moves, and the band directly under the table head keeps the section gap, the edge keeping its law. The hover "+" renders on structural set headers only — a property bucket has no inferable create location. It's the affordance alone; the creation surface behind it is a Prospect.
