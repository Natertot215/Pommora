...
| Delete a property (global) | A recovery pair — the definition and every value, keyed by page id — lands in `.trash` atomically as the provenance pair's artifact-less variant, then the value is stripped across every collection's pages and assignment lists, every Remove-cache block for it is purged (a cache without its definition is corrupt state), and the definition leaves the registry — nothing restorable in-app → [[NexusRecord]]. |
