...
Resting on a resolved connection past a short dwell raises the glance pane (`Core/Interface/Glance/GlancePane.tsx`): a compact, read-only render of the target page through the embed framework, without its banner or inline title, on the PickerMenu chassis rather than a window — no backdrop, so it never steals a click. It is mounted once at app level, and every host reaches it through one seam (`Core/Interface/Glance/glanceAction.ts`) that takes any anchor element and a page or website target, owns the dwell (one duration per host kind), and keeps the pane standing while the content view scrolls until its anchor leaves the DOM. The pane resolves its content before opening (a page that can't load opens nothing), centers on the live anchor and tracks it as the line reflows, scrolls within itself with headings folding on click, and closes on hover-off, Escape, navigation, or the anchor leaving view; **Hover Preview Linger** extends the stay. Its text selects and copies in place, read-only, and a press inside it hands focus back to whatever held it when the pane closes. It resizes from every free edge and corner to one remembered size per machine, and it keeps a small per-page store of editor state and scroll, so re-glancing a page returns to where it was left unless the page changed in between. The seam refuses an anchor inside the pane's own body, so a glance never glances itself. MarkdownPM is its only host today; a sidebar row, a tab, or a view row could raise the same pane through the same call.
