...
The **confirmation** (`Windows/ConfirmationWindow.tsx`) is the exception, and deliberately so: a question is not a window you move or resize, so it takes the ImagePicker's chassis instead — a centered glass panel on a click-swallowing scrim, portaled to the body and clamped rather than given geometry of its own. It states a message and a detail line above Cancel at the foot-left and the action at the foot-right, the action drawn destructive or tinted by the question's tone. Escape and an outside click cancel, Return answers the default, and nothing takes focus on open. It holds the picker-open shield while it stands, so the menu that asked survives underneath it and Escape peels one layer at a time. One question stands at a time; a second displaces the first, which answers as a cancel. Every destructive confirmation in the app — a delete, a removal, a sweep — routes through it, with the copy for each held in `Windows/confirmations.ts`.
