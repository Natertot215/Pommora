...
The shell and the surfaces it is built from: the three-pane window, the toolbar, the sidebar, the footer bar under every content view, the floating windows, and the glance pane. Each is a surface that hosts content owned elsewhere — pages, views, the navigation layer — and this document describes the surfaces themselves: how they are laid out, what they show, and how you move between them. `App.tsx` composes them; the components live under `Core/Interface/` — `Sidebar/`, `Toolbar/`, `SidePane/`, `Header/`, `Subfield/`, `Glance/`, `Windows/`, `Confirm/`, `Notifications/`, and `Menus/`. The commands and menu models every surface acts through are `Core/Actions/`, whose one door `popMenu` draws a menu natively or through `Menus/MenuPresenter`, and the assets they draw are `Core/Assets/`.

### The Shell

A three-pane shell: the sidebar, the main pane, and the SidePane, with both side panes drag-resizable and their widths persisted per machine. Both are the one component, `Core/Interface/SidePane/SidePane.tsx` — a docked glass pane that takes its side, its width, and its resize floor and ceiling, and slides in and out on the shared pane motion. The toolbar runs across the top of the main pane, the Subfield along its bottom, and the sidebar collapses on its own motion with the ribbon folding away alongside it. The SidePane is reserved for a future decision; its own design pass is pending, and properties do not live there. Every entity opens under a consistent header: containers and pages can set a banner image that bleeds edge-to-edge under the side panes, with the title overlaying its bottom-leading corner and the banner and title locking in place while the body scrolls.

### The Toolbar

The toolbar (`Core/Interface/Toolbar/Toolbar.tsx`) holds the tab bar on its left and a trio of glass buttons on its right, with one slot between them that changes with the selection. The trio is the Navigation button (its menu is a placeholder), the Settings menu — the container's SettingsFrame or a page's own Settings — and the SidePane toggle; when the SidePane opens, the trio rides its edge on the shared swallow motion, dropping the dividers between its buttons and taking one at its trailing edge, the seat side-pane-only tabs will fill. On Windows the trio drops its glass pill and closes on a divider that meets the system's caption controls, whose width the toolbar leaves free at its trailing edge; with the SidePane open, those controls sit in its top corner while the trio slides in beside the SidePane's leading edge. The middle slot holds the ViewMenu on a container and the Outline on a page, since a selection is one or the other, and a Space shows its own menu there.
...
Every in-app window mounts one chassis, **WindowBase** (`UIX/Windows/`): a glass shell with per-window-id geometry, one floating toolbar (a zero-height strip whose action clusters float over the content), side-panel slots, and a footer, opening and closing on a scale-fade and sealed off from the main shell's pane geometry so the SidePane behind it shifts nothing inside. Five windows mount it: the Page Window, the Web Window, the NavWindow, the Page History window, and the Nexus Settings window.

#### The Page Window

The floating page window (`Core/Interface/Windows/PageWindow.tsx`): a movable, resizable, fully editable window that opens Pages without touching the main pane's selection, tabs, or history. One exists at a time; a new summon overtakes it in place. The left scan glyph **promotes** the page — it opens for real in the main pane while the window plays the engulf, a FLIP from the window's rect onto the detail pane's — and the right cluster holds a parked Settings glyph, the SidePane toggle, and the close. That SidePane — an overlay side slot at one remembered width, shared with the NavWindow's page tabs — is the frontmatter SidePane: contexts and properties in two group fields that render only once something is assigned, an Add affordance alone on an empty page, and rows built as menu rows whose value opens the shared property picker, with right-click popping the property menu or, on a linked value, the link menu. The footer is the Subfield, driven by the window's page.

The window is a small tabbed app of its own. A connection clicked inside it opens as a tab beside the origin rather than navigating away, deduplicating against one already open for that page; closing the active tab falls to its left neighbor, closing the origin re-parents the set to the left-most survivor, and closing the last closes the window. A single-tab window shows the page's ancestry as a centered NavTrail; the second tab's birth collapses that into a standard tab strip, and closing back to one returns it. Tabs drag-reorder and move to and from the main bar, and the set persists per origin across sessions, per-machine, restored against the live tree. Warmth is per tab: editor state, undo, and scroll return on switch.
...
The two ways the app speaks for itself, and neither hurries the user. A **confirmation** (`Core/Interface/Confirm/ConfirmationWindow.tsx`) asks before a destructive act: a centered panel on a click-swallowing scrim, on the ImagePicker's chassis rather than a window, since a question is not something you move or resize. Cancel sits at the foot-left and the action at the foot-right, drawn destructive or tinted by the question's tone; Escape and an outside press cancel, Return answers the default, and nothing takes focus on open. It stands on the dismissal stack like any popup: Escape and the buttons leave the menu that asked underneath it, and a press on the scrim, being a press outside the whole stack, closes that menu with it. A **notification** (`Core/Interface/Notifications/NotificationLabel.tsx`) is the opposite posture — it reports one finished act at the frame's right edge under the toolbar, offers at most one action, and retires itself when its three-second bar drains. Coming near it eases that drain between full speed and a standstill rather than switching it off, and the radius is around the label rather than on it, so the countdown relents while a hand is still travelling toward Undo. Deletion is the first act to use both: what leaves a bundle in `.trash` can be restored from it, a view's Undo saves back the configuration the surface still holds, and a delete sent to the system trash offers none. Copy for every question lives in `Core/Interface/Confirm/confirmations.ts`.

### The Glance Pane


The Glance Pane is Pommora's compact page-peak surfacace which displays an anchored window previewing it's targeted page, accesiable via most of the interface. A resolved connection raises the glance pane on a short hover-dwell; every other interface surface — a tab, a sidebar row, a nav entry, a card or table row — raises it on Shift instead (`Core/Interface/Glance/GlancePane.tsx`): a compact, read-only render of the target page through the embed framework, without its banner or inline title, on the PickerMenu chassis.. It is mounted once at app level, and every host reaches it through one (`Core/Interface/Glance/glanceAction.ts`) that takes any anchor element and a page or website target, owns the dwell (one duration per host kind), and keeps the pane standing while the content view scrolls until its anchor leaves the DOM. The pane resolves its content before opening (a page that can't load opens nothing), scrolls within itself with headings folding on click, and closes on hover-off, takes priority over a conflicting ghost-creation hover action, with Escape, a second Shift, navigation, or the anchor leaving view dismissing it, and a click outside it dismissing it too when **Dismiss Preview On Pointer Actions** is on; **Preview Persistence** extends the stay with options on how long the pane persists when not actively hovered on. It resizes from every free edge and corner to one remembered size per machine, and it keeps a small per-page store of editor state and scroll, so re-glancing a page returns to where it was left unless the page changed in between. 

A markdown link naming a website raises the same pane as a live, non-interactive render of the site: it opens under a quiet cover that fades once the page paints, a page that fails or never paints closes the pane, and a shield keeps every pointer event on the pane while passing the wheel down so the site scrolls without becoming clickable.

---

#### Pending

- **The SidePane** — reserved; its design pass is pending. Its nexus-wide configuration has a reserved `SidePane` key in `.nexus/state.json` and a cap of six user-made tabs (`MAX_SIDE_PANE_TABS` in `Core/Tiles/tiles.ts`); nothing reads or writes them yet.
