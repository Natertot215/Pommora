...
- **Sorting** reads the filename, so a column orders A → Z by what the label shows. Filtering is presence only — the same Is Empty / Isn't Empty pair every other type offers.
