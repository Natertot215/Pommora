...
Each type's definition-level configuration lives on the `propertyDefinition` schema; its per-view look lives on the view's `column_styles`.[^2] The editor pane for each type is a leaf of the Properties pane.
...
**The Properties Pane.** The Properties leaf of the toolbar's Settings dropdown (`src/renderer/src/Components/Detail/PropertiesPane.tsx`) is the assign surface for a Collection: the assigned properties on top, each opening its per-type editor, and an **All Properties** disclosure pinned to the bottom listing every unassigned registry definition in the nexus order, each promotable by its `+` or by dragging into the assigned group. Dragging within a group reorders it — the Collection's order above, the nexus order below — and dragging an assigned row out removes it. The pane's `+` creates: it mints into the registry, seeds per-type options, and assigns here. Renames, type changes, and option edits change the global definition for every assigner. The global Delete lives only inside a property's own editor pane, behind its ⋮ menu and a native confirm.
...
- **A stray bare-string Multi-Select value drops out of grouping and filtering.** The read-side coercion handles the single-string types only. Unreachable while nothing writes that shape; it goes live when a lossy Select→Multi-Select change is implemented, and should be fixed there as a value migration.
