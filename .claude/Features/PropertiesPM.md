...
A filled link row or cell right-clicks to the **link menu** rather than a cell menu of its own — the same menu the editor pops on the same link (→ [[ConnectionsPM]]), so what a link offers doesn't depend on where it was found. A connection opens on Open Preview · Open New Tab and closes on Copy Link · Copy Path; an address opens on Open Preview · Open Browser · Copy Link. Both carry the authoring pair, and both end on **Clear** where the editor's ends on Remove Link · Delete, a value being the thing a property surface can act on. A card's menu adds the **Remove** that drops the property from the view; an inspector row's adds the one that takes the property off the page.
