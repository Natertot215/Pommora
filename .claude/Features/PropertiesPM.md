...
**The Properties Menu.** Any full page menu — a sidebar page row, a view row's title cell — carries a **Properties ▸** branch under Edit Icon, built once in `Core/Actions/propertyRows.ts` and assembled per page in `Core/Interface/Menus/propertyMenuActions.ts`. It lists the nexus Contexts in registry order, then the Collection's own schema behind a divider, skipping the auto-managed stamps. A row whose type offers a fixed set expands into it with the page's standing values checked — the options for Select, Status, Multi-Select, and Context, **Check** and **Uncheck** for a Checkbox — and a pick writes through the same value path a surface uses, undo included. A native menu resolves on the first click, so each pick on a multi-value property is its own visit. The types with nothing to enumerate stay leaves: File opens the operating system's file dialog, and Number, Link, and Date open their picker over the row the menu stood on, hosted for every surface at the shell (`Core/Interface/Menus/ValuePickPresenter.tsx`). A property with no options to offer reads disabled.
