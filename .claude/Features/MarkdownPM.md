...
Resting on a resolved connection past a short intent delay opens the hover preview card — a compact, read-only view of the target page beside the link, rendered through the shared embed framework without its banner or inline title. The card is a glance surface: content scrolls within it, headings fold and unfold on click in place of the chevron, and the caret never enters. It anchors to the live link through scroll, and closes on hover-off, Escape, navigation, or the link leaving view; the Settings ▸ Pages linger slider extends how long it stays after hover-off. The card resizes from its right edge, bottom edge, and corner — wearing the shared resize accent — to a single remembered size every card opens at, and resting table cells raise the same preview. The card itself belongs to the Page Preview system (→ [[PagePreviewPM]]).
...
- **Auto-pair + paired-delete** — brackets plus the single emphasis / code / quote markers. Each is gated so ordinary prose stays literal, and the gate reads **both sides of the caret**: nothing closes when a word sits immediately ahead of it, since the closer would land buried in text the keystroke never meant to wrap; behind the caret, `[` pairs only at a line start or after whitespace, and the emphasis / code / quote markers only when not right after a word char, where they instead **type over** their own closer on the way out. A construct's own closer isn't a word, so type-over survives the forward rule. The doubled emphasis forms promote rather than pair. Backspace inside an empty pair removes both halves. All of it runs in table cells too.
...
- **One heading scan serves both the folds and the outline, and they disagree on purpose** — a fold section is dropped when nothing sits beneath its heading, because there is nothing to fold; an outline keeps it, or two consecutive headings would show only the second. Both read the same fence-aware scan, so what counts as a heading — and the ordinal that keeps duplicate titles apart — can never diverge between them. The editor also publishes a reveal seam beside its fold toggle: scroll to an offset, opening every collapsed section hiding it, innermost and ancestors alike. 
...
- **One pass decides where every code block ends, and it is shared across processes** — the fence grammar and its pairing live in the shared code module beside the mask the write side reads, so the renderer's chrome and the rename cascade's protection are the same answer by construction. The layers above it (block resolver, decoration pass, heading folds, document stats) each consume that pass rather than recognizing a fence for themselves. This matters beyond appearance: a layer that pairs fences on its own eventually under-claims a block, and an under-claimed block is one whose `[[Title]]` a rename will rewrite — a file edited inside its own code sample, not merely a construct drawn wrong. Two divergences from the reference parser are load-bearing and easy to reintroduce: the info-string capture must stay unanchored at the line's end, or a CRLF document loses every fence to the carriage return; and a backtick fence whose info string holds a backtick is prose, never a fence.
...
- **Hot-path reads share one per-doc-version derivation** — the doc string, the whole-doc line / fence / callout scan, AND the caret-free per-line decoration intents are each computed once per document version and cached against the CM6 doc. A caret move re-derives only its own affected lines (its line, plus its fence's edge lines) and assembles the rest from the cache, so per-caret cost stops scaling with document length; the pure whole-doc derivation survives as the reference an equivalence test pins the assembly against. Inline tokenizing is scoped to the visible ranges rather than the document, which is why a scroll rebuilds decorations too; it is cached against the same doc version keyed additionally by that span set, so only an edit or a moved viewport pays the parse and a caret move re-reads it. Because that slice carries no memory of the lines above it, it opens on a line whose block context is self-evident — past any fence it would otherwise resume inside, and at the line owning an indented run — so the slice's own fence model reaches the same verdict the whole document would.
...
- **An unreproduced renderer crash on a programmatic scroll toward a table inside an embed tile** — the window goes black with no crash log and no console errors, and the same jump replays cleanly, as does gradual scrolling. Unreproduced since, so no mechanism is established.
