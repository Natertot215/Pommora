...
- **Connections** (`[[Title]]`) — title-only, styled colored inline text (never a chip), three states (resolved / phantom / ambiguous) riding the live `@shared/connections` layer: resolution, styling, click-routing, and the rename cascade are all the connection layer's, so the editor cannot drift from it. The `[[`autocomplete (a glass popup at the caret, prefix-matched, keyboard-driven)]] is one shared primitive between the page editor and table cells.
