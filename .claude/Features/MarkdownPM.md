...
GFM reference footnotes: a `[^label]` marker in the body and a `[^label]: text` citation in the run at the document's end, with the file left as plain GFM. A citation counts only while that run reaches the end — one anywhere above live content is prose that happens to look like a citation. Markers draw as their first-use ordinal rather than their label, in the body and in table cells; a marker nothing binds to stays literal, and a citation nothing points at draws dimmed behind an en dash. The section is hidden by default and shows through a **Show Footnotes** / **Hide Footnotes** control above the [[SubfieldPM|Subfield]] and the divider above the section itself, both writing one per-page override; it contributes nothing to the Subfield's counts, is inert to grip-drag, and stops a collapsed heading from swallowing it. Clicking a marker travels to its citation, or follows it where that citation is exactly one link or Connection; right-clicking either gives Edit · Copy · Delete, and deletions cascade only where the range is exactly the construct. **Insert ▸ Footnote**, **Paste As ▸ Footnote** and typing a fresh `[^label]` each write a complete pair in one undoable transaction and renumber the rest — numeric labels follow first-use order, a word label like `[^note]` holds its position unrewritten, and typing a label that already has a citation adopts it. Because one line left standing after the section would literalize every citation at once, a change that would do that has its text relocated to the end of the body instead.
