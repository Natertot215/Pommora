...
GFM reference footnotes, in the pair the format defines: a `[^label]` marker in the body and a `[^label]: text` citation in the run at the document's end. The file stays plain GFM — GitHub and Obsidian read the same page identically — and everything the editor draws over it is presentation.

**The section is the trailing run.** A citation counts only while the run it belongs to reaches the end of the document, blanks and continuation lines included; `[^1]: text` written anywhere above live content is prose that happens to look like a citation, and it renders as the characters it is. That boundary is derived in one place and every layer reads it, which is why the counter, the fold, the guard and the renderer can never disagree about where the footnotes begin.

**Numbers are positional, never what is on disk.** A resolved marker draws as its first-use ordinal in the control label's ink, cap-aligned and permanently opaque — it never reveals its syntax and the caret cannot enter it. Citations draw as numbered rows sharing the ordered list's glyph column, with a hanging indent across their continuation lines. Markers bind to citations by case-folded label; one nothing binds to stays literal text, and a citation nothing points at — an orphan, or a duplicate whose label an earlier row already took — draws dimmed with an en dash where its number would be. Markers render inside table cells too, resting and while a cell is being edited.

**The section hides.** It is hidden by default (a per-Nexus **Show Footnotes By Default**, → [[ConfigurationPM]]) and shows through a **Show Footnotes** / **Hide Footnotes** control in the band above the [[SubfieldPM|Subfield]], offered only on a page that has footnotes and carried into the [[PagePreviewPM|Page Preview]]. The divider above the section is the other half of the same control: it draws as the title seam's own rule, blur-fades when the section is hidden, and ghosts back under the pointer so a reader scrolled to the foot of a page can find the footnotes without knowing the control exists. Disclosure is the editor's fold motion, and the state is a per-page per-machine override that clears when it lands on the current default. A footnoted document ends at its footnotes rather than trailing the editor's usual typing room, and a collapsed heading stops where the section begins rather than swallowing it.

**Following a footnote.** Clicking a marker opens the section if it was hidden and travels to its citation; where that citation's whole content is exactly one link or one Connection, the click follows that instead. Right-clicking either construct opens its own menu — Edit, Copy and Delete on a marker, Edit and Delete on a row. Deletions cascade by range rather than by gesture: removing exactly one marker takes its citation only when it was the last reference to it, removing exactly one citation takes every marker bound to it, and a wider sweep takes only what it covered. One undo reverts whichever cascade just ran.

**Creation and numbering.** Three doors write a pair: the context menu's **Insert ▸ Footnote**, **Paste As ▸ Footnote** (which shapes any clipboard into one citation's text), and typing a fresh `[^label]` by hand. Each writes the marker and its citation in one transaction that one undo takes back whole, and each ends on the same normalization — numeric labels renumber to first-use order in the body and the section together, the rows sort to match, and orphans collect below them. A word label like `[^note]` is never rewritten but holds its position in the arithmetic. Deletions renormalize the same way. Hand edits that create nothing rewrite nothing, so a label typed out of sequence stays as typed until the next gesture settles it. Typing a label that already has a citation adopts it rather than seeding a second, which is how one footnote comes to be cited twice. Whether a creation takes the caret to the new citation follows **Jump To Citation On Creation**.

**Nothing can strand the section.** Because the run has to reach the document's end, one line left standing after it would literalize every citation at once — so a change that would do that has its text relocated to the end of the body instead, above the section, where a stray paste at the foot of a page was going to belong anyway. The same rule protects a citation's hidden `[^label]:` prefix from being written ahead of. The section is inert to block drag in both directions, carries no grip, and contributes nothing to the Subfield's line, word and character counts; a marker counts its own characters and no word.
...
Right-click pops the OS-native menu, built in the main process off the `context-menu` event so the system edit roles, spelling, Speech, and Share come native, with Pommora submenus (Insert / Format / Embed / Heading / Lists) whose active state reads the live `EditorState`. **Insert ▸ Footnote** joins the block rows wherever a marker can bind — anywhere but the citations section itself and inside code. **Insert Link** appears where the selection is itself an address, pointing it at itself in place — the words you selected stay the label, and a schemeless one gains the scheme that makes it open. It sits apart from `Format ▸ Link`, which opens an empty target for words you have yet to point anywhere. **Paste As ▸** closes that block, offering what the clipboard could become rather than what a plain paste would make of it: an address offers the three link forms and the address itself, a copied `[[Connection]]` offers either syntax that reaches its page, and a markdown link is read through its target the same way a link's own menu reads one. **Footnote** leads every list and answers to the clipboard alone — any non-empty clipboard, a multi-paragraph one included, becomes one citation with its marker at the caret. On a blank line each list gains its embed — **Embedded Link** for an address carrying an explicit http(s) scheme, **Embedded Page** for a page whose title the `![[…]]` syntax can spell — written over that line rather than inserted around it, since both embeds are lone-line constructs and the caret is already sitting where one belongs. Elsewhere on a line, inside a fence, or inside a table region, neither is offered. A clipboard the other forms can make nothing of still offers Footnote; an empty one shows no submenu at all. The menu is main's, built off the `context-menu` event, so it never pops over a table's non-editable widget — Paste As is absent inside a cell, where ⌘V and the inverse chord still work. Shortcuts: ⌘B / I / E / K, ⌘⇧X (strike), ⌘⇧K (connection), and ⌘⇧V for the inverse paste (→ [[ConfigurationPM]] §Shortcuts).
