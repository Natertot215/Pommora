...
- **Inline Marks** — bold, italic, strikethrough, highlight, inline code, links, and Connections. Each reveals with the caret, scales with the heading it sits in, and is suppressed inside code. All of them are reachable from the context menu's Format submenu and their ⌘ shortcuts, and each auto-pairs as you type. Bold pressed over an italic span, or italic over a bold one, replaces its markers rather than nesting.
...
Settings › Pages & Writing turns each group on or off: the Transformations section holds Dashes, Arrows, Equations, Ellipses, and Callout, and the Autopairing section holds Brackets, Markers, Quotes, Delete Pairs Together, and Exit On Enter.
