...
- **Renames are journaled**: a rename writes the pending-rename journal first, cascades the title across all three file scopes, commits the registry, then settles — a crash replays forward on the next open, and a live registry-commit failure reverses the cascade. Renames are id-keyed; ids never change. Because membership is title-keyed, the cascade IS the rename — so the generic path-addressed rename every other entity uses can't name a Context or a Space at all, and a bare folder rename that would strand every tag is unreachable.
...
- **SpaceView**: a Space selection renders its banner scaffold over its block surface — `_space.json` is the second BlockHost beside the homepage, with a per-Space board lock — plus its Subfield breadcrumb. Its heading wears the Space's icon through the shared banner title header, behaving as it does on Collection and Set views, banner or no banner.
