...
**A parent is given an identity before it is named by one.** A folder the filesystem handed Pommora — made in Finder, or by an agent — carries no persisted id until an open stamps it, and the tree's stand-in for it is derived from its path, which is an address this record may never store. So the delete resolves that folder's kind the way the open-time pass does and mints it one. Only a sidecar that exists and cannot be read stays `unaddressable`: minting over it would replace the schema, views and cache it still holds, which is worse than an honest refusal.
...
**One walk carries every governed-key sweep.** Clearing a property, unlinking a Context or a Space, and reconciling a returning artifact each decide something different about a key, and each reaches that decision through the same five steps: enumerate the roots, take the file's lock, ask whether the file may be rewritten at all, run the decision, write back only what changed. The plumbing is stated once and the decision stays the caller's, so a page the decision left alone is never re-serialized and foreign frontmatter and the body never move. What differs rides as parameters because it is genuinely different — **which roots** (a Context tag is legal on any page and on a Space sidecar, while a property value only means anything inside the Collection whose schema governs it) and **whether the write re-dates the page**.

**Clearing a value is a content edit; renaming a key is not.** A page that loses a tag or a cell value no longer holds what it held, so both layers stamp it. A key-only rename changes a spelling, not a relation, and leaves the date alone.

**One page can never fail the fan-out around it.** The gate every sweep asks has two halves — identity admits the file, and its frontmatter can round-trip — because a page nobody can parse used to take the whole sweep down mid-destruction. Such a page is left byte-identical and named as refused, which is how the record admits the sweep was thin. A body-only rewrite asks the identity half alone: it governs no key a broken map could lose, so an inbound link still heals on a page like that.
...
**A returning artifact is reconciled against the world it comes back to.** Every nexus-wide sweep is tree-derived and the tree excludes `.trash` — deliberately, since nothing may rewrite trashed content — so a bundle stays frozen at the moment of its delete while the world moves on. Replaying it verbatim would put governed keys back into the live tree with nothing behind them: a value for a property deleted or unassigned since, a tag for an erased Context, a dead Space inside a surviving one. The danger isn't the dormant key, it's that a later property or Context taking that name inherits values the page never legitimately held. So the content is reconciled in the trash, before anything lands, and a governed key survives only if what it names still exists. **Every root the Contexts layer governs is reached** — a page's frontmatter and a Space's sidecar alike, because the sweeps have always treated both as context roots. The one exception is a returning Context's own key: the live world cannot answer for a subject still in transit, since it is absent from the tree by definition and a Context that has since taken its title would answer in its place, so that key is left for the post-move rekey. Nothing under a trashed Context can have gone stale beneath its own key — the whole subtree froze together. **Restore drops; it never repairs.** A surviving value comes back exactly as the file spelled it, a multi-value key narrowing to its survivors. Standing is judged the way the destination judges it: a Space title resolves through the same coercion every live path uses, so a spelling the tree resolves is a value that stands, while a property value is decoded strictly against its definition — a read renders whatever text a value holds, where a restore writes it back into a live schema. Foreign frontmatter and the body never move.
...
**One predicate answers whether a value still stands**, and every route that puts one back asks it: the returning artifact, the property rebuild, and the Remove cache's own restore. A record and a trashed file are both frozen pictures of a world that has moved on, so asking the question twice is how two routes end up disagreeing about the same value. The caller resolves only what *governs* the key at its destination — a definition, a Context's live Spaces, or nothing — because only the caller knows where the content is landing.
