...
- **Interaction** — within-zone drag reorders, pinned among pinned and unpinned among unpinned; `Ctrl`+`Tab` cycles all tabs; a tab's right-click menu offers Pin/Unpin · Close, with Open Preview above them where the tab holds a page. The `revealTabBarOnHover` knob can hide the bar when idle (→ [[ConfigurationPM]]).
