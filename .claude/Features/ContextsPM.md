...
- **Spaces are folders** at `.nexus/contexts/<Context>/<Space>/`, gated by their `_space.json` sidecar — a folder without the sidecar isn't a Space. 
- **A Space's Context is its folder, and nothing else records it** — the sidecar carries the Space's own identity and appearance, never its parent. The trade is deliberate: the path stays self-describing and re-homing a Space is a move rather than an edit, at the cost of a Context rename being a folder rename and a cascade rather than a single field.
- **Membership lives in member files as parenthesized title keys** at the root, over arrays of bare Space titles — identical in content-file frontmatter and `_space.json`, where JSON quotes the key. An emptied key is removed, never written empty. Member files carry no ULIDs — the registry resolves titles to ids at read.
...
- **The Space settings pane** rides the toolbar trio's settings button: the (Icon)(Title) heading over the footer's board lock, with the icon button and title field outlined in the Space's color through the field's OutlineTint channel. Right-clicking the heading offers the shared identity menu — Rename, Edit Icon, Hide Icon — with Change Color below them. Spaces manage through this pane; the app-level Settings window is separate.[^3]
- **Pipeline** — context columns are default-off; creating a Context never changes an existing view.[^4] Cells read each row's walk-resolved context values, and chips everywhere wear the Space's icon and assigned ramp color.
...
- **ContextView + Linked-From** — a Context's own aggregate surface, and the inbound list of every entity tagging a Space. Both ride the reverse query a content index would answer, and no index exists.[^5]

[^1]: [[PropertiesPM]] §Schema Mutations
[^2]: [[NexusRecordPM]]
[^3]: [[ConfigurationPM]]
[^4]: [[ViewsPM]] §II. Columns
[^5]: [[ArchitecturePM]]
