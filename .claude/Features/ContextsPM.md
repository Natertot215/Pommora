...
- **Pipeline** — context columns are default-off; creating a Context never changes an existing view (→ [[ViewsPM]] §II. Columns). Cells read each row's walk-resolved context values, and chips everywhere wear the Space's icon and chip-solid color.
