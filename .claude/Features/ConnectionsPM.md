...
Right-clicking any link, wherever it sits, opens one native menu built from one model (`Core/Actions/connectionMenu.ts`), so the actions a link offers never depend on where it was found. The rows follow what the link is and where it sits:
