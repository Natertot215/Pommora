...
For the next PM work: MarkdownPM footnotes.

**The model — GFM reference footnotes, and the pair is the syntax:**
- `[^1]` markers in the body, `[^1]: text` definitions at the bottom. Plain GFM on
  disk; no inline ^[text] syntax anywhere.
- The footnotes section is the LAST contiguous run of definition lines in the doc.
  Definition syntax anywhere else is not parsed — literally plain text. (Accepted
  divergence: GitHub would honor a mid-doc definition; we don't. Resolve repairs.)
- A marker renders as a footnote iff its definition exists in the section; an
  unmatched [^1] stays literal text, exactly like GitHub renders it.

**Markers:**
- Render as the positional number (first-use order, like GitHub/Google Docs), accent
  at tint-primary, permanently opaque — no live-syntax reveal at any caret position,
  atomic and caret-proof (backspace removes it whole).
- Click jumps to the definition; if the definition is a markdown link, click
  navigates to the link instead. Right-click: Edit (jump caret into the definition)
  and Delete. Deleting a marker (backspace or menu) also deletes its definition when
  it was the last reference — one transaction, one undo. Duplicate labels legally
  share one definition/number. Selection sweeps/cuts remove only the swept range so
  a cut-paste move reconnects.
- Markers render and work inside table cells; creating one from inside a cell is a
  future item.

**The section:**
- Hidden by default; body shows numbered markers only. Divider = the shared heading
  seam token; clicking it folds. A left-aligned "Show Footnotes" sits at the doc's
  end in flow, never higher than the pane bottom; discloses with the disclosure
  animation, bar on ease-standard, divider blur-fading in/out. State remembered per
  page per machine like heading folds.
- Definition text: label-secondary at 0.75em (editor is em-scaled; don't consume the
  type ramp). Connections and markdown links render normally inside definitions.
  Multi-line definitions per GFM; Enter doesn't auto-continue one.

**Creating:**
- Every creation path writes the complete pair in one undoable transaction and
  auto-discloses the section: Insert ▸ Footnote (context-menu Insert submenu, caret
  seated in the new definition), Paste As ▸ Footnote (pasted content becomes the
  definition; internal targets offer Footnote · Connection · Markdown Link), and
  typing [^1] by hand auto-seeds its empty definition (typing-transform category).
  Empty definitions are valid.
  Right-click gives the option to "Edit" which takes you down into its section definiiton, and a "Delete" option whcih does just that.

**Numbering:**
- Display is always positional. Footnote gestures also renumber the numeric disk
  labels to match position (skipping already-correct spans); hand edits never trigger
  rewrites; non-numeric labels like [^note] keep their names. 

**Stats/settings:** one new Pages & Editor toggle — "Include footnotes in page
statistics", default OFF (definitions and markers excluded from lines/words/chars).
No metadata toggle; frontmatter is already excluded.

**Prospects (later, don't build):** hover-preview of a definition on its marker,
drag-to-reposition markers, footnote creation from table cells.
