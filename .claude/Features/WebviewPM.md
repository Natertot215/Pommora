...
A markdown link alone on its own line with an explicit http(s) scheme is a webpage embed: `![Label](url)`, recognized by the grammar in `src/shared/webpageEmbed.ts` and rendered as a live website tile on the shared embed framework, height-draggable and persisted per host and address like a page tile. The document stays plain CommonMark; a shared vault reads the line as an image reference pointing at a URL. Formation is deliberate: a line the selection sits on stays raw text, and the tile forms when the selection departs, so typing an address never pulls a tile in under the caret. A tile is created through the context menu's **Embed ▸ Webpage**, through **Paste As ▸ Embedded Link** where a copied address meets a blank line, or by hand.
