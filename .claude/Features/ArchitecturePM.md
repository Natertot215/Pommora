...
**Embeds and Floating Windows.** One `PageEmbed` renders a real page inside any foreign surface — the Page Window, the NavWindow, hover cards, dashboard tiles, and the editor's `![[Title]]` widget are the same component — and an edit made anywhere routes through the page's own save path. Floating windows share one chassis, `WindowChassis`, mounted by the Page Window, the NavWindow, the Web Window, and Settings; hover cards ride the lighter menu chassis instead. Live websites are webview guests governed by one main-side owner and one renderer adjudicator deciding where every external link opens.[^10]
