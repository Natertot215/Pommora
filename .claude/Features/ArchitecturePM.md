...
**Write serialization.** Atomicity keeps a file from tearing; it says nothing about a lost update. Every read-modify-write therefore runs under a lock keyed on the file it rewrites and reads fresh inside that lock, so two writers to one file queue rather than race and neither drops what the other just set.

For JSON the primitive takes that lock itself, deriving the key from the path it writes, so a caller can neither omit it nor spell it a second way — the settings file, both registries, the app config, and the banner and icon patches all inherit it by calling the primitive at all. A page is rewritten through the same shape. The exception is a write needing a span wider than itself, most often a schema-validated read: those hold the lock at the caller over the sidecar read/write pair instead, which is why the container sidecar's writers still take their key explicitly. Reaching for a self-locking primitive from inside a lock on the same path is refused rather than left to hang, and refused before the chain is touched, so the attempt cannot wedge the file it names. Two keys carry the weight. A page's own path is shared by its body write, its property writes, and the relocate that a rename or a move performs — so a relocate can never strand a write aimed at the path it just vacated, and a write arriving after one fails cleanly instead of recreating the file it was aimed at. A container's sidecar path is taken by every writer of that file, whether it is saving a view, persisting a within-folder order, changing the open-in configuration, or assigning a property. A sidecar that several unrelated surfaces each rewrite whole is precisely the shape a second lock name would break, so the key is built in the path module rather than spelled out at a call site.

Because those locks are process state, one instance is a correctness boundary rather than a convenience — a second process over the same Nexus would share none of them. The app holds a single-instance lock, and a relaunch raises the window that already exists. Multiple windows stay reachable: one process may own many.
