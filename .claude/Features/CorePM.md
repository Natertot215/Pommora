...
The read side is one eager, read-only walk — `readNexus` — producing a pre-ordered `NexusTree` in a single pass, consumed by the interface without re-sorting and held in the session store. There is no per-kind manager layer, no per-entity cache, and no dependency-injection graph. The walk runs at open and on Reload; between them Core holds its result as the **live tree** (`Core/Nexus/liveTree.ts`), serving reads from memory and patching it in place as writes confirm and watcher events classify. An mtime-gated parse cache keeps a walk cheap by reusing decoded sidecars and frontmatter for unchanged files, and a structural-sharing pass in the interface collapses an unchanged subtree back to its previous object identity so a push re-renders only what moved. Core reads its own settings from that tree — the labels a native menu shows, the zoom a window opens at, the exclusion list — with the disk read as the fallback before a walk has installed one.
...
- **Index consumers** — Linked-From, backlinks, and ContextView membership each have their substrate in the content index: the relationship rows carry kind and count, so each surface is a read over them. Full-text search waits on the FTS table, which remains unwritten.
