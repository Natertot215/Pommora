...
- **Alignment · Format · Icon · menu** — right-click a header for the OS-native column menu. Format is per-type — the looks a type can wear, plus the date and time format rows, whose labels are format-type names rather than rendered samples. The choice persists per-view in the SavedView's `column_styles`. A url column's rows are the three link forms, and a column naming none reads the way its property says to. Number's format is property-wide, set in the Number editor pane; only its look is per-view. Select and multi carry no Format — their chips wear the squared label shape, the pill being Status's alone. **Icon** toggles the per-view **Column Icons**, default off, rendering each header's property type glyph view-wide. The title is the primary column — not hideable, not alignable, not styleable — and pops an empty menu.
