...
One of the built renderers behind the modeled [[ViewsPM|view types]] — a Collection's or Set's Pages drawn as rows on a single CSS grid. It's presentation only: the pipeline hands it resolved groups and per-cell values, and TableView owns the layout, the column ergonomics, and the row and group chrome.
...
**Every column — the title included — holds its resolved width** (the Apple table model).
...
- **Alignment · Style · Icon · menu** — right-click a header for the OS-native column menu. The style submenu is per-type — the looks a type can wear, plus the date and time format rows, whose labels are format-type names rather than rendered samples. It is named **Format** for the two types whose rows are one, a url's link forms and a number's, and **Style** for the rest. The choice persists per-view in the SavedView's `column_styles`. A url column's rows are the three link forms, and a column naming none reads the way its property says to. Number's format is property-wide, set in the Number editor pane; only its look is per-view. Select and multi carry no style submenu — their chips wear the squared label shape, the pill being Status's alone. **Icon** toggles the per-view **Column Icons**, default off, rendering each header's property type glyph view-wide. The title is the primary column — not hideable, not alignable, not styleable — and pops an empty menu.

### Rows & Cells

A data cell's content is type-aware — a page icon, title text, chips, or a link — and its look and formats read the per-view column style: a status renders as a Standard labeled pill or a Compact icon-only chip carrying its group glyph; a checkbox as the square or the real DualSwitch; files as one chip per attachment; dates through the date formatters; numbers through their property-wide format or, in the Bar look, a progress bar filling its accent over a muted track. Column dividers differ by band — the heading draws short, centered, fully-rounded segment bars between columns, while data rows draw full-height hairlines. The row divider is a top border spanning the filler too, and the first row of each group drops it, so a line only falls between two rows. **Row drag** is the drop-line DnD[^1]: a hover-revealed grip in the gutter lifts the row, which mutes in place while an accent line and dot mark the slot.

**Every cell owns its click** — the shared gesture matrix, portable to every other renderer. The title cell is the only navigate; status, select, and multi open the shared value dropdown, where multi toggles and stays open; a checkbox toggles; a Number-look number enters the inline editor, while a Bar-look one opens the text-value dropdown with a right-pinned out-of hint; a link opens externally through the sanctioned link IPC; a file chip opens the OS file dialog and replaces the file it names, while the value's own area adds one.

**Right-click always opens a menu.** The title gets the shared page-meta block with the New Page Above/Below pair; style-bearing types get their column's own radios under the same per-type name; a file cell gets Add File · Replace File · Remove File instead, and on a card its hide reads Remove from View so the two can't be confused; a link cell holding a live link pops the link menu itself — the same one the editor pops on the same link, closing on **Clear** where the editor closes on Remove Link · Delete — while one holding no link, empty or naming a page nothing answers to, gets Edit alone and Edit · Rename · Clear respectively, since there is nothing there to open or copy; a link's form is set from its column's header menu or from the property itself, never per cell; picker-based cells add Clear. The gutter grip carries its own menu — Open Preview · Open New Tab — Rename · Edit Icon — New Page Above · New Page Below — Delete — and its right-press is defaulted away like the drag's left, so the menu never seats a caret or arms the reorder.

**Chip-look values carry a hover ×**, the shared chip-level remove: hovering the chip's right third reveals an × at the right edge in the chip's text color while the label's tail blurs into the fill beneath it — a progressive text blur touching only the text. Clicking removes that chip's value — one option off a multi, one Space off a context value, the whole value off a single — without opening the picker or arming the row drag.
...
- **New Page Above / Below** — the pair, on the grip and title menus alike, creates beside its anchor: the new page inherits the anchor's group value and its values on the active sort criteria that carry one — Select, Status, Checkbox, Number, and Date — and the order write lands it at the gesture slot. Under any other criterion (Title, Modified, text, links, files, multi-value properties) the row lands where the sort places it.
- **The ghost row** — dwelling on a row extends a ghost "New Page" row beneath it on the shared disclosure motion, and dismissal collapses it back on the same motion; it wears the row's full chrome — dividers, the page glyph, the standard label — at the inactive dim, and clicking it runs the Below path. It never appears while a naming session or native menu is open, and any pointer press outside it stands it down before a drag can measure. The dwell/grace machinery is the shared hover-ghost mechanism every surface's ghost rides — the table's chrome is the disclosure row; Cards and the sidebar wear their own.[^2]
