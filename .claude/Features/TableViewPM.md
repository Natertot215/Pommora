...
- **The ghost row** — dwelling on a row extends a ghost "New Page" row beneath it on the shared disclosure motion, and dismissal collapses it back on the same motion; it wears the row's full chrome — dividers, the page glyph, the standard label — at the inactive dim, and clicking it runs the Below path. It never appears while a naming session or native menu is open.
