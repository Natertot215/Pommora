...
A recursive group of rules under a match mode, evaluated at every depth: All (AND) and Any (OR). Negation lives on the per-rule operators — Isn't, Isn't Empty, Doesn't Contain and their kin — never on the group, so "none of these" is spelled as an All of negated rules. A filter the schema cannot read drops alone on read and the view survives unfiltered. Whether the filter runs at all is a separate axis, `filter_enabled`, so parking a filter costs it neither its rules nor its mode.
