...
**Trigger (F2-proof):** runs whenever **the nexus schema version < the context-registry version** — version alone, never on-disk shape (earlier steps consume the tier dirs, so shape can't be the re-entry signal), and the bump commits last only after a fully-clean run, so a killed or incomplete migration re-triggers on every reopen (H-5's resumability made real). **The barrier is `openSessionIndex`, not "the renderer's walk":** the index cold-build itself calls `readNexus` (`build.ts:108`), so migration (and the Task 2.4 journal replay) runs between `prepareOpenedNexus` and `openSessionIndex` at BOTH open call sites (`index.ts:596` adopt-open and `index.ts:2124` launch-restore). One-time work may block open — it is NOT part of best-effort `prepareOpenedNexus`.
