...
**Deviations**
- **Phase 5 (webpage dashboard-tile parity) cut.** It is the one net-new capability (a `WebBlockEntry` schema variant + a renderTile arm), which the execution mandate — "emphasize moving + merging; create only if verified otherwise-impossible; this is a refactor" — does not admit, and which the plan itself flagged as "can be cut entirely for a pure refactor." The unification is complete at the code level: `WebTile` is a first-class peer tile on the shared chassis, `block-tile-base.css` (`.web-tile`), `block-title.css` (`.web-tile-title`), and the tile vocabulary. Webpages remain inline-embed-only (their pre-refactor state — a clean, consistent stop, not a half-built feature). Making a webpage a creatable/mountable dashboard tile is a ready follow-up feature on this unified base, not built here.
