...
- [x] **Phase 4** — The menus
  ...
  - [x] Task 11 — Gate 4's four corrections · `44eeb4a0`, `70bb20e5`, `ee81bea5`
...
### Rulings
- **R6 — the url column's looks are the link forms** (Nathan: "Unify it and mark that this link work. It's full-cohesion is it not?"). Requirement 1 said the vocabulary is adopted by URL properties, and the column Style menu was a URL-property surface that never came along — it offered "Title" and "Full Link", two of three forms in words nothing else used, and wrote a `look` no renderer read, since a url cell resolved its display from the property alone. Both faults close together: the looks are the three forms, and a column reads the way its property says to unless its view says otherwise. The schema drops a look it no longer knows, so a column saved under either old name falls back to the property's Format — which is what it was already showing, so there is nothing to migrate.
...
- **A url cell's right-click carries no Style, and its stated reason is now false.** `cellMenu.ts`'s `baseCellMenu` routes `type === 'url'` to the `link` variant, which offers Edit · Rename · Clear and no style rows — documented as "its look is per-property", which R6 has changed. Every other style-bearing type reaches its column style from a cell right-click as well as from the header. Whether url should join them is a menu decision, not a consequence.
- **`CellMenuKind`'s `style-edit` variant declares a `url` member nothing produces.** `baseCellMenu` returns `style-edit` only for `file`; the `'url' | 'file'` union is reachable for one of them. Only `cellMenu.test.ts` constructs the url case. Pre-scaffolding or residue — it wants a ruling either way.
...
- **Gate 4 — four corrections from the live pass, folded rather than deferred.** (1) `Edit Link` was offered only to a link naming a *page*, leaving the one form whose target cannot be renamed through anything else with no way to edit it; it now leads the menu beside Rename, each selecting its half rather than merely reaching it. (2) The menu order is Nathan's: Rename · Edit Link · Copy Link · Format ▸, the unlink pair still below the separator. (3) Paste and Match Style is back as **Paste Without Formatting** — the act was never the problem, only the accelerator it claimed — and the paste block reads outward from the plain act. (4) A resting table cell now carries the link menu at all: it draws its links as spans and deliberately ignored a right press, so Task 8's `CellEditor` mount only ever reached a cell already entered. What only rewrites text commits against the cell; Rename and Edit Link enter it with their half selected. The span math both need is hoisted out of the two editor appliers — `linkHalves` and `wikiAuthorTarget` — so the cell performs the same edits rather than keeping its own idea of where a link keeps its parts.
- **Gate 4 — a resting cell's commit did not settle the table.** `refreshTableEffect` fires when a cell editor *demotes*, deferred so typing costs no block re-measure. A cell that was never entered has no editor to demote, so a menu edit sat undrawn until something else entered and left a cell. The resting commit settles immediately: one discrete edit, none of the per-keystroke cost the deferral exists to avoid.
- **Gate 4 — the simplifier found one real second writer and one real defect.** `linkMarkdown` had consolidated the *text* half of a formatted link, but the other half — whether a fetch is still owed (`link-title` with no cached title`) — had drifted into three independent copies across the decision, the Paste As write and the Format rewrite. `linkPaste` now answers both together and all three read it, so a Paste As pick and a plain paste can never disagree about whether a title is pending. Separately, Paste As's Markdown Link form composed its label inline and unescaped: a page titled `Notes [WIP] final` produced a link the grammar reads back as nothing at all. It writes through `serializeLink` now, and the test asserts the round trip through `markdownLinkRegex` rather than only the string.
  - **Two findings rejected.** The claimed escaping gap in `main/connections/rewrite.ts` is not one — that rewriter carries the matched link's *existing* label through, already escaped by whoever wrote it, and only re-encodes the target. And the "formatting drift" was a false signal: bare `npx biome format` reports dozens of untouched files across the repo, so it says nothing about this range; the two cosmetic edits it prompted were reverted where they read worse. `npm run lint` remains the floor, and it is at zero diagnostics.
  - **Carried forward, not fixed here:** `ActionItem<A>` is defined in `main/returningMenu.ts` while three shared menu models re-type its shape by hand — `CONN_UNLINK_ROWS` is the third. The type is Electron-free and belongs in `shared/pageMenu.ts`. Outside this range.
