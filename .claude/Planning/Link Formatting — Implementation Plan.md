...
### Rulings
- **R7 — a url cell carries no Format rows of its own** (Nathan: "a url cell on a markdown table does; TableView doesn't"). The two surfaces hold different things: a markdown table's cell holds a *link*, which carries its own menu with Format on it, while a view table's url cell holds a property *value* whose form is its column's or its property's. The rows are not missing from the cell menu, they belong elsewhere — so `CellMenuKind`'s `style-edit` variant narrows to `file`, the one type it was ever produced for.
