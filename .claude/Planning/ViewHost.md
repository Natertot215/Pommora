...
**Simplification pass (08-31-2026).** The boundary was reviewed for what existed only because of it. `ViewHostSeam` and the memo that built it are gone — `useViewHost(source, flattenStructural, upward)` reads the renderer's four refs directly, since `useViewCreation` takes its config from a fire-time thunk and the wrapper lambdas' stable identity was never a dependency anywhere. `foldOverrides` lost its optional marker (the seat mints it). `mergeOverrides` retired: four of its six parameters had no non-empty caller once Table's fold ref owned widths and alignments and `setStylePatch` owned styles, so the fold is `upward.foldOverrides.current({ ...liveView, collapsed_groups: [...collapsed] })` and `viewMerge.ts` keeps only the per-key style fold; its suite became two `mergeStyleRecords` pins. `foldedView()` and its two duplicated save expressions collapsed into `saveFolded`. The band label and the container walk each renderer kept its own copy of moved to the host, and `refreshValues` took the same in-flight path guard the mount effect has. Requirement 4 survives as written — the flattening flag stays the seat's, and the seat's second `useActiveView` is a deliberate keep, since removing it would mean the host switching on `view.type`. `manualOrder` and `effectiveValues` stay on the api as pinned host state with no renderer reader. Gates: typecheck 0 · lint clean 982 files · 304 files / 3749 tests (the six `mergeOverrides` tests became two). Comments 309, unchanged; markers intact. Net **−34** code lines, putting the bundle at **+4**. An attack pass over the result returned one Low, pre-existing: views written before this pass carry `column_widths: {}` / `column_alignments: {}` / `column_styles: {}` on disk — 11 empty records across 5 sidecars in the live nexus — which the retired fold minted and `liveView` now carries forward; every reader optional-chains, so it is legibility residue awaiting Nathan's call.
