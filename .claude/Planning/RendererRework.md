...
- [ ] **The zoom family** — scale vars with no composition rule. The renames: `--card-scale` → `--cards-scale`; the per-tile `--block-zoom` and the tile-side `--zoom` feeders split into per-kind tile zooms — `--view-tile-zoom`, `--webpage-tile-zoom`, and `--page-tile-zoom` (page and markdown tiles share it); `--preview-zoom` renamed out as a crop. "View scale" is spoken for — it's the per-view `view_scale` knob on the SavedView and the `interfaceScale` personalization above it — so the main pane's `--zoom` takes a name that isn't that one. One merge (`--editor-scale` + `--mdpm-scale` → `--page-scale`) that rewires the font path so an embed's font is not scaled twice (`Styles.css` stops reading the page factor; the main page's font takes it through `MarkdownPM/index.tsx`). A `Tokens/scale.ts` stating which axes multiply and which are terminal. *Why:* a var per surface, growing; a single compounding `--zoom` is impossible (a custom property can't reference itself). *Status:* the split and `--cards-scale` ruled (09-01); the merge awaiting ruling (§3.1).
