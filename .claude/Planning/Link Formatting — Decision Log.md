...
- **C-5:** [confirmed] The late rewrite is an **ordinary history entry**, so removing a paste whose title arrived takes two ⌘Z presses — the first restores the Short Link, the second removes the paste. Undo-transparency was tried on paper and does not work: `addToHistory` is not a `TransactionSpec` key (the real API is the `Transaction.addToHistory` annotation), and once corrected, CodeMirror maps the paste's stored inversion through the non-history swap and cannot account for the text the swap inserted — leaving the fetched title behind as prose a second press cannot reach. Two presses is honest about what happened: the swap is a real edit, and the cost lands only on the first paste of a URL, since the title is cached afterwards.
- **C-5a:** [confirmed] The rewrite belongs to a live editor. A table cell's editor is destroyed the moment the cell stops being active, so pasting into a cell and tabbing away before the fetch lands leaves the Short Link permanently — the title never arrives on its own. `Format > Page Title` applies it later against the cache. This is a disclosed consequence, not a defect to guard.
...
- **C-8:** [confirmed] A URL carrying parentheses is fixed **at the grammar**, not at the writer. `markdownLinkRegex`'s target group is `[^)\r\n]`, which ends at the first `)` — so `…/wiki/Foo_(bar)` already truncates mid-address today, wherever it is authored. CommonMark's link destination allows *balanced* parentheses, and widening the target group to `(?:[^()\r\n]|\([^()\r\n]*\)){1,2048}` matches that: no encoding anywhere, the address stays on disk exactly as a person would type it, and every other reader of the file agrees with Pommora about where the link ends (→ Reasonable Translation).
  ...
  - **Nesting goes two levels deep**, matching what cmark renders. One level refuses a two-deep balanced target (`…/a_(b_(c)_d)`) that other CommonMark readers accept; the second level costs one more alternative and shows no backtracking against the same pathological inputs.
  - **A target carrying an unmatched opening paren stops matching**, at any depth — `[t](https://a.com/a_(b)` tokenizes today and will not after. CommonMark reads it the new way (an unmatched `(` in a bare destination is invalid), so the change is a correction rather than a regression, but the link does become plain text and the tests name it deliberately.
...
- **E-1a:** [confirmed] `Paste As ▸` is built **main-side, on that same menu** — the one `installEditorContextMenu` already pops. A renderer-side menu would either duplicate it or replace it, and replacing it costs the system edit items, spelling, `Format ▸`, `Heading ▸`, `Lists ▸`, `Insert ▸`, Speech and Share; there is no third outcome, because main pops for every editable target and only the renderer-set `gripHot` flag suppresses it.
- **E-1b:** [confirmed] That menu's actions travel back on the `menu:action` push channel, and `menu={{ pushState, onAction }}` is passed **only** by `Detail/PageView.tsx:134` — `Blocks/MarkdownBlock.tsx:76` and `Embeds/PageEmbed.tsx:152` mount the editor without it, so the existing `Format ▸` is already inert in blocks and embeds. Paste As inherits that hole unless those two surfaces are wired, which is why wiring them is part of the work rather than a follow-up.
...
- **E-7:** [confirmed] ⌘⇧V is taken from Paste and Match Style. `main/menu.ts:81` is a bare `{ role: 'editMenu' }`, whose `pasteAndMatchStyle` carries ⌘⇧V as its default macOS accelerator and registers it main-side — so the keypress never reaches the renderer and no renderer-side binding can fire until that role is expanded into an explicit submenu. The item is dropped from the app menu and from the editor's context menu (`editorMenu.ts:57`). It costs little here: MarkdownPM is CodeMirror, so a paste is stripped to plain text regardless, and the app's other inputs are plain too.
