...
- **Success Criteria:** Every Cards creation trigger produces a page instantly (Untitled on disk, stamped by its birth context) with an empty naming field over a real card; the ghost card appears on dwell, displaces its neighbors on the cards' own move motion, and creates on click — landing where the ghost stood under a seedable sort or none, including under a prior manual reorder. Under a non-seedable criterion (Title, Modified, ID, text) the newborn lands where the comparator places "Untitled" — the ratified PM-096 behavior.
...
- **A-4:** [confirmed] Ride-along: cross-location card reordering rides the **existing `movePage` order contract** — the op already carries `order?: string[]` ("Absent = legacy append"); the fix is passing it from the drop, honoring it in the store's optimistic arm (the `moveSet` arm beside it already does), and sweeping the table's `relocateRow`, which discards its index identically. The Set-Card drag flash rides along **as its own item, undiagnosed**: the no-optimistic-set-order hypothesis is refuted (the `moveSet` arm patches order optimistically, its comment naming the exact snap-back) — diagnose before fixing.
...
- **A-6:** [confirmed] The order-write laws port whole, and one shipped gap is fixed in the same bundle: **Cards' reorder must also write its local `viewOrders` copy** (today it writes only the wire — the stale local copy is masked by the live override, and A-5 would unmask it, losing the user's whole drag). `bandAdd` keeps **no** order tail on purpose: a band-add's ruling is "born at the pipeline's end of the group," and an id absent from the manual order already ranks last within its band — the ruling's placement, delivered by the ranking itself. Full-membership arrays, one writer per channel, everywhere.
