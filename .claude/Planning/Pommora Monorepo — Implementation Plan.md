...
- The simplification pass (`9c9b994e`) and Task 11's review (`a9d737ab`, 69,398, −61 vs base) are committed; (b) skipped (every MarkdownPM file ≤ 20). Task 11 (c), the re-nest, was dispatched to an Opus agent owning `Core/MarkdownPM/**` plus forced importer repoints (Engine/ not Model/; Editor/ keeps only Task 14's host wiring; smoke types into the editor). Land it as Task 11's commit, tick Task 11, then Task 12.
