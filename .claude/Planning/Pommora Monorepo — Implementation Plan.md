...
- Whole-tree simplifiers S1 (Views/Tiles/Pages, −70) and S2 (Interface/Properties/Navigation/Settings/Assets, −26) reported done and uncommitted; S3 (Nexus, Session, Actions, Trash, Contexts, Contract, IO, Locations, Platform, Web, Index, Connections, Utilities, Desktop) and Task 11's MarkdownPM review agent were still live. Land: gates once the tree is still, `git add -A -- Core/Views Core/Tiles Core/Pages Core/Interface Core/Properties Core/Navigation Core/Settings Core/Assets <S3 folders>` as "Simplification pass", then Task 11's review as its own commit (Core/MarkdownPM plus the importers it repointed, e.g. Core/Pages/pageEditor.ts).
