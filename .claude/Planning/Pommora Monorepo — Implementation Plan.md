...
| `PommoraUIX` | spine mirrors `PommoraUIX/` subfolders; `Components/` heading | UIX root categories | 19 |
...
- `PommoraUIX/`, `renderer/Interface`, `renderer/Actions`, `renderer/Utilities`, `src/shared` in `.claude/Features` → 0. Control: `UIX/Menus` in `PommoraUIX.md` → ≥ 1.
...
- `sidecarMode`, `vite.config.app`, `dist-app`, `react-markdown`, `pngjs` → 0 across the repo excluding `.claude/Planning/MonorepoAudit` and `HistoryPM.md`. Control: `rmwJsonStrict` → ≥ 3. (`assetMigrate` and its `INVENTED` constant are live by ruling 46.)
...
Core/Utilities        PommoraUIX/Util/{capMap,checkSet,moveItem,pad}, Utilities/iteration-window (+ css)
Core/Settings         Settings/SettingsWindow, TrashFrame → Core/Trash/TrashFrame.tsx, AssetDirectoryRow, ExcludedDirectoriesRow, ClearActionRow, css; SETTINGS_WIN/SETTINGS_RAIL → UIX/Windows/window-bounds.ts
Core/Platform         Assets/assetUrl's scheme line, App.tsx:83–173 → useBridgeSubscriptions.ts, Actions/nativeMenus, openWebLink
No Core/Testing (ruling 09-05-2026: the folder collapses; each harness lives with what it exercises): Testing/editorHarness → Core/MarkdownPM/editorHarness.ts; Interactions/pointerHarness → UIX/Interactions/pointerHarness.ts (nine importing folders, a gesture harness); Testing/pageValues + propsAtRoot → Core/Views/; Testing/setup → Core/vitest.setup.ts (the Core project's `setupFiles`)
...
- [ ] `rg -F "getComputedStyle" UIX/Interactions/OverScroll.tsx` → 0 (it reads the token, not the DOM). Control: `rg -F "duration.base" UIX` → ≥ 2.
...
**Now** — after Task 5, `rg -l "@pommora/core" Showcase` lists whatever leaves still reach Core (PanesLeaf's bounds import is already gone: Task 5 lands `SETTINGS_WIN`/`SETTINGS_RAIL` in `UIX/Windows/window-bounds.ts`; the Lab left at Task 1).
...
**Becomes** — Haiku sweeps the mechanical patterns per A18 §2 with the path table below; Opus rewrites the 38 misses and the folder-spined sections of `PommoraUIX` (its `Components/` heading gone; sections follow UIX's root categories) and `MarkdownPM.md` §Architecture; every Features doc gains a first-line `**Workspace:** Core · UIX · Desktop · cross-cutting` tag.

Path table (every doc reads through it): `Pommora/src/main/<x>` → `Core/<domain>/<x>` per Task 4's table; `src/renderer/<Folder>/` → per `renderer-moves.tsv`; `src/shared/<x>` → per Task 3's table; `src/preload` → `Desktop/Bridge`; `PommoraUIX/<Sub>` → `UIX/<Sub>`; "run from `Pommora/`" → "from the repo root"; `npm run dev` → `npm run dev` (root forwards).
...
- [x] **Phase 3** — Filing inside the domains · Gate 3 `e25db9ea`
  - [x] Task 10 — Domains settle · review `d1e42e19` · comments `3804eb52` · move `48b23960`
  - [x] Task 11 — MarkdownPM re-nests · review `a9d737ab` · move `9dfe2e59`
  - [x] Task 12 — UIX settles; motion · `50eded4f`
- [x] **Phase 4** — The two layers · Gate 4 `eba69eda`
  - [x] Task 13 — One menu path · comments `7b6a6ed1` · `ab246bac`
  - [x] Task 14 — Editor host · comments `8afea447` · `d4369bd2`
- [x] **Phase 5** — Removals, fixes, docs · Gate 5 `2053ee50`
  - [x] Task 15 — Kill list · `f2db2a34`
  - [x] Task 16 — Write-path bugs · `4198404f`
  - [x] Task 17 — Showcase severed · `ffd56b07`
  - [x] Task 18 — Harness scripts · `ffd56b07`
  - [x] Task 19 — Docs sweep · `734baa79`
  - [x] Task 20 — Rules, Architecture split, Mobile plan · `734baa79` · `9d2fb001`
...
- Phase 3 is closed (Gate 3 `e25db9ea`, 69,193, −266 vs base); the Phase 4 notification was sent; the comment killer over Task 13's files landed (`7b6a6ed1`). Task 13 is done (`ab246bac`, 68,710, −749 vs base). The comment killer over Task 14's files landed (`8afea447`). Phase 4 is closed (Gate 4 `eba69eda`, 69,000, −459 vs base). Task 15 is done (`f2db2a34`, 68,745, −714 vs base; the inventory filed at `MonorepoAudit/task15-inventory.md`). Task 16 is done (`4198404f`, 68,748, −711 vs base). Tasks 17–18 are done (`ffd56b07`; the ledger republished). Tasks 19–20 are done (`734baa79`, `9d2fb001`). Gate 5 is done (`2053ee50`; Phase 5 ticked). CLOSEOUT steps 1–4 landed (`12f52789` comments; `a2fcd3f0`/`29c84ba1` the Claim; `817e967e` the attack's fixes). Step 5 landed (`60603de2`, 68,679, −780). Finalization: History PM-129, ContextPM, and HandoffPM written; the ledger rebuilt; the board final; four gates; the push with the summary commit; the final notification. Continue in the ruled order: (1) Opus comment killer over the whole of Core UIX Desktop (the twelve over-cap files from Task 18's census first; markers survive), commit; (2) Delivery Claim (Opus writes `.claude/Planning/Pommora Monorepo — Delivery Claim.md`), commit; (3) neutral verification (a fresh Opus general-purpose agent handed only the Claim and the repo), fixes via agents, re-verify, commit; (4) build-breaking-agent (Opus) over `7941edec..HEAD`, verified findings fixed with red tests via agents, commit; (5) code-simplifier (Opus) whole tree, delta stays negative, commit; then History (PM-129 || The Repo Restructure per History-Format at `The Studio/.claude/references/`), ContextPM/HandoffPM final hashes, `loc.py --rebuild` + `--update` and the ledger republished, the board final, four gates, `git push origin main` with the summary commit, the final notification.
...
- Task 20 (Windows note): `nexus:openPath`'s `getPathForFile` and `nodeMachine.realpath` still emit native separators; `posixPath` covers only what `main.ts` hands over. `mutateDeps.trashToSystem` rejects when the machine has none; a phone must never choose `trashMode: 'system'`.
...
- Task 16 (found at Gate 4): the editor's host facet is pinned at mount (`MarkdownEditor.tsx`'s mount effect seats `editorHost.of(hostRef.current)` once); `renderTile` closes over the `connections` handed to `buildEditorHost`, so after a tree push an embedded tile's `ConnectionsApi` is stale until the outer page remounts. Fix in `Core/Pages/editorHost.tsx`: `useEditorHost` keeps `connRef.current = connections`, `buildEditorHost` reads it inside `renderTile`; red test: a harness re-render with a changed host must reach the tile. Also Task 16's sibling sweep: any other host member that closes over a mount-time value.
...
- Task 12 or 15: `UIX/Pickers/IconPicker.tsx:80-86` `openContext` omits `favorites.onMenu` from its deps.

### Deviations

- Closeout step 5 (`60603de2`): the code-simplifier over the whole tree, allowed one small helper per byte-equivalent twin; 10 files +110 / −262 plus three new files (94 lines); code lines 68,742 → 68,679 (−63; the run ends at −780 against base on its counter, −1,002 on the strict count). Landed: `Core/Session/pageConnections.ts` (`useWindowTabConnections` for PageWindow + NavWindow, `usePreviewConnections` for TileHost + PageView; each hook owns its open policy so every memo dep list is byte-identical — an `open` parameter would have re-identified `connections` every render); `Core/Assets/NexusIconEditors.tsx` for the NexusPhoto/HomepageSettings pair (the parent calls `useNexusIcon` and hands the result over); `Core/Settings/SettingsFieldRow.tsx` for the three field rows; `SettingsWindow.tsx`'s `ToggleRow`/`DeviceRow` on one `switchRow` function (no fiber added). Left with evidence: `GripMenuContext.zoomSteps` (a Core/Actions → Core/Tiles edge and a test rewrite); the `useInsertionDrag` tail (two different contexts; a component costs more than it saves); `escapes`' `isAbsolute` limb is reachable (`Core/Index/indexSeed.ts:59` passes a raw walked path); `TileHandleMenu`'s two "disabled" derivations are two facts; `treePatch.ts`'s three factories stay apart by the file's own why (identical key sets let `stabilize` prove convergence by identity); `readNexus.ts`'s `readSet` ≡ `readPageCollection` a readability wash; `LayoutFrame.tsx:108` ≡ `SettingsFrame.tsx:52`'s four frame rows and three labels are a real two-writers pair (a cycle blocks a shared table; filed in ContextPM). Zero dead exports (every zero-consumer export is read by the Showcase or its own file; `MIN_H`/`MAX_H` the A14 audit called unread are used). Tests 334 / 4,050; lint 1,069; the comment manifest 0 over the cap.
- Closeout step 4b (`817e967e`): the attack's finding 1 fixed — `Core/tsconfig.src.json` (extends `Core/tsconfig.json`; `types: ["vite/client"]`, no `node`; excludes the tests, `vitest.setup.ts`, `vitest.config.ts` (whose `vite` import re-references `@types/node` — without that exclusion the gate compiled green and proved nothing), `editorHarness.ts`, `testTree.ts`, the fixtures) in the root `typecheck` chain after `tsc -p Core`; red first at exactly the four lines (`confirm.ts:12,27` `setImmediate`; `fileHistory.ts:17` `NodeJS`, `:95` `.unref`); `confirm.ts` defers with `setTimeout(fn, 0)` (a macrotask after the microtask drain, so the ask's reply still reaches the renderer before the confirming push; `queueMicrotask` would invert that); `fileHistory.ts` types its timer `ReturnType<typeof setTimeout>` and drops `timer.unref()` (every path that ends the history clears the timer — `disarm`/`disarmAll` on `before-quit`, the nexus switch, the rename, the suite's `afterEach` — so it was a no-op; a seam method for it would have been a Node-shaped hole). Finding 2: the Claim's Requirement 4 gains the globals control (→ 0) and names the gate. The two Open items for those files closed. Lint 1,066 (the new tsconfig); tests 334 / 4,050; code lines 68,743 → 68,742.
- Closeout steps 2–4 (`a2fcd3f0`, `29c84ba1`): the Delivery Claim written from the plan's Completion Criteria with every figure re-run at the tip (44,641 characters); the neutral verifier (a fresh Opus agent handed only the Claim and the repository) re-ran roughly 130 assertions — all held but five arithmetic or enumeration slips (a control's count 22 → 20; the widget enumeration 11 → 14; a line breakdown; a new-test list of 15 for 14; an `rg` count that silently skipped five Sessions transcripts, now `git grep`), corrected in the Claim; every "ruled by a Deviations entry" line marked uncheckable from the Claim alone, as designed. The attack (build-breaking-agent over `7941edec..HEAD`, plan-and-implementation mode): three findings, none in the code's behavior — (1) Medium: the seam's compile gate the plan specified at line 152 (`Core/tsconfig.json` without `types: ["node"]`) was widened at Task 4 (`["node"]`) and Task 5 (`+ "vite/client"`) to satisfy Core's 67 fixture tests, and the widening reached the source: under the plan's configuration Core's non-test sources fail on exactly the four lines Open Against Later Tasks already names (`confirm.ts`'s `setImmediate` ×2, `fileHistory.ts`'s `NodeJS.Timeout` and `.unref()`); fixed at closeout with `Core/tsconfig.src.json` (tests, the setup file, the harnesses excluded; no `node` types) in the `typecheck` chain and the two files made host-neutral; (2) Low: the Claim's Requirement 4 evidence was an import grep blind to globals — a globals control added beside it; (3) Low: Task 18's counter reconciliation blamed `Core/Views/fixtures/` (JSON, which neither counter reads) — corrected above to the two `vitest.setup.ts` files. Nine candidates killed with reasons (the phone-seat menu latents Gate 4 ruled; a Windows drive-letter path with no Windows build; the Showcase's absent tsconfig under the never-a-priority ruling; `editorHost.tsx`'s uncapped `tileCache` and `menu.ts`'s `subject` nulling, both byte-identical to pre-run code; `renameFolderEntity` outside the sidecar lock, serialized by the mutate path; `main.tsx`'s five re-imported sheets a single-evaluation no-op). Cleared with evidence: `installMachine` runs at `Desktop/main.ts`'s top before any handler; `installStores` defaults null-tolerant; no third `posixPath` gap; of 23 bare-reply channels only `linkTitles:get` can throw uncaught; every Push has both ends; every container-sidecar writer holds `withSidecarLock` on the same key `rmwJsonStrict` takes; `settings.json` has one funnel; every writer reaches `recordWrite`; every dynamic import, `vi.mock`, `?raw`/`?url`, and CSS `url()` resolves. The stricter delta (tests, harnesses, the setup files, fixtures, Showcase, comments, blanks excluded): 69,397 → 68,395, **−1,002**, against the run's counter's −716 — the run absorbed 286 net lines of test scaffolding into its product number, so the negative delta is understated, not an artifact.
- Closeout step 1 (`12f52789`): the comment killer over Core, UIX, and Desktop; 461 files, +993 / −2,713; comments 3,663 → 2,385 lines (−35%; the ledger's character count −15.9% — compression, not deletion, cleared the cap), the twelve over-cap files brought to 20 or under (`Table.css` 43 → 19, `tile-base.css` 41 → 20, `window-base.css` 39 → 20, `markdown-pm.css` 31 → 20), max file 20; `KNOB` 149, `LOAD-BEARING` 2, `(Nathan's call)` 1, `// PLACEHOLDER` 7, `biome-ignore` 85, `@vitest-environment` 96 byte-for-byte; `comment-ledger.mjs --verify`: token streams identical; two orphaned comments cut (`Input/format.ts:140`, `Input/edits.ts:113`); code lines 68,748 → 68,743 (−5, blank lines the cuts left).
- Gate 5 (`2053ee50`): one Opus code-simplifier; 2 files, 2 comment lines cut (a `nexusSlice.ts` comment stating the opposite of its code after Task 15 removed the catches; a stray blank in `governedSweep.ts`'s imports); code lines 68,748 (the run stands at −711 against base on its counter; `loc.py`'s product areas 68,718 against the 69,704 baseline, −986). The Acceptance block line by line: four gates 0 (334 / 4,050; lint 1,065); `from 'electron` → 14 files, all under `Desktop/`; `@pommora/core` in UIX → 0; `Pommora/` gone; six workspaces; `build:showcase` 0; the test-file count 321 → 334 fully reconciled (19 adds and 6 deletes by `--diff-filter=AD`, the six deletes renames git lost across the range and each paired with its add: `session.test.ts`, `subfieldItems.test.ts`, `zoom.test.ts`, `colorMap`+`solidColor` → `colors.test.ts`, `TileCache.test.ts` → `editorHost.test.ts`; 14 genuinely new; no removal unnamed). The Dead Vocabulary sweep at zero with its controls (`@pommora/core` 716, `@pommora/uix` 671, `'row-menu'` 20, `kind: 'nav'` 2), with three notes: the `window.nexus.` control string cannot match `dialer.ts`'s `return window.nexus` (the one reach, in Platform — intent met); `Pommora/src` → 2 in `loc.py`'s legacy root by Task 18's ruling; `assetMigrate` and `INVENTED` are live by ruling 46 (the constant is pinned by `assetMigrate.test.ts:68` "keeps a real name and gives an invented one its owner's") — the Dead Vocabulary line corrected here. Smoke (agent-driven): state open, the tree rendered, the page body round-tripped and `cmp`-identical, a native `row-menu` resolved null on Escape; request shapes for the record: `page:open` takes a relPath, `row-menu` takes `{items, anchor:{left,top,width,height}}`. The Phase 5 attack, every question to a conclusion, zero bugs: `withRoot` reorders no validation (no old handler validated before the root check); `splitFrontmatter` accepts everything both parsers did (the only narrowing is arrays, which every caller already read as nothing); `connectionsFor`'s six sites byte-equivalent (the resolve-only pair is literally the deleted `resolveOnlyConnections`); `isNavRef` sites pass `TAB_KINDS` explicitly (the old six-member set) and the default applies only at `navigationFile.ts`; `pathSafety.escapes` matches `..`, absolute, trailing slash (symlinks stay `resolveUnderRoot`'s `realpath`; a `..`-prefixed filename is now indexed where `startsWith('..')` dropped it — correcting); `DATE_FORMAT_LABELS` carries both trash formats (the `monthDayYear` label 'Short Date' → 'MM/DD/YYYY' is ruling 47's unification); `normalizeTitle`'s NFC touches only `schema.ts`'s property-name clash check; every `vi.mock` target resolves; the remint lock has no enclosing lock and `fileLock.ts` rejects a re-entrant take rather than hanging; `encodeValue`'s `undefined` reaches no writer; the cross-group drop line is computed before the branch; `connRef` cannot loop (every mounter memoizes `connections` on the tree). Arguable, left: `connectionsFor`'s four literals are two identical pairs (a new helper each); `escapes`' `isAbsolute` limb unreachable through `relative()` but the predicate's contract for a raw path; `treePatch.ts` at 21 comment lines (the closeout killer's); `citations:get`/`linkTitles:get`/`activeViews:get`/`aliases:get` declare bare reply types so a main-side throw is stored by `set()` as data — pre-existing, filed in ContextPM's Known Issues at closeout.
- Tasks 19–20 (`734baa79`, `9d2fb001`): 30 files, +567 / −454 under `.claude`; the registry (`CLAUDE.md`, Features, Guidelines, ContextPM, HandoffPM, FrameworkPM) 331,425 → 331,553 characters (+128, inside ±250); `CLAUDE.md` 10,835 → 11,102 (+267: the two restated Hard Rules and the corrected launch lines — `POMMORA_DEBUG_PORT=9333` arms CDP in dev, the built binary from `Desktop/`, `$HOME` in the userData assignment). `ArchitecturePM.md` → `CorePM.md` (git mv; Nexus Layout, Data Layer, Persistence kept; §The Domains at depth one with one line each, §The Platform Seam, §The Contract; main/renderer vocabulary gone) + `DesktopPM.md` (Shape of the App, Platform, Bridge, Push Path, Store, File Watcher, Actions, Web, Capture, Config, Packaging, Renderer); `[[ArchitecturePM` → 0. `PommoraUIX.md`'s spine on `ls UIX` alphabetically, `### Components`/`### Composite Shells` dissolved, a Caret section added, the atlas tables corrected to the tokens that exist (`node .claude/scripts/check-atlas.mjs` → 16 checked, all agree; three tables had drifted on the generated `--system-*`/`--solid-*` vars and the constants Gate 3 inlined — the doc changed, never the code), −4,652 characters. `MarkdownPM.md` §Architecture on the Engine/host shape (its ten subfolders named bare under the stated `Core/MarkdownPM/` parent, the doc's own convention). Every Features doc opens with `**Workspace:**`; A18 §4.3's unnamed folders named (`SidePane`, `Core/Tiles/Layout`, `Core/Actions`, `Core/Assets`, `Core/Utilities`, `Engine/`). Guidelines: `Development-Environment.md`'s CJS claim restated per G-5 (preload only, the transpiler caveat, the host may go ESM), the GUI from the root, four tsconfig projects; `Dependencies.md`, `Web-Guests.md`, `Editor-Internals.md` swept. The PRD: one `ID` key with the ULID kind mark, `<Projects>:` keys, the catalog reconciled to Link / Creation Time / Last Modified, "composed-blocks" → tiles. `ContextPM.md`: Current Focus rewritten; Immediate Work is Nathan's own pass alone; the `main/index.ts` split and the `WEB_PARTITION` call retired as done; five Open Calls (`showError` vs `notifyError`; `ActionItem.confirm` write-only; `RowMenuHost` the phone's presenter; `ALL_ICONS` Lucide-only; TileLab a blank stage; the harness-counting note) and six Known Issues (the Windows notes; the five unlocked single-writer `writeJson` sites; `page:open` not raising a window; `NativePickerContext` and detached roots) added; two stale debt rows retired, three corrected. `HandoffPM.md` rewritten for the run; `FrameworkPM.md`'s restructure line completed; the Mobile plan's Task 0 and Phase 8 marked superseded with an eight-row path table, A-6 and K-1 confirmed against this plan, H-1 repointed at `Sync`, the list-menu Prospect removed. Three audit claims were stale the other way (`PagePropertyRows` already one with `variant`, the index.ts split at Task 8, `WEB_PARTITION` in `Core/Web`) and were retired; two drifts the audit missed corrected (`accent.ts`/`personalization.ts` claimed under `UIX/Theme` — the writers are `applyAccent` in `UIX/Theme/ramp.ts` and `applyPersonalization` in `Core/Session/personalization.ts`; `ImagePicker` claimed as a UIX picker — it is `Core/Assets/ImagePicker.tsx`). 254 workspace-rooted citations checked, 0 misses; the Dead Vocabulary in `.claude` at 0 each with its controls. Ruled at commit: Gate 5's review covers the Phase 5 range (`eba69edad..HEAD`); the closeout's attack and simplification cover the whole run (`7941edec..HEAD`), so the range is not reviewed twice.
- Tasks 17–18 (`ffd56b07`): 15 files, +306 / −1,272 (the one-line JSON ledgers and the page's data tag); code lines unchanged at 68,748 (Showcase is outside the run's counter); tests 334 / 4,050; lint 1,065; `npm run build:showcase` 0 (it failed at HEAD on `TileLab.tsx`'s five `@renderer/` imports). Task 17: `PanesLeaf` reads `UIX/Windows/window-bounds` and restates the six `.settings-*` classes it wanted as `.panes-*` in its own `panes-leaf.css` (two app-shell tokens as literals — the kit does not publish them); `ComponentsLeaf` loses the ImagePicker section (a Core surface with no kit equivalent) and formats dates with a leaf-local callback; `TileLab` is a blank stage — every symbol it used (`TileLayout`, `tileIds`, `insertBand`, `splitAtTile`, `TileGrid`) is `Core/Tiles`' by the 09-05 ruling and the never-delete list keeps the leaf; ruled at commit: blank (the placeholder rule permits it; moving `Tiles/Layout` + `TileGrid` to UIX would reverse the tile ruling; retiring the leaf would breach the list) — Nathan's call, filed in ContextPM. `@pommora/core` out of `Showcase/package.json`; `vercel.json` already correct. Task 18: `loc.py` walks `Core UIX Desktop Showcase` plus the legacy `Pommora/src` root so `--rebuild`/`--history` keep 58 days of the chart (the Verify's `Pommora/` → 0 is therefore 2, both that root and its docstring — kept, reported), nine areas ("Main Process" → "Nexus & Data"; "Desktop Shell" and "Showcase" new), `migrate()` re-keys the stored history; `check-atlas.mjs` reads `UIX/Theme/{theme-vars.css.ts,colors.ts,color.css.ts}` and runs to its count (16, all failing on the Features' `Pommora/src` SOURCE lines until Task 19 rewrites them); `comment-ledger.mjs`/`comment-manifest.mjs` resolve TypeScript from the root (6.0.3), walk the three workspaces, and parse with `createSourceFile` (the raw scanner's `${}` desync gone); the manifest run bare is the comment census against the cap (688 files, 3,664 lines, 12 over: `Table.css` 43, `tile-base.css` 41, `window-base.css` 39, `markdown-pm.css` 31, `markdown-tables.css` 27, `mutatePatch.ts` 22, `watcher.ts` 22, `fields.css.ts` 22, `segment-run.css.ts` 22, `treePatch.ts` 21, `nativeCaret.ts` 21, `OverScroll.tsx` 21 — the closeout comment killer's list); `comment-baseline.json` re-snapshotted at `b74b5ce58` (983 files, 3,906 comments), `comment-units.json` regenerated (14 units); the four dead `cp` permissions gone; the README rewritten; the format exclusion needed no change (`biome.json`'s `includes` already carries `!**/.claude` since Task 1; `npm run format` leaves `.claude` untouched, verified). `loc.py --update` at HEAD: `b74b5ce58` 70,968 (with Showcase; product 68,718 — the run's counter says 68,748; the difference is the two `vitest.setup.ts` files, 31 lines, which `loc.py` excludes by name and the run's counter does not); the Line-Ledger republished. Known: `loc-history.json`'s 09-05 sample is a mid-move measurement (`[0, 3263, 0…]`); a `loc.py --rebuild` repairs it (58-day re-walk; deferred to the closeout ledger step).
- Task 16 (`4198404f`): 14 files, +193 / −80 (non-test +81 / −78, +3 code lines; tests +105); code lines 68,745 → 68,748 (the run stands at −711); tests 334 / 4,045 → 334 / 4,050. Each red-first: (1) `remint.test.ts` "a container write held across the pass keeps both facts" — red `expected '01KV…' not to be '01KV…'` (the fresh id clobbered back by a write through the held read); `remintSidecar`'s read + stamp inside `withSidecarLock`, the tile-doc write outside (a different key). The `writeJson(` sweep (16 non-test sites): eleven locked; five unlocked single-writer sites left with reasons — `Nexus/identity.ts` (`ensureIdentity` at session establishment), `Properties/journalSlot.ts`, `Trash/record.ts` (a just-minted bundle), `Contexts/contextsRegistry.ts:26` (the seed-on-absent write — the one real, benign double-seed window on a registry-less nexus, most-recent-wins), `Nexus/adopt.ts` via `writeSidecar`, `folderEntity.ts`'s creation after an `exists` guard. (2) `homepage.json`: Task 15 had left three sites; `updateNexusConfig` exported (returns the `Result`; `updateSettings`/`updateCrops` keep their throwing contract) and called at `setHeadingIconHidden.ts`, `setBanner.ts`, `assetMigrate.ts`; no red test — the funnel is `rmwJsonStrict` already, a two-writers consolidation with zero behavior delta. Verify: `homepage.json` non-test → 2 (`Locations/paths.ts`'s constant; `assetMigrate.ts`'s report label — not a path); `updateNexusConfig` → 9. (3) `page.test.ts` "a value kind the schema has no shape for is refused, never a clear" — red `expected undefined to deeply equal ['todo']`; the cause was `encodeValue`'s switch falling off the end (`undefined` → `setGovernedRootKeys` drops the key), not `isBlankValue`; `encodeValue` returns `undefined` explicitly as the refusal signal and `updatePageProperty` fails `invalid-property` before any write; the sibling sweep found the same hole in `createPage`'s IPC-borne `seeds` (fixed at the same source); `applyValueAtRoot` shares the shape but is renderer-optimistic only (reported). (4) `tableDnd.test.tsx` "a drop below the row the dragged one already follows is a no-op" — red (the drop line drew at the row's bottom edge; the commit would have been one slot above); the block routes through `slotInGroup(order, near, y, id).beforeId` then `nextOrder`; a crossing test in `reorderModel.test.ts` pins the two mechanisms agreeing. (5) `editorHost.test.ts` "a host seated at mount renders a tile against the live connections" — red `expected {generation:'first'} to be {generation:'second'}`; `useEditorHost` keeps `connRef.current = connections` and `renderTile` reads it (no Compartment, no CM lifecycle change, no smoke); the memo's `connections` dep kept because `useConnectionAutocomplete` keys on the host identity to refresh an open candidate list. Sibling sweep of the host: `pageId` is settled before either mount (PageTile waits for `body !== null`; PageView/PageWindow key on path), `inert` is a per-site literal, every other member reads the store at call time. (6) `IconPicker`'s deps confirmed at line 85 (Task 12's).
- Task 15 (`f2db2a34`): 225 files, +734 / −1,025 (non-test +683 / −931); code lines 69,000 → 68,745 (−255 against a −550 target; the run stands at −714 against base); tests 334 / 4,049 → 334 / 4,045; lint 1,065. The inventory (`MonorepoAudit/task15-inventory.md`): dead regardless 11 items / 104 lines, dead by standing rule 3 / 41, presumed-shape 10 (all confirmed live, none touched); the never-delete list checked item by item, every marker byte-for-byte. The finding: the audits' high-confidence lists (A02/A08/A10/A12/A14/A16) were already consumed by Tasks 3–14 and the simplification pass — verified against the tree, not the audits; the tree holds no fully dead export (0 of 2,854), no orphan source file, no dead CSS class (the 24 selectors without a literal reader are template-generated), zero dead dependencies, no dead build config. Two audit claims wrong: `--disclosure-indent` is read at `UIX/Table/table-tokens.css:10-12` (a Biome-wrapped `var()`); `PickerMenu`'s non-self-managed branch is driven by its test and two Showcase leaves. Landed: `withRoot` in `Core/Contract/handlers.ts` over 19 handler sites; two frontmatter parsers → `splitFrontmatter` in `Core/IO/pageFile.ts` (18 files; the array-frontmatter hole closed); four `ConnectionsApi` builders → `connectionsFor` beside the page index; four `WarmSeam` stores → `mapWarmSeam`; `isTabRef` → `isNavRef(v, kinds)`; `containerOf` → `parentOf`; seven boundary checks → `pathSafety.escapes`; the date-format table one (ruling 47: trash labels and `DateTimeEditor` read `DATE_FORMAT_LABELS`); normalizers on `normalizeTitle` (the `Properties/schema.ts` NFC gap fixed); `withoutCacheBlock` ≡ `patchCacheBlock`; three kind→schema tables → `readContainerSidecar`; `toggleLink`/`toggleConnection` → `toggleWrap`; `LayoutToggles` + `CardsOptions` → `ViewSwitches` (two files gone); one `state.json` writer; `reorderTopInTree` folded; `navSlide` ×4 → `slide()`; `nexusFolderRefusal`'s aliases gone; `memoFmt` for `numFmt`/`dateFmt`. Ruled already single-sourced: `MutateOutcome`, `isPlainObject`, the asset URL builder; `MeasuredRow` only `tableDnd`'s copy folded (the others differ in shape); the two disambiguators differ in mechanism. A20 §1 renames: `Links/connections.ts` → `connectionClicks.ts`, `Links/links.ts` → `linkClicks.ts`, `Session/selection.ts` → `reconcileSelection.ts`, `Properties/Schema/linkFormat.ts` → `linkFormatOptions.ts`, `Desktop/Store/schema.ts` → `ddl.ts`; the other six no longer collide. Defensive sweep: seven envelope catches on channels `serveIpc` cannot reject; the 19 no-nexus guards; `scopeSet`'s `typeof` guard; of 71 `?? []`/`?.` candidates run through the checker, 68 kept — indexed reads of a `Record`/array that TS types present without `noUncheckedIndexedAccess` and which are absent at runtime; `treeIndex.walk`'s pin, `window.matchMedia?.`, the `localStorage` try/catch kept. 178 dead `export` keywords narrowed. Tests removed (4): `setProfileSubtitle` ×2 (its op deleted with the fence; `updateSettings`' foreign-key preservation pinned at `settings.test.ts:63`), `skipTopLevel`, `allStructuralIds`; two repointed (`treePatch.test.ts`'s null-op on `setProfileIcon`; `windowTabs.test.ts` pins the `navOverride` default). Kept with reasons, not bought for the number: `splitAtTile`/`tileIds`/`validateLayout` (the Tiles layout suite's construction primitives, 27 references in `ops.test.ts`), `decorationsFor`/`parseTable` (the equivalence oracles), `closeSession` and `sessionDb()`/`sessionVersionsDb()` (the test escape hatches, 18 and 21 test files), `restoreScrub` as a sweep call (needs a subtree scope the sweep lacks), the sidecar/order layering and `holdersOf` (each pinned by a test), the single-consumer pure-logic view files (folding would drag pure tests into components), 26 dead emitted typography classes (runtime CSS; trimming `ramp()` adds TS for zero ledger movement), `chrome.ts`/`viewRow.ts` filing (zero lines, import churn). Smoke (agent-driven): tree rendered, page body round-tripped and restored, the trash surface's Time Deleted column rendered. Noted for the closeout ledger: the run's counter counts test harnesses as product code (`editorHarness.ts` 184, `pointerHarness.ts`, `testTree.ts`, `propsAtRoot.ts`, `pageValues.ts`, ~300 lines) — the definition is not changed mid-run; ContextPM records it. For Task 20: `npm run dev -- --remote-debugging-port=9333` does not forward through the workspace hop (`cd Desktop && ../node_modules/.bin/electron-vite dev --remote-debugging-port=9333` does), and `POMMORA_USERDATA=~/…` does not expand inside the assignment (`$HOME` does).
- Gate 4 (`eba69eda`): one Opus code-simplifier over `e25db9ea..HEAD`, dual-briefed; 2 files, +4 / −14; code lines 69,009 → 69,000 (−9; the run stands at −459 against base). Cut: `ZOOM_MENU_STEPS` (an identity re-map of `ZOOM_STEPS`), `editorHost.tsx`'s annotated `state` wrapper. The attack cleared the range item by item: every push has a sender and a receiver; `presentRowMenu` settles the standing promise with null before seating the next; `checked` reaches both presenters (native `type:'checkbox'`, in-app `MenuRow.selected`); `NativePickerContext`'s provider matches what each `PickerControl` computed before; `entityMenuActions` is action-for-action complete against the deleted `contextMenu.ts`; every `glance?` reach is guarded; the inert history window blocks doc changes through `readOnly` and the change filter; `intents.ts`/`docScan.ts` cache one fact on one side; `blockContext`'s WeakMap keyed on the scan is sound (a scan is a pure function of the text); ruling 41's viewport slice rewinds to a math block's open line; ruling 43 clamps to 7.5–22.5 pt (a stored 2.0 → 22.5); the header observer guards the absent-header case; `OutlineMenu`'s importers all inside `Interface/Toolbar`; `linkTitles.subscribe` attaches in the ViewPlugin constructor before any paste. One live regression, ruled to Task 16 with its fix: `MarkdownEditor` seats `editorHost.of(hostRef.current)` once at mount and every host member is late-binding except `renderTile`, which closes over the `connections` handed to `buildEditorHost` — after any tree push `PageView` mints a new `ConnectionsApi` and an embedded tile keeps the mount-time one (a `[[NewPage]]` typed inside the embed does not resolve until the outer page remounts); the pre-Task-14 widget read `getConn: () => connectionsRef.current`. Fix: `useEditorHost` holds `connRef`, `buildEditorHost` reads `connRef.current` inside `renderTile`; red test through the harness once it can re-render a changed host. Two Nathan's-call items: `ActionItem.confirm` is write-only (set on two `optionMenu` rows, read by no presenter) though ruling 35 keeps it — a ContextPM open question; `RowMenuHost`/`presentRowMenu` have no desktop caller (`Desktop/main.ts` wires `HostContext.menu` to `popRowMenu` unconditionally) — the phone's seat by Task 13's ruling, ~110 lines unreachable today, visible in the ledger. Latent, not live: `NativePickerContext` does not cross `reactWidget.ts`'s detached roots (nothing under a widget renders a `PickerControl` today); a submenu whose first row carries `separatorBefore` would draw the separator natively and not in-app (no model produces it). Stranded one-file exports handed to Task 15. Tests 334 / 4,049; lint 1,067; Task 14's smoke stood as the gate's.
- Task 14 (`d4369bd2`, Fable): 89 files against `8afea447`, +1,743 / −1,242 (non-test +1,338 / −1,005); code lines 68,710 → 69,009 (+299 by the run's counter — `Engine/intents.ts` 469 new after the `docScan.ts` split lost its rename detection, `editorHarness.ts` 61 → 184 as a full in-memory mounter, `Core/Pages/editorHost.tsx` 138, `api.ts` 84 — against the agent's own read of −115 product / +123 harness; the run stands at −450 against base and Task 15 is sized to clear with margin); tests 332 / 4,041 → 334 / 4,049; lint 1,067. Created: `Core/MarkdownPM/api.ts` (`EditorHost`, `EditorMenuApi`, `EditorSettings`, `TileMount`, `GlanceTarget`, the `editorHost` facet), `Core/Pages/editorHost.tsx` (`buildEditorHost({ pageId?, connections?, inert? })` reading the store lazily; `useEditorHost` memoized on citations shown, `codeblockLineCount`, `pageAliases`; `nativeEditorMenu`, `tileWarmSeam`), `Tables/cellAlias.test.tsx`, `Core/Interface/Menus/connectionMenu.test.ts`. Moved (14 `git mv`): `Editor/input.ts` → `Input/markdownInput.ts`, `Editor/formatKeymap.ts` → `Input/`, caret/caretPlacement/selection/travel to the root, `Embeds/tileCache.ts` → `Embeds/scrollHeal.ts` (the warm seam to `editorHost.tsx`), `Engine/docScan.ts` → `Engine/intents.ts` with a new `docScan.ts` holding the scan + cache (a pure split), `Menus/OutlineMenu.tsx`/`OutlineDnd.tsx`/`outline-menu.css.ts` → `Core/Interface/Toolbar/` (a toolbar surface reading the store; its only importer). `Editor/` gone. Mounters: PageView (`{pageId, connections}` + the header slot), PageTile (`scale={embedScale}`), MarkdownTile, PageHistoryWindow (`{connections: resolveOnly, inert: true}` — no glance, no format menu); the harness builds its own host with `seedHost`/`testHost`/`harnessState`/`settleTitle`, nine suites moved off `useSession`. Rulings red-first: 38 `cellAlias.test.tsx` (`expected false to be true`; `refusedInAlias` in `CellEditor`'s input handler); 39 `autocomplete.test.ts` inside a fence / inline code (`expected {query:'Pro'…} to be null`; `autocompleteQuery(scan, caret)` reads `inCodeAt` on the cached scan); 40 `tokens.test.ts` (code tokens from `inlineSpans`, now exported from `Core/Connections/markdownCode.ts`; `inlineCodeRegex` deleted; `subfieldStats` strips through the same spans); 41 (`tokenize(text, maths)` projects `scan.maths`; `blockLatexRegex` deleted; viewport slices never cut a math block); 42 already so (Detect's `calloutLines` already governed both consumers; the two literal-code lambdas collapsed into `DocScan.literal`); 43 `zoomFontSize(scale) = 15 × coerceScale(scale)` within 0.5–1.5, prop `zoom` → `scale`, the settings surface untouched; 44 the one-row table's comment reworded; 45 already flagged. `blockContext` cached in a WeakMap keyed on the `DocScan` (`blockModel.test.ts`: `expected 3 to be 1` red). `<AutocompletePane ac candidates index onPick>`; `Tables/widget.tsx`'s `index` contract at `transformFor`. Rulings at commit: `renderTile(tile: TileMount)` not `(range)` (the tiles need `editing/locked/ancestors/onBeginEdit/visible/zoom`); `menus.gripHot(hot)` added (the native menu's stand-down had no path) and `menus.format?`/`glance?` optional (history is inert); `ConnectionsApi.glance` removed (Connections never holds Glances), `GlanceTarget` in `api.ts`; `linkTitles.resolve(url): void` (the store's resolver is fire-and-forget; the editor reads through `subscribe`); `settings().pasteInverse` is one chord string; `pickTree()` returns the whole tree and the editor prunes (`embedPickTree`); `EmbedHost.self` removed (every mounter lists itself in `embedAncestors`); the header slot landed without a wrapper (`MarkdownEditor` observes `shell.firstElementChild` when it is not the editor div; `PageHeader` rendered by PageView, eight props left the editor); `EditorMenuApi` in `api.ts`, `FoldsApi`/`EmbedHeightsApi`/`TableHeadingColsApi`/`WarmSeam`/`ConnectionsApi` beside their owners. Markers: `KNOB` 41, `LOAD-BEARING` 1, `(Nathan's call)` 1, `// PLACEHOLDER` 1 byte-for-byte; `biome-ignore` 39 → 38 (the deleted `onRename` prop's). Smoke (agent-driven): every Verify item exercised — pane on `[[`, none inside a fence, native grip menu, footnotes fold/unfold, glance after the dwell, `![[…]]` mounted a PageTile at 13.5 px, `embedScale` 1.25 → 18.75 px, file byte-identical; the scratch Nexus's `settings.json` was rewritten by the zoom set/unset (key absent again). `page:open` still does not raise a window (open ≠ focus).
- Task 14 comment killer (`8afea447`): 74 files, +107 / −492, code byte-identical (token streams compared against HEAD); the editor set's comments 772 → 376 (−51%): source 485 → 342 (Task 11 had already left one why per case), test narration 287 → 34; `Engine/docScan.ts` 22 → 14; max file 17 (`Embeds/embedWidget.tsx`, every line a CM6/Chromium trap); `KNOB` ×8, `LOAD-BEARING` ×1, `biome-ignore` ×17, `@vitest-environment` ×26 byte-for-byte.
- Task 13 (`ab246bac`, Fable): 98 tracked files +1,299 / −2,043 plus 16 new (non-test 79 files +972 / −1,735); code lines 69,193 → 68,710 (−483; the run stands at −749 against base); tests 322 / 4,003 → 332 / 4,041; lint 1,068 → 1,062 files. Removed 23: `Core/Actions/tileMenu.ts` (+ test), `Desktop/Actions/poppers.ts`, `contextMenu.ts`, `styleMenu.ts`, the 18 per-surface poppers; `menu.ts` → `appMenu.ts`. Created: `Core/Actions/createMenu.ts`, `entityMenu.ts`, `Core/Interface/Menus/RowMenuHost.tsx` (64), `rowMenuRows.ts` (22), `Core/Interface/Sidebar/entityMenuActions.ts` (89), and eleven tests (eight models, `rowMenuRows`, `RowMenuHost`, `Core/Tiles/tileHandleMenu.test.ts`). Channel map: Asks 131 → 106 (menu asks 26 → 1, `'row-menu'`), Pushes 14 → 7; `HostContext` loses `contextMenu` and `MenuChannel`, `menu(req: RowMenuRequest)` serves the one channel. 37 call sites on `popRowMenu(xMenuItems(ctx))`. Rulings at commit: (1) no `useMenuPresenter()` hook — ruling 31 leaves it no in-app case on desktop and ten of the 37 sites are outside React (CM plugins, the store, plain functions); the presenter choice sits on `HostContext.menu` (Desktop pops native; a host without a popper answers with `presentRowMenu`, the Mobile plan's own sketch); `useNativeMenus()` stays for the two pref surfaces. (2) `presentRowMenu` is a `chromeSlice` action (`pendingRowMenu`, the `askConfirm` pattern), not a module singleton — the locked no-global-client-state rule. (3) Object-shaped actions became strings (`GripMenuAction`, `TrashMenuAction`, `TrashColumnAction`, `create:<i>`) since the one channel carries `string | null`; type names kept. (4) `cellMenuModel`/`cardMenuModel` return `ActionItem[]` laid out as the poppers laid them; `StyleMenuItem`, `CellMenuModel`, `CardMenuModel`, `OptionMenuContext`, `MenuChannel` gone; `destinationRows` is Core's twin of the deleted `destinationNodes`; `alignRows` shared by column and table menus; `afterSeparator` and `createdRequest` the only helpers added. (5) Ruling 34: `tileMenuItems` in `TileHandleMenu.tsx` reads the pane's own inputs; `TileHost`'s native path calls it. (6) `PickerControl` takes its native list from `NativePickerContext`, provided once by `App` as `useNativeMenus() ? popRowMenu : null` — every Core call site identical, UIX imports nothing from Core. (7) `UIX/tsconfig.json`: the dialer include reverted; `types: ["vite/client"]` kept (`import.meta.env.DEV`, `.css` imports) and `skipLibCheck` kept on its own merit (`@tabler/icons-react`'s d.ts fails against `@types/react` 19 without it). (8) Sidebar and band picks run through the store's `mutate` (the one renderer write path: optimistic patch + main's confirming push); the old `reload-state` full reload after each sidebar mutation is gone; a failed mutation reports through `mutate`'s existing `error:show`, which is what the sidebar showed before — `notifyError` not added (ruling 6 keeps the two as they are; the ContextPM open question). (9) Ruling 32: `checked` → a checkbox item everywhere; identical on macOS (Electron draws both states as ✓); on Windows/Linux the five ex-radio groups (column Align/Style, grip Size/Scale, trash Format) would draw ✓ instead of ● — Task 20's Windows note. (10) Two wire-validation coercions in the old `Core/Actions/handlers.ts` dropped as unreachable in-process. (11) Twelve suites that stubbed per-surface channels now stub `'row-menu'` and assert `{ items: xMenuModel(ctx), anchor }`. The Becomes' "14 existing model tests" was 13; `tileMenu.test` left by ruling 34; `ls Core/Actions | grep -c Menu.test` → 18. `Desktop/Bridge` holds no channel literal by construction (it iterates the handler map), so the Verify control `rg -F "'row-menu'" Desktop/Bridge ≥ 1` cannot be met there; the literal sits in `Desktop/main.ts` and `Core/Actions/handlers.ts`. The editor context menu untouched (ruling 37); `Core/MarkdownPM` touched at three channel repoints only. Smoke (agent-driven): native `row-menu` fired un-awaited → Escape resolved null, ↓⏎ resolved `'a'`; in-app (dev mode, `POMMORA_DEBUG_PORT=9333`) `presentRowMenu` rendered the pane with its rows, Escape → null, click → `'a'`; page body round-tripped and restored byte-for-byte. For Task 20: `npm run dev -- --remote-debugging-port=9333` does not forward the flag through the nested `npm run -w Desktop`; `POMMORA_DEBUG_PORT=9333` is what arms CDP in dev.
- Task 13 comment killer (`7b6a6ed1`): 55 files, +113 / −413, code unchanged (`git diff -U0` filtered to non-comment lines empty); the menu set's comments 515 → 233 (−55%), max file `Core/Contract/bridge.ts` at 18 (its section headers gone but `// Per-machine scopes (nexus.db rows)`, the one stating what the prefixes don't), every other file ≤ 10; `KNOB` ×8 and `biome-ignore` ×11 byte-for-byte.
- Gate 3 (`e25db9ea`): one Opus code-simplifier over `de8c32b8..HEAD`, dual-briefed; 5 files, +30 / −63; code lines 69,223 → 69,193 (−30; the run stands at −266 against base). Cut: `fileTypes.ts`'s 23 extensions written twice after Task 12's named-import rewrite (`TABLER_FILE_TYPES` the one list, `FileTypeExt`/`FILE_TYPE_EXTS` derived), seven `theme-vars.css.ts` constants with one reader each inlined where they are set, five hand-rolled clamps on `UIX/Utilities/clamp` (`pageEditor.ts`, `Tiles/Layout/ops.ts`, `webGuests.ts` left: a UIX import into a pure engine module or the main process for one clamp). The attack ran the checklist to conclusion and found zero bugs: every `var(--…)` read across Core/UIX/Desktop resolves (the `--tint-*` ladder is generated from `TINT_STEPS`; `--menu-max`/`--trio-w` are set imperatively); `paneMaterial`'s emitted style byte-identical to the deleted literal; `Engine/`'s full transitive closure (29 files) reaches nothing from CodeMirror or React; `docScan = perDoc(doc => scanOf(docString(doc)))` keys the same on both sides of the split; the `gesture.ts` migration's only unlisted difference is `group.tsx`'s `onScroll` no longer firing before activation (harmless: `frozen` is empty until then); every `vi.mock` string, stylesheet import, and `url()` resolves; `.tabs-compact` applied at `WindowTabStrip.tsx:80`; `menuCompact` cascades from an ancestor; both `PagePropertyRows` variants reachable; the six `tree.contexts ?? []` removals safe (`readNexus.ts:405` always writes an array). Left because each needs a new symbol: `NexusPhoto.tsx` ≡ `HomepageSettings.tsx`'s icon-editor pair (~27 lines), the `<AutocompletePane>` call in `MarkdownEditor.tsx` ≡ `CellEditor.tsx` (the pane should take `ac`; Task 14's), the `useInsertionDrag` JSX tail in `sidebarDnd.tsx` ≡ `OutlineDnd.tsx`, `toggleLink` ≡ `toggleConnection` in `Input/format.ts`. Kit barrels `Menus/index.ts`, `PickerControl/index.ts`, `Theme/index.ts` are public entry points and stay (the first two Task 13's). Resolved from earlier carries: `cards.css`/`over-scroll.css` now one importer each; `Interface/styles.css` owes UIX/Theme nothing. Six files over the 20-comment cap (`Table.css` 41, `tile-base.css` 40, `window-base.css` 32, `Contract/bridge.ts` 32, `Engine/docScan.ts` 29, `markdown-tables.css` 25): the reviewer read all six and cut none — `biome-ignore` directives, `KNOB`s, and whys the code cannot state; the closeout comment killer re-reads them. `Engine/docScan.ts`'s name understates its contents (two thirds decoration-intent building; ex-`Decorations/intent.ts`) — Task 14 names it when the host lands. Tests 322 / 4,003; lint 1,068. Task 12's smoke stood as the gate's.
...
The run closed 09-06-2026 at `60603de2`: 69,459 → 68,679 code lines on the run's counter (−780; −1,002 excluding test harnesses and setup files), 334 test files / 4,050 tests, lint 1,069 files, five typecheck projects, the Showcase building. Gates: 1 `90e2436b`, 2 `de8c32b8`, 3 `e25db9ea`, 4 `eba69eda`, 5 `2053ee50`. Closeout: comments `12f52789`, the Delivery Claim `29c84ba1` (neutrally verified), the attack's fixes `817e967e`, the simplification `60603de2`. History PM-129 || The Repo Restructure; ContextPM carries Nathan's own pass as the one pending item and the open calls the run left; the Line-Ledger rebuilt at the new sources. Pending: Nathan's own pass.
