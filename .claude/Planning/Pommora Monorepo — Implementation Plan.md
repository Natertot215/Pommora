...
- Phase 3 is closed (Gate 3 `e25db9ea`, 69,193, −266 vs base); the Phase 4 notification was sent; the comment killer over Task 13's files landed (`7b6a6ed1`). Task 13 was dispatched to a Fable agent owning the whole tree (rulings 31–37 over the Becomes: native stays the desktop default, `checked` the one flag, `confirm` stays, sidebar/band picks in the renderer, the editor context menu native and whole; the PickerControl edge severed and UIX/tsconfig's hacks reverted). Land it as Task 13's commit, tick, record, ping; then the Opus comment killer over Core/MarkdownPM's host-touching files and Task 14 on Fable.
