...
- The whole-tree simplification pass is committed (`9c9b994e`, 69,433, −26 vs base). Task 11's MarkdownPM review agent (id a60c80c587e9e67cc) was still live; it owns `Core/MarkdownPM` plus `Core/Pages/pageEditor.ts` (import repoint) and `Core/Connections/connections.ts` (`/dg` flag). Land it as its own commit, then (b) and (c) of Task 11 below.
