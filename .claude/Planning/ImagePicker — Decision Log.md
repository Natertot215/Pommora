...
- **A-4:** [confirmed] (Nathan, 08-25) Two frame shapes on the same crop model, the shape split by geometry. **`rect`** (banners, card covers): the frame *is* the viewport — `280 × round(280·boxAspect)`, ring radius `12px`, no blurred margin — so the picker shows the seat's exact paint, per the mockup. **`circle`** (the nexus icon and the rounded profile photo): keeps `PhotoCropModal`'s current geometry — a `220` circle inside the `280` viewport with the masked-surround blur, ring radius `50%`. `coverStyle` resolves against the circle's `220` box or the rect's own box either way, so the split is display-only; the surround is a decorative second paint the circle keeps and the rect drops.
- **A-5:** [confirmed] (revised Nathan, 08-25) The footer's path field echoes the image being edited (`InputField chrome="bordered"`, a leading glyph, `<NavTrail segments={pathSegments(path)} />`, the recipe `Settings/AssetDirectoryRow.tsx` and `Components/Detail/FileEditor.tsx` share) **and re-picks it**: a trailing action opens the OS file explorer exactly as `AssetDirectoryRow`'s does, and a paste sets the image from the clipboard. Both change the picker's image — trailing/paste run pick-or-clipboard → adopt → the seat's set op → the picker reframes on the new reference (which Task 7's draft already resets on a `value` change). Paste writes the pasted bytes as a **normal asset file** and keeps the crop separate, so it stays on the path model and does **not** revive Task 6's deleted crop-baking byte channel; a copied file pastes by its path. The field is no longer read-only.
