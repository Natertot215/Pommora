...
- **A-4:** [confirmed] Two frame shapes: `circle` (the current geometry) and `rect` with ~12px radius. Both are the same crop model; the shape is the mask and ring only — `photoCropModal.css.ts:50`'s hard `border-radius: 50%` and the radial `surroundMask` at `PhotoCropModal.tsx:95` are the two spots the shape parameterizes; the rest of the stylesheet is reused as it stands.
