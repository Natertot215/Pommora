...
- **Task 6 — the plan's `writeImageAsset` trust census is wrong at one site.** The plan gates `isUlid` at the page-banner call only, on the premise that every other caller passes a trusted constant. `mutate.ts:378` assigns `assetKey = existing.id` read off a container sidecar through `readJsonObject` — arbitrary disk content, the same untrusted class as page frontmatter, reaching the same `join(root, '.nexus', 'assets', assetKey, file)`. Genuinely trusted: the nexus id from `ensureIdentity`, the `''` literal, the `'homepage'` literal. Live census: every container sidecar id on both nexuses is ULID-shaped, so a guard breaks nothing on real data.
- **Task 9 — `baseSidecar` is the right reader for an agenda config**, but it is loose and non-discriminating: it validates any sidecar. It may extract the id from a file already located by filename; it must never stand in for the kind decision.

#### Task Log

- **Gate 1 — green, Phase 1 closed `600054e8`.** 171 files / 1853 tests; typecheck · lint · build all 0. Four more comments describing deleted machinery swept in the gate commit: `treeIndex.ts` (agenda rides a snapshot, not the tree), `shared/mutate.ts` (the three-scope cascade naming agenda JSON roots), and `store.ts`'s two nav guards, whose rationale was phrased as a wait rather than a reason. Phase 1 net, code-only: 42 files, +140 / −1368.
- **Task 4 — done.** App confirmed fully stopped before deletion; all four folders moved to the system trash via Finder (Put Back intact); re-enumeration returns zero on both nexuses; no tolerance code written. The `_type` taxonomy was corrected to Nathan before deletion, recorded in the Decision Log's Schema Scrub Inventory, and all four config files snapshotted so a trash empty can't lose them.
- **Task 4 — held, app running (superseded).** The dev process (electron-vite + Electron main/renderer) is live; the plan's first checkbox is "App closed (both nexuses)." Hold is correct. **Enumeration finding carried forward for the Agenda Rethink:** both NexusOS agenda configs carry a second, user-authored `_type` Select property that exists nowhere in code and is not recoverable from `defaultStatusSeed()` — Tasks: Task / To-Do / Phase; Events: Event / Meeting / Appointment. Nathan's own taxonomy, and a real design input for the agenda schema scrub. The test-nexus configs carry Status only. All four are Swift-serialized and key their schema under `properties`, not `property_definitions` — the deleted `agendaConfigSidecar` could never have read them.
