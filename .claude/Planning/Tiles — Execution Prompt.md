...
Execute `.claude/Planning/Tiles.md` — the ratified Tiles implementation plan. Its spec is `.claude/Planning/Tiles — Decision Log.md`; read both whole before touching anything, then `ContextPM.md`, `Guidelines/Editor-Internals.md`, `Guidelines/Cohesion-Rulings.md`, and `Guidelines/Lint-And-Accessibility.md`.

Preconditions, verified before Phase 1 opens: `git log --oneline -5` shows the Menu Recipe's closing commit — that hash is Phase 1's base and goes in the Log's Progress tree first. `git status` is clean. `npm run typecheck`, `npm run lint`, `npx vitest run` are green from `Pommora/`; write the test count into the Baseline invariant. The D-6 ruling (Task 12) is in the Log's Rulings — if it isn't, ask Nathan before Phase 2 opens, and take the declined branch the plan writes if he says no.
