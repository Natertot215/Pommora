...
- `Pommora/src/renderer/src/Components/Detail/PagePropertiesPane.tsx` + `pageProperties.css.ts` — the Settings dropdown's Properties leaf. Rows are `(icon)(label)(value)`, value `margin-left: auto`, grouped into two `fill.tertiary` field blocks, with an Add Property picker.
