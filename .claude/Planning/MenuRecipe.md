...
**Derivation (re-counted at `c67b36b8`):** `largeTitle` 3 · `title1` 7 · `title2` 7 · `title3` 25 (the Phase 2–3 migrations added `Showcase/lab/Surfaces.tsx:306`, `ContentView.tsx:214`, `ViewFrame.tsx:158,163`, `NavList.tsx:133`, `LayoutFrame.tsx:335`, `frames.css.ts:36`, `PropertyTypes.tsx:51`, `DateTimeEditor.tsx:35`, `window-base.tsx:237`, and `size.css.ts:53,63` feed the button ramp) → each 0; `--icon-body` has a fourth reader at `Styles.css:217`; `--icon-picker-w/h` match the `--icon-` grep and are not ladder vars. `grep -rF "largeTitle" src`, `"title1"`, `"title2"`, `"title3"` → each 0 after (the `Symbols/index.tsx:230` doc line reads `headline`); `grep -rF -- "--icon-" src` → the three `--icon-body` readers + its one definition. Control: `grep -rF "ICON_PX" src` ≥ 3.

**Steps:**
- [x] Rename the four rungs and delete the fifth; repoint every reader; drop the ten unread vars.
...
- 08-27 (Nathan): the Trash's row affordance sits in the lead gutter as the NavList pin does. The trail pads through `--trail-pad`; the cards' uneven bottom padding is a defect to diagnose, not pad over.
...
- Round 7 (build-breaking-agent, the whole range `27c5171c..9d2826c0`, 08-28 — landing round 1 of ≤3): 0 High · 0 Medium · 1 Low (pre-existing) · 21 kills. The Acceptance measured on the built CSS for every surface the claim names plus the autocomplete (23) — Round 4's unknown closed. The Low: `Sidebar.css` keyed five rules on `[class*="titleText"]`, which matches vanilla-extract's dev identifiers and not the production hash → the rules key on `.over-scroll-ellipsis`, the plain class the same span wears; a `grep -rn 'class\*=' src --include='*.css'` is now zero and stays a guard. Two items for Nathan's pass: a pin reorder by drag in NavView, and the packaged build's rename field (whether it was narrowing or vanishing under the dead rules). No round 2 is warranted — nothing above Low surfaced.
