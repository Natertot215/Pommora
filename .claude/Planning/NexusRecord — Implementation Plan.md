...
- [x] **Phase 4** — the resolver spends the pairs · base `0d2695fc`
  - [x] Task 10 — one reconciliation loop · `a7d85d0d`
  - [x] Task 11 — the resolver · `ff29e53d`
  - [x] Task 12 — restore · `0562a4a6`
  - [x] Task 13 — the record tells the docs · `82aee012`
...
- **Needs Nathan's ruling (Gate 1 finding, sharpened by the Gate 4 attack — code shipped with the proxy plus a mitigation):** `latchBaseline` is pure, so A-5's drop condition "the recorded path no longer exists at all" is implemented as "no walked claimant sits at the recorded path and it is not on the unreadable list." Wider than the spec's literal condition in one edge: a duplicated id whose recorded home left the walk for a *non-destructive* reason (its subtree added to `excluded_folders` while closed, or the id key hand-stripped) drops its evidence. The attack pass proved the ordinary trigger is a rename-plus-copy in one closed window, and that the subsequent record-one pick decides which twin keeps the id — so record-one now records the **eldest claimant by birth time** (a copy is born after its original; birth time survives a rename), which makes the ordinary case converge on the true original. The residual exposure is the exclusion edge plus filesystems that do not carry birth time. Ruling wanted on whether that residue deserves more.

### Deviations
- **Gate 4 (closeout):** simplifier landed 2 (list-taking `disambiguate`, one-join `artifactBaseName`); comment pass removed nothing (every added comment held as why-only) but caught one unused local its gates surfaced — which also exposed that `biome lint` exits 0 WITH warnings, so gate checks read the count line now. Correctness review: 2 Medium folded — a Context now **re-enters the registry before anything moves** (a swallowed append failure had let a restore reply ok with its evidence destroyed) and the final-title-under-collision round-trip is pinned end-to-end; kept membership entries are named in a warning, never silently claimed. The **attack pass then confirmed five crossings no round had run**, all folded with red-first tests: (1) a disambiguated Context restore now **re-keys its own passengers** to the final title — the passenger skip and the resolver's rename had contradicted each other, leaving in-subtree links pointing at whoever took the recorded title; (2) record-one now records the **eldest claimant by birth time**, not walk order — a copy is born after its original and birth time survives a rename, so the copy re-mints and the live original never does; (3) a failed move **rolls the registry re-entry back**, closing a self-poisoning ghost entry that tripped the retry's own id-live guard; (4) the mover **contains its target** — a plain basename, exactly in the resolver's chosen dir, inside the nexus, outside the trash — so a hand-edited pair title cannot steer user data out of the nexus; (5) the pair records the artifact's **original basename** (`name`), because the stamped leaf cannot be parsed back reliably for a user's own `12__Notes.md`. Recorded, not fixed (attacker's observations): a pair whose id has a live twin refuses forever and never prunes (correct per E-1, dead weight until a surface can say so); cross-kind id collisions share one baseline namespace (hand-edit-gated).
...
**Delivery Claim** (adjudicated against the spec's Core, below): every Core item ships and is end-to-end tested. (1) The pair — shape, parent union, per-kind payloads, the artifact-less property variant — written best-effort by the delete arm from the four gather points, all-or-nothing per pair, partial-marked whenever thinner than the truth. (2) The resolver — a placement with final names or a typed refusal, id-live outranking all. (3) The restore op — IPC-reachable via the mutate channel, headless, spending pairs through the resolver; a Context re-enters the registry before anything moves; membership re-applies through the shared reconcile loop joined by id. (4) The baseline — one explicit pre-watcher walk per genuine open, union-diff with three-state existence, last-non-empty drift, silent. (5) The re-mint — content and container, baseline-adjudicated, evidence-preserving, copies duplicating device rows with payload ids re-minted, originals untouched by construction. Consolidation: snapshot absorbed, residue removed (`removed_at` 0 · `cascade.ok` 0 · control 15), one reconcile loop, one tuple owner, the sweep skip + honest widening. Acceptance's four sequences run as tests. Closing state after the closeout folds: typecheck 0 · lint 0 warnings · 1,994 tests / 179 files · build clean.
