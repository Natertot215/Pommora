...
Separately, `GlancePane.tsx:87-94` hand-rolls an insertion-order cap (delete → set → while-loop evict) that PM-126 (`ContextPM.md:91`) collapsed elsewhere onto `capSet` (`PommoraUIX/Util/capMap.ts`, used at `Store/tabState.ts:28,41`, `webRetention.ts:18`, `docCache.ts:35`). This one survived. Removable: 6 lines → 1 `capSet` call. Confidence: high.
