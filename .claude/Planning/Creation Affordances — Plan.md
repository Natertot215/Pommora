...
- **Deviations (Phase 4):**
  ...
  - Phase 5 — 5.0–5.4 ✓ · the acceptance pass and Nathan's ten-scenario interactive matrix ran live in the Test-nexus sandbox (all pass; the fully-filtered view offering no "+" is the pipeline's own honest answer); the final breaker's five findings and one unknown all fixed and re-verified (positional optimistic splice + test, ghost grace both ways, dwell re-checks the editor, the grip outlives its drag, two doc rewords); comment audit cut five narration lines; purge clean; gates 207/2319 green; atlas green. Nathan's live rulings folded mid-run: `--state-inactive` became a bare 0.55 in the opacity ramp worn over the standard label, and `--state-disabled` was removed outright, its six consumers adopting inactive. Teardown complete — the throwaway Collection deleted through the app, its trash bundle scrubbed by record-verified path, the minted-for-testing registry file removed, `lastNexusPath` restored to NexusOS, and a clean dev instance left running on the new build.
