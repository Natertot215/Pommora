...
- **Resolution-aware, deliberately:** the widget is resolved-only, so **token suppression must be too** — a blanket lone-line suppression would strip the dim-token styling from an *unresolved* lone-line `![[Nowhere]]` and show raw syntax, breaking A-5 and the deleted-target degrade (round-2 finding, executed). The line-construct gate (no lists/headings/rails on embed lines) reads the pure derivation; the token suppression is applied in `editor/decorations.ts`'s `build`, which already holds `conn` and resolves wikiLink statuses — suppress only where `resolve(title).status === 'resolved'`. The pure derivation and the widget therefore deliberately **disagree** on unresolved lines: block kind + grip are resolution-free (an unresolved lone-line keeps its grip, so Page Source ▸ can aim it), the tile and the suppression are resolution-gated.
- **Files:** `decorations/intent.ts` (the `inEmbedLine` construct gate); `editor/decorations.ts` (the resolution-gated token suppression).
- **Negative controls:** a non-lone `![[x]]` keeps today's inert inline treatment under the renamed class; an **unresolved lone-line** keeps the dim-token treatment (this is the deleted-target render — pin it); both pins go red with their gate mis-applied.
...
- **Accent gating stays host-owned:** the shared modifier carries only the color binding; **each host applies its own trigger selectors** — SurfacePM keeps its full gated selector including `.blk-surface:not(.is-host-locked)` (the host lock suppresses the accent on locked surfaces — round-2-verified that a bare re-scope loses it), the embed applies hover/editing per C-3/C-4. One color definition, per-host reachability.
