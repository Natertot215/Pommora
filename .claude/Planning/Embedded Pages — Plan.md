...
**Task 6.2 — Feature doc.** MarkdownPM.md gains the embed construct section (or a new `EmbedsPM.md` if the section outgrows it — judge at write time); ContextPM.md's AutocompletePanel lesson line is deleted (resolved by 5.1); the decision log's role ends — the plan and Features docs carry everything forward.

**Task 6.3 — Closeout.** Two dispatches, never one: (1) a neutral verifier adjudicating the acceptance criterion against the running app, phase by phase; (2) the attack reviewer against the full diff, carrying the doc-flagging mandate. Every concern fixed or Nathan-ruled. Then simplification + comment-killer passes over the full range, `HandoffPM.md` via `/handoff`.
...
- Phase 5 base: `ae0f97df` · shipped `63f3169b` (autocomplete/slot/cascade/nudge) + `077a8cf5` (simplification; the panel's z rung restored) + `89f20d2d` (attack folds). Gate: 2160/2160 · all green · live pass (typing `![[` → filtered panel → Enter mounts the tile + writes disk; a DISK rename degraded the tile with zero interaction and the rename-back restored it — the nudge both ways).
- Phase 5 deviations & rulings:
  - **The rewrite's embed pass re-masks its own string** — chaining two replaces over one code mask misread offsets on any length-changing rename (live embeds skipped, fenced samples rewritten — executed both ways); the regression pin now renames across lengths, because same-length fixtures cannot see a mask misread.
  - **A rename clears the whole detail slot and refetches the open page** — the cascade rewrites bodies nexus-wide, so every cached body is suspect, and the open pane's detail is the same fact in another home; without this a rehydrating tile wrote the pre-cascade body back over the heal. (The open-pane half was a pre-existing latent made visible by embeds.)
  - **The block sweep's prefilter learned embeds** — `mentionsTitle` at both call sites; a markdown block holding only `![[Old]]` now heals on rename.
  - **The panel's class stops declaring position** — a fixed inner pane left NotchedPane's measured wrapper zero-sized, so the frame/shadow/clip never rendered; positioning + the z rung ride the wrapper inline.
  - On record: inner tile editors are beyond the nudge's reach (widget eq() keeps them clean; self-heals on any doc change touching the tile). Walkthrough seeds += the panel's flip-above math with the new wrapper · Sapphire's `![[` grammar mirror (grep its tree).
- Resize arc (post-plan, B-15): base `8eec9cab` · shipped `41bbbfe3` (feature) + `0932778c` (simplification fold) + `2a8b4ad9` (breaker folds + the live UIX round). Gate: 2163/2163 · all green · live pass (drag 521→441 persisted across reload; per-target keying held). Simplifier catches: handle inside Suspense (absent for the loading frame — moved out), a fixture missing `nexus`/`personalization` flaking a sibling suite. Breaker: 2 Medium + 2 Low, all folded — the drop commits the drag's computed height (a detached rect's 0 would poison the page's map), the heights load merges under any already-dragged value and sequences the warm scroll restore, a deleted tile's root unmounts once connectivity settles. On record: nothing can clear a single height yet (the grip menu has no Reset — the empty-map delete path waits on one); the 8px strip overlays the embedded page's last 8px of clickable text.
- Live UIX round (Nathan driving): the banner parks on the tile's own scroll (the shell's timeline-scope stood down inside a tile, re-homed at `.pgembed`, the page's own keyframes retargeted); fencing blanks + the tile line's strut + CM's 1em widget buffers all render closed (13px symmetric block gaps, was ~60); a click on the tile's line seats by nearer edge (`embedClickSeat` — CM's atomic default always snapped backward, teleporting bottom clicks above the tile); the drawn caret survives boundary seats (assoc flip on a null side, floor at a collapsed seam). Root-caused, not patched: the caret vanish traced to `RectangleMarker.forRange` returning nothing when `coordsAtPos` nulls toward a replaced range — CM's native cursor layer shares the hole.
- Post-arc review round (the previously-unreviewed surface, `0932778c..d9fd3647`): simplifier then breaker, both fully adjudicated. Simplifier: `TILE_MIN_PX` + `--accent-stroke-hot` minted into the design system (both SurfacePM + embed consumers), caret repairs collapsed to one condition, seat handler early-outs, park range reads its own variable — and it caught the seat handler's hand-rolled Y conversion omitting the content's top padding (fixed via `documentTop`, proven on a 120px tile where the bias mis-seats). Breaker (`9065aa84` folds): a page ending in a tile handed the seat handler its whole bottom padding, killing drag-select/shift-extend there — the band gate returns padding presses, extends, and multi-clicks to CM; `scopeGet` self-wraps like `nexus:state` so no scope read rejects; the mount loads settle independently. 12 candidates killed with evidence (fence ordering, destroy lifecycle ×5, caret floor blast radius, token import cycles, timeline scope leaks, grip/drag interplay). Held deliberately: the scroll-park property block (2 sites), the `.spm-edge` family relocation (a standalone refactor, offered), `.md-checkbox-checked` (a fill, not the hot stroke). Walkthrough item 11 carries the real-mouse selection checks no harness can drive.
...
11. Selection near tiles by real mouse (synthetic drags can't drive CM's mouse selection): drag-select from prose above a tile to prose below; shift-click and double-click beside a tile; drag-select in the empty space below a page that ENDS in a tile; and on such a page, click just below the tile — a caret should appear. A banner tile inside the floating preview should simply keep its band static while the preview scrolls.
