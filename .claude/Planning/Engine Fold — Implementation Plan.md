...
- **Typecheck was red from `dcdd95c58` through `b018edc41`.** The branch's `engine.test.tsx` typed its spies as `ReturnType<typeof vi.fn>`, which Vitest 4 resolves to a non-callable mock, and every gate report in between read green on typecheck without it being so. `1148820e5` types the spies by signature, renames `useZoneItem` to `useDragItem` at its source so the façade re-exports without an alias, and moves the pure `placeCell` and `keyboardNext` tests into `engine.test.tsx` and a new `keyboard.test.ts`, retiring the only `.test.ts`/`.test.tsx` twin in the tree.
