...
- Post-plan review (two simplification agents, two correctness agents, `08bbc0f5`): narrowing the push exposed that `movePage`/`moveSet` never noted a value write (pre-existing; the whole re-read had been healing it) — fixed like `createPage`; a parse straddling `forgetParse` could cache the retired bytes under an unchanged (mtime, size) — the cache now refuses an entry whose parse spanned a forget; the scoped merge ignored a container swap and retired overrides for pages it never resolved — both guarded; a denied `utimes` aborted a sweep mid-corpus — now best-effort. Five simplifications folded (one `useContainerValues`, one `FootingItem`, generic option-field setters, the picker base peeling one pane per Escape, `ViewRow` stamps as `string | null`). Left open: a whole read in flight at mount can land over a newer scoped merge (narrow: an edit inside the mount read's window); a page dropped in mid-session and body-edited before adoption seeds its id from the edit's inode, not the file's age.
