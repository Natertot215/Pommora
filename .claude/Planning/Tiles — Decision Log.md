...
- **C-2:** [confirmed] The closed entry union becomes the tile recipe, keyed by one `TileType` in three arms: `TILE_KINDS` beside the schemas in `shared/tiles.ts` (schema, file-backed, menu rows), `TILE_COPY` in `main/tiles.ts` (the view re-mint needs `newId`, main-only), and `TILE_SURFACES` in `Tiles/tileKinds.tsx` (surface, source identity) — `shared/` compiles under the node project with no React. `TileHost`'s render switch, `tileMenuModel`'s three branches, and `TileHandleMenu`'s five kind checks read the table. This is one file and a lookup, not a plugin system; a later panel kind (properties, backlinks) or a list kind is one entry plus its surface.
...
- **C-4:** [confirmed] A host is a folder, and a markdown tile's body is a `.md` in it — the existing rule. A Collection folder is corpus, so a Collection tab's bodies live in a `_`-prefixed subfolder the walk refuses (a bare `<ulid>.md` there would be adopted as a page); the Pages tab and each custom tab get a folder under `.nexus/inspector/`. The Homepage already works this way (`.nexus/homepage/`).
- **C-7:** [confirmed] The document leaves `nexus.db` for files, and Space and Homepage layouts go cross-device with it (mobile and sync are near-term). It is one `_tiles.json` in each host's folder, its own file with its own lock key: the scout showed `_space.json` and `homepage.json` have no schema, four writers rebuild them whole, two write them unlocked, and the layout debounce would make the document the hottest writer on the identity file. The existing `local_state` rows migrate into the files once on open, file-wins. (Ruled 09-04-2026 under Nathan's "past comments are inputs, not constraints.")
...
- The document in the host's `_tiles.json` (C-7), the nexus-wide configuration key in an existing `.nexus/` file (C-9), the cap constant and the warm-seam reuse declared (C-8, C-10); the existing Space and Homepage rows migrated once.
