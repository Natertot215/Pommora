...
- **A-2:** [confirmed] Lone-line only — the embed resolves (and its menu appears) only when it is the line's sole content, so it can never clobber surrounding text. Mirrors the display-math lone-line model, trailing whitespace tolerated — a *leading* indent is list-continuation context, so an indented `![[…]]` rides its bullet as the inert token rather than tiling mid-item, the same hosted-context rule quotes and callouts follow (F-3). Detection is a whole-doc, per-version derivation on that same pattern — the embed regex over lone lines, fence and table regions excluded, cached beside the other doc derivations — because the tile's StateField needs doc-wide ranges the viewport-scoped token pass can't supply. The token pass keeps styling non-lone occurrences inline, as it does today.
...
- **B-9:** [confirmed] The embed line is skip-over — the atomic range absorbs the line's boundaries so the caret can never rest on it, and no keystroke can break lone-ness from an invisible seat. The remaining seam — an embed as the document's first or last line — gets proven with the executable harness before anything builds on it.
- **B-13:** [confirmed] Embeds are **blank-line-fenced against adjacent content**, the discipline tables already ship: an insert lands with its separating blanks, and the lone blank between an embed and its neighbor is guard-protected, which dissolves the adjacent-embeds seam outright. The fencing blanks stay **ordinary visible lines** — deliberate, because they're the caret's legitimate home beside a tile; a blank visually consumed by the tile would host an invisible caret seat, recreating the exact problem B-9 removes. The tile reads flush through the box-construct outer-gap convention, not by swallowing lines, and a document-edge embed carries no forced padding line.
...
- **B-12:** [confirmed] The `[[` autocomplete's mechanism extends to `![[` — today the panel's trigger pattern deliberately excludes it, so typing an embed by hand currently gets no completion; the embed reuses the same panel with B-11's filter. The extension is a second pattern local to the trigger detection, never a widening of the connections pattern itself (its four consumers deliberately treat `![[` as not-a-connection), and it must not leak into the table-cell call site that shares the hook.
- **B-14:** [confirmed] While the panel is open for that work, the standing consolidation rides along: AutocompletePanel's hand-rolled body portal re-homes onto PickerMenu's beak-less surface — the flagged lesson, resolved here because this feature is the panel's next consumer.
- **B-15:** [confirmed] **Full tile resize is core, not a prospect** — a bottom-edge drag sets each tile's height, persisted per-machine in `nexus.db` as a new `local_state` scope on the folds pattern, keyed **(host page id → target page id)**: stable and unique because a page embeds at most once per document (B-11). Width stays the gutter box; the KNOB height is the default for tiles never resized. The per-embed zoom prospect is unchanged — this decision is height only.

#### C — Chrome & Signals

- **C-1:** [confirmed] Banner rule follows the page itself: a page with a configured banner shows it with the in-line title; a coverless page shows the hover breadcrumb instead. No per-embed storage. The embed's header is **display-only, banner-only in its affordances**: the banner renders with the page's title as static text over it, and neither is editable from the embed — the rename field stays a page-surface affordance (an in-embed rename would need the host's own `![[Title]]` re-resolved as part of the gesture, complexity the feature doesn't buy). An existing banner keeps its **change-banner** affordance inside the embed — the page header's own control on the same banner mutation, whose warm-detail invalidation already refreshes rehydrating tiles — while the coverless add-banner strip stays excluded.
