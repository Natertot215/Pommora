...
- **A-4:** [confirmed] Glance-only: never editable, wheel scrolls, text selection harmless; clicks inside the card do nothing. Whether an in-card connection click should instead open a preview is parked for Nathan's future call — "do nothing" is the ruling for now.
...
- **B-2:** [open — needs Nathan] Resize rides the SurfacePM edge/corner chrome and gesture skeleton, but only on the card's **free edges**. An anchored pane has pinned edges — under the ratified anchoring, dragging a pinned edge moves the *opposite* edge, growing the card away from the cursor. Recommendation: pin the card's left edge to the link (`origin: 'left'`), giving honest zones on the right edge, the free vertical edge (bottom when placed below the link, top when flipped above), and their corner — horizontal + vertical + diagonal, tile-style strips and cursors, minus the zones that cannot physically track the pointer. The alternative — all eight zones writing a placement offset into PickerMenu — adds position-coupling machinery to a shared design-system component for zones nobody needs.
...
- **B-4:** [confirmed] The card anchors to the **live link element**, not a rect captured at hover time — the hover signature carries the element so PickerMenu's existing measure machinery tracks editor scroll for real (today's frozen rect leaves the card pinned to dead coordinates the moment anything scrolls, which a linger makes routine). The link scrolling out of the viewport (its DOM node destroyed) is a close condition. Placement gains the mirror viewport clamp on the pinned-edge branch, and the flip decision re-decides when the card retargets to a different link — the linger keeps the card open across retargets, so a decision frozen per-open no longer converges.
- **B-5:** [confirmed] The close lifecycle suspends during an active resize drag — the pointer leaving the card mid-drag must not dismiss it (`swallowActiveEscape` already keeps Escape aborting the drag rather than closing the card). After the drop, the grace re-arms only once the pointer has been observed back inside and leaves again, so releasing a drag just outside the card doesn't close it.
