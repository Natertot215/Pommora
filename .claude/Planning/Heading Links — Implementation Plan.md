...
- Frozen interfaces: `LinkStatus` stays `'resolved' | 'phantom' | 'ambiguous'`; `PageIndex.resolve` keeps answering for pages; `mentions(path, title)` keeps its shape and `queryMentions(title)` its contract; `registerPageEditor` stays a singleton; `pageEmbedPattern`'s embed stays a token to the tile field.
  - The storage half of that freeze was overridden by the ratified plan in `.claude/Planning/Relationship Matrix — Implementation Plan.md`: `mentions` merged into `matrix_nodes`. `queryMentions(title)` kept its contract and every caller is untouched, and decision B-1's operative guarantee holds — a heading consult runs only against a ready index and never takes the corpus fallback.
