...
│   ├── // Glass                        | • The material — glass-base, -window, -surface, -control, -pane
...
- **The convergent values.** `--labels-gap` (4px in three). *Recommendation:* absorb both into `Tokens/`; `--subline-h` first, because `previewPane.css:21` already reads it with a hardcoded fallback inside the design system.
...
| `--subfield-h` · `--toolbar-h` | 24px · 38px | The shell's bands, beside the `--button-*-height` trio the bridge already publishes. | call |
