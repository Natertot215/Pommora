...
**`Detail/Views/Table/` is the renderer's cell-and-value library wearing a view type's name.** Thirty-eight imports from twenty-four files reach into it, nine from outside `Detail/` entirely. What they import is not the table: `resolveContext` (8 importers), `solidColor` (6; token math), `Cell` (3), `columnStyles` (3), `columnLabel` (3), `checkboxLook`, `tableDnd` (the nav list's row drag), and two stylesheets `main.tsx:18-19` loads globally so every surface depends on the table's CSS.

**Property editing has two homes and the upstream one is filed under a view.** `Detail/Views/PropertyEditing/` holds the app's shared property-value vocabulary — `formatValue.ts` (12 consumers, 5 folders), `PropertyPicker.tsx`, `OptionChip.tsx`, `PropertyEditor.tsx`, `DatetimeValuePicker.tsx` — and `Properties/` imports `Detail/` seventeen times across seven files to reach it. Eleven of `PropertyEditing/`'s fourteen consumers are view surfaces, so it is not the panes' row engine the open question assumed. `usePropertyRows.ts` is the exception: its two consumers, `Properties/PagePropertiesPane.tsx:26` and `PagePreview/PreviewInspector.tsx:28`, share roughly 470 identical lines of row chrome.
...
- **R4 — One home for schema, one home for values.** `Properties/` is the schema-editing surface — the panes that assign, rename, and retype a property, and the per-type option editors. `Views/Values/` is value rendering — the formatters, the cell, the pickers a value opens, the checkbox glyph, column styling. *Test:* nothing under `Properties/` imports from `Views/Values/` except the two panes that display values, and nothing under `Views/` imports from `Properties/`.
...
Sixteen top-level folders become fourteen, each answering "what is this" in one word. New folders are marked; files are shown only where they carry a finding.
...
Folder counts after the moves, over the Cartographer's 453 rows: `DesignSystem` 139 (WindowChassis and two Interactions arrive, Detail empties; the Showcase's 27 files sit outside the 453 and move as one folder row), `Views` 60, `Interface` 46, `Components` 28, `Surface` 23, `Properties` 20, `Navigation` 15, `Windows` 14, `Core` 10, `Settings` 8, `Connections` 6, `Embeds` 5, root 5, `testing` 5, `MarkdownPM` 69 unchanged.

**What moves and what holds.** 228 files move, 225 stay. Of the moves, 87 are renames alone (casing, the `glass-*` six, lowercase subfolders, stylesheets to lowerCamel) — mechanical and `git mv`-able in one commit; 47 ride the two rulings already taken; 58 move because their folder did not name them (R3); 19 move to their consumers (R2); 15 consolidate value rendering (R4). `MarkdownPM/` moves nothing but subfolder casing. `Properties/` holds as minted. `Components/Detail/` stays by ruling — its correct name is `Components/ViewSettings/`, and the rename waits for the folder to be opened for real work. `DesignSystem/Pickers/` stays: `PickerMenu` is the rectangle ~30 menus mount, and moving it would strand them on a feature-folder import.

**What the tree does not do.** It does not split the store, unwrap IPC, or touch a line of behavior — those are edits, carried below as decisions. A session can execute any move in isolation; the typecheck catches every miss. The rules, not the tree, survive growth: a tree drawn for 453 files will be wrong in detail at 1,400, but the eight rules still file every new module. Three seats are built for that growth — `Core/` for the models every future feature reads, `Windows/` for the fourth and fifth floating windows, `Views/Values/` for every renderer a new view type needs — and a new feature folder appears only when a domain has three or more surfaces of its own, the test `Connections/` is the first to pass.

#### Open Decisions — Structure

The nine that gate the tree's non-mechanical moves. Each is a fork the rules do not resolve; the recommendation is a design call, the ruling is Nathan's.
...
- **Where value rendering lives.** The open question asked whether `PropertyEditing/` folds into `Properties/`; the evidence contradicts the premise — eleven of its fourteen consumers are view surfaces, and folding it would mint eleven cross-folder imports. *Recommendation:* one folder, `Views/Values/`, holding `PropertyEditing/` plus `checkboxLook`, `Cell`, `columnStyles`, `columnLabel`; `Properties/` takes a stated scope — schema and options, never value rendering. `usePropertyRows` goes with it.
...
19. **No shared `design-system/tables`** — what leaks out of `Table/` is four homes, named in the value-rendering decision.
