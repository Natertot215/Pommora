...
**What this forecloses, accepted deliberately.** Less than a first reading suggests, and the difference is worth stating precisely because review round one got it wrong. The claim was that a synchronous mutator excludes any write needing an async decision inside its lock span, and that five sidecar sites already have that shape. Reading all seven, **none does**: every one is an awaited read, a synchronous decision, an awaited write. Nothing in the codebase currently needs the shape the primitive cannot serve.

What actually keeps the sidecar family on `withSidecarLock` is typing, not asynchrony. `readSidecar` zod-validates and hands back a shaped object; `rmwJsonStrict` hands back a bare record and validates nothing, so migrating a sidecar write onto the primitive would trade schema validation for lock automation. `saveView` additionally needs the id it mints returned alongside the write, which a `(cur) => next` mutator cannot express without a closure.

So the real cost is **uneven protection rather than lost capability**. The primitive becomes impossible to misuse; `writeSidecar` stays callable with no lock at all, which is exactly how the two unlocked writers in Sequenced After came to exist, and the sidecar is the file with the most writers. Any future feature touching a container sidecar — a trash browser restoring into one, a type-change surface, bulk moves, duplicating a Collection — inherits "remember the lock" rather than being handed it. The second path also earns its keep: the first write that genuinely needs to hold a lock across an async decision (checking every page before dropping a select option, say) can only be built there. Closing the sidecar half is the branded token, declined here as an addition and worth revisiting if a third bare writer appears.
...
### Rulings
- **08-08** (Nathan) — Close the enforcement gap now rather than deferring it. The arc was ratified as removal-only, and this adds roughly twenty lines; his call overrides that scope because an unrefused re-entrant take wedges a file for the process's life rather than failing it.
...
*(none open — the enforcement finding was ruled on and closed; see Rulings and Closeout.)*
...
**Both attack findings were closed rather than carried.** On Nathan's ruling the enforcement gap was fixed in the same session: `serializeOnFile` now holds the keys of the call in flight in an `AsyncLocalStorage` set and rejects a second take of one, before the chain is touched, so a refusal cannot wedge the file it names. The three shapes the attack used to hang the tree each reject now, and the suite passing unchanged is the evidence that no production path was legitimately nesting.
