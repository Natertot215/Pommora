...
- **A-2:** [confirmed] The fragment is the literal heading text after `#`, Obsidian's form; no GitHub slug is generated. `[Alias](Page#Heading)` and `[Alias](#Heading)` carry the same fragment through the markdown-link grammar in `links.ts`, a bare `#` target naming the current page; the split reads the raw target before it's decoded, so an encoded `%23` inside a title stays a title.
...
- **A-12:** [assumed] An empty fragment (`[[Page#]]`, `[[#]]`, the state the pane opens in) names the page itself: it resolves, renders, and indexes as a page link, and a bare `#` key is never written to `mentions`.
...
- **B-1:** [confirmed] The index answers, per page, which of its headings carry inbound links; a rename of a listed heading rewrites its inbound links across the files the index names, and an unlisted heading edits at no cost. The `mentions` table gains the heading as part of its key, and `INDEX_GENERATION` steps so an existing database rebuilds; a page-rename query over the new key dedupes paths. A heading consult runs only against a ready index and no-ops otherwise; it never takes the page cascade's full-corpus fallback. The governed sweep already re-indexes every file it writes, so an immediate undo's consult finds the new key.
...
- **C-1:** [confirmed] **Heading Link Style**, a picker: Page & Heading · Heading Only. **Hide Heading Symbol (§)**, a toggle, off by default. Page & Heading renders `Page ┃ §Heading` with `UIX/Elements/segment.css`'s hairline as the divider, the same divider menu rows and the tab bar draw; Heading Only renders `§Heading`; the toggle drops the `§` from link-syntax rendering only, never from a bare token (I-1), whose `§` is its only marker.
...
- **C-4:** [confirmed] `§` typed inside `[[ ]]` rewrites to `#` on the keystroke, so the file only ever holds `#`. It lives with the connection input in `Links/`, since every transform in `Input/edits.ts` stands down inside a wikilink and this one fires only there.
- **C-5:** [confirmed] The two settings sit in Files & Links › Connections beside the existing connection rows.
- **C-6:** [open] The pixel treatment of `Page ┃ §Heading` (spacing, the divider's height against the text, the `§` weight) is presented in Figma before the rendering phase, not settled in prose.
...
- **F-1:** [confirmed] The heading grip menu gains **Copy Link**, writing `[[Page#Heading]]` with the page's current title and no alias, the way `title:copylink` writes `pageLinkText`.
...
- **I-6:** [assumed] The setting sits with the two display settings under Files & Links › Connections.
...
- **G-3:** [confirmed] [[ConfigurationPM]]'s Files & Links › Connections table gains Heading Link Style, Hide Heading Symbol (§), and In-Page Heading Resolution.
...
- The fourth resolution status, live for same-page and cross-page links, its rendering across the four selector sites, and the index's heading table with its push; the two settings and the `Page ┃ §Heading` / `§Heading` display; same-page and alias display rules; `§` → `#` on the keystroke.
