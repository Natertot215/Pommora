...
| 8 | The value's area adds and a label replaces at that file's folder, on all four surfaces, off one arm; the hover-× removes on three of them | `valueClick.ts` · `filePick.ts` · `usePropertyRows.ts` — and see the Preview Inspector note below |
...
**The Preview Inspector has no hover-× on any value**, file or otherwise. It calls `Cell` without a `remove` handler, and always has — multi-select options and Context Spaces are equally ×-less there, and `git log -S` finds no commit where it ever passed one. Nothing this feature did caused it and nothing this feature did could reach it: adding one changes every chip type in that pane, which is a call about that surface rather than about this property. A file value is still removable there through the row's own menu, which clears the whole value. **Recorded as a gap, not fixed.**
