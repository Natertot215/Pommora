...
#### Gate 6 — the attack pass, and two false verdicts

Five findings, each re-opened against the code before folding. Two of them falsify verdicts written earlier in this Log, which is the point of dispatching an attacker after a verifier.

- **Fixed · one unspellable entry nulled every attachment on the page, and the next click overwrote them.** A dangling `- ` under an attachment key is YAML `null`, and `decodeValue`'s file case returned `NULL` for the whole array on any entry `fileEntry` could not spell. The cell then rendered blank, so the value's own area was a bare Add target, and one click committed a one-entry list over references whose files were still on disk. **This is the exact damage A-1a was written to prevent**, reached by a spelling the coercion did not cover — and the function's own comment said so, one screen above the code that did it. An entry nothing can name is now dropped rather than fatal, and a list with nothing left in it is null, which is what `strict` already answered. The restore gate inherits it.
- **Fixed · the Directory could reach a write outside the asset root, through an in-nexus symlink.** `resolveUnderRoot` bounds the **nexus**, and its canonical answer was computed and discarded. A symlinked folder inside the asset root pointing at the content tree passed every gate: the bytes landed among the user's pages, and `readdir({recursive:true})` does not descend a link, so `buildAssetMap` never saw them — the chip resolved for one session through the write-side map patch and was permanently unresolved after the next launch. A link *out of the nexus* was correctly refused, which is why two passes read the comment and moved on. The canonical path is now spent rather than thrown away, re-read against the asset root. The root itself is exempt: `underAssetRoot` reads strictly below its root, and a linked root is the asset-directory setting's business.
- **Fixed · `embeddableTitle` admitted a line break.** The pattern that reads a reference back is single-line, so a name carrying one minted a reference that could be written and never parsed — the permanent blank the bracket and pipe refusals exist to prevent, reached by a character rather than a bracket. The refusal list was one character short.
- **Fixed · `SORTABLE_PANE`'s header claimed file was deliberately absent**, eleven lines above where file is present. Task 18 opened that gate and left the sentence that justified it closing. A comment stating a state the code contradicts is an invitation to re-ship the defect the Lessons already name.
- **Recorded, not fixed · a second Add started while the first adoption is still copying loses one reference.** Every surface reads the value at click time and the adoption after the dialog is a plain await, so two picks straddling a large copy both tail the pre-commit list. Closing it means threading a getter through four call sites — more machinery than a narrow window whose damage is fully recoverable, since the lost file is already under the asset root and re-picking it takes the reference-in-place branch.

**Two written verdicts were false** and are corrected above rather than left standing: Gate 4/5's "the Directory cannot reach a write outside the asset root, by any entry," and `mutate.ts`'s claim that realpath settles the symlink case. It settled it against the wrong root. Each fix is ~2 lines at a seam that already existed, each has a test, and each test was negative-controlled — the guard removed sends it red, restored sends it green.
