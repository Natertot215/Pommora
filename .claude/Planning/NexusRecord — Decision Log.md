...
- The identity arc's D-15 (duplicate ids — folds in via the baseline, see A-5) and D-17 (one record serving structural revert and trash restore).
...
- **A-5:** [confirmed — Nathan] **A duplicated id re-mints — content and container alike — and the baseline is what makes that safe.** One Finder folder-duplicate copies the sidecar id along with every page id inside, and container ids key the fold, view, thumbnail and asset stores, so the copy and the original share all of them until re-minted. A sidecar id is exactly as re-mintable as a frontmatter one. The prior session's baseline names the path that legitimately held each id; what sits at that path is the original, the other re-mints.
  ...
  - **The re-mint copies rows, never moves them.** The rows keyed by the old id belong to the original, which keeps that id — moving them would hand the original's board and folds to the copy and leave the original blank. The copy's fresh id receives **duplicates** of the device-local rows — block layout, active view, folds, preview origins — so the tile files that rode the Finder copy stay referenced and both entities own a working board. The original is untouched by construction. Order arrays need nothing: unknown ids drop and the entity re-enters by title.
  - **The re-mint's tree never reaches a render.** The open pass's walked tree has exactly one consumer — the baseline projection, whose fields are scalars — so the re-mint folds its results into that projection and mutates nothing (the walked page nodes are the parse cache's own objects). The renderer walks fresh after the open path completes and sees the re-minted disk directly, `contextValues` attached by its own walk's existing pass. The frontmatter write takes the file lock like every other frontmatter writer.
  - Silent, per C-7.
...
- **B-1:** [confirmed — Nathan] Provenance is a **paired JSON beside the trashed artifact**. Nathan's rule constrains *what* may be recorded — **ids, never name-based locations**, since a rename rots a name — not where the record lives; an id on the artifact itself would also be legal. The pair wins on cohesion: on-artifact storage needs three shapes (a frontmatter key for pages, a sidecar key for containers, and a new file anyway for Contexts, which have no sidecar), where the pair is one mechanism for every kind, holds the long payloads that have no business in frontmatter, writes nothing into the user's file on the way out, and leaves no residue to strip on the way back.
  ...
  The write is **best-effort, gathered at four named points** in the delete arm's real order: the scoped title→id read and the registry entry before the erase (the resolution window G-3 needs closes there), the membership during the sweep, the pair name after the move returns. A pair write that fails does **not** fail the delete — the artifact is already irreversibly moved, and a fault reply would leave the renderer showing an entity that is gone from disk. A missing pair degrades that one entity to hand-restore, which is today's behaviour for everything.

  **All-or-nothing per pair:** if a gather point a kind requires fails — a Context whose registry entry could not be read, a Space whose own sidecar could not be read — **no pair is written at all**. A missing pair is the accepted degradation; a present pair that is silently incomplete is worse, because restore trusts it. The parent is not a required payload: per B-4, an unresolvable parent records `unaddressable` and the pair still writes. The partial marker (G-2) covers everything that leaves a pair thinner than the truth and says so.
...
- **C-2:** [assumed] The **open path owns one walk explicitly**: every genuine open — the picker, a folder drop, Open Recent, launch-restore — walks once after the database opens and before the watcher starts, hands that one tree to the re-mint pass, **then** the baseline writer; the renderer's own state read walks after the open path completes, so it sees post-re-mint disk through the warm parse cache. In that order, or the baseline records the pre-mint state and the next open reports the re-minted entity as created. Not "whichever walk happens first" — a sync daemon materialising another device's changes would otherwise make the post-change walk the baseline. A mid-session re-point — the nexus rename, re-adopting the already-open nexus — is not an open and never latches: it would diff the live session against the launch baseline and overwrite the closed-window record.
