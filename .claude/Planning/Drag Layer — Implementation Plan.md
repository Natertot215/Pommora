...
**Verification chain (08-10), complete.** Neutral verifier: all eleven Requirements HOLD; its two overstatement findings were fixed (`615fd6c2` — which flushed a real defect, the Slider's never-set scrub flag) and the claim corrected. Attack (`build-breaking-agent`, interleaved brief): 4 findings — 0 High, 2 Medium, 2 Low — against 15 killed candidates, every one at a seam between Phase-6 mechanisms; all four fixed in `3b51bcbe` (the spring-open remeasure staying dirty through the reveal, the column cursor-follow anchored to the flowed centre, the grouping Escape swallow, the bands-push re-aim, plus the paneDnd fail-closed resolve that retired the attack's one testable unknown). Its second unknown — screen readers staying silent on repeated identical announce text, and no cancel announcement existing — routed to the standing a11y-phrasing debt line. The skeleton itself survived the full attack: throw-guards, pointer-id filtering, blur/zero-buttons aborts, the scrollTarget predicate, teardown ordering, and both Escape modes all confirmed by execution.

**Delivery Claim.** The ratified scope shipped whole: all eleven Requirements trace to landed tasks (R1/R2 → `a14eea47` + the leak tests; R3 → `7100bd2c`/`0af0b7db`/`52ca8c94`/`c1403087`; R4 → Phase 1 + `d9f5908f`; R5 → `c1403087` + the Gate-3 fold in `600ea9de`; R6 → `7fa0ee51`/`0fc563fb`/`7100bd2c`; R7 → `600ea9de`/`4587ed55`; R8/R9/R10 → `fe365604`; R11 → `3e5509bc` + the falsifying commits). The acceptance criterion holds in its testable half — the neutral verification (08-10) adjudicated all eleven Requirements as holding, corrected the test enumeration this claim first overstated, and its named coverage gaps were closed in `615fd6c2` (which also caught a real defect: the Slider's scrub flag was declared but never set, so its cancel guard and unmount reassert were dead until the new tests forced them live). Precisely: scroll-re-aim and release-without-move are pinned on the grouping pane, the GFM table, the table rows, the option hook, and the sidebar; Escape aborts on the sidebar, tables, bands, and the option hook; the mode-switch leak by listener-identity tests; the Slider by commit/cancel/unmount tests. The column drag and grip carry the shared skeleton's contract tests plus the walkthrough rather than component harnesses (a Detail-TableView mount drags the store in — ruled not worth the harness), and the six lint warnings retired at close were this plan's own mid-phase residue, not inherited. The running-app half is the walkthrough handed to Nathan. No new dependency. No mechanism duplicated: one skeleton, one snapshot helper, one ghost, one line chrome, one scan, one offset, one guard core. Nothing left with nothing to vary: the deleted lifecycles, listeners, and inline chrome have no residue the census can see. No work added to a high-frequency path — the changes *removed* per-move geometry work (`group.tsx` caches, the sidebar's sibling filter is snapshot state, the grouping callers memoized). Gates green at every commit; the full suite is 2,275 tests, lint zero including six pre-existing warnings retired.
