...
5. **The section hides and shows.** Hidden by default. The divider is the shared heading seam and is itself the fold click-target. Show/Hide is one Subfield text control reading **Show Footnotes** / **Hide Footnotes**; the divider and that control write one per-page per-machine override, which falls back to a nexus-wide **Show Footnotes By Default** and clears when it lands on that default. The control appears only when at least one citation line exists, and rides into the Page Preview. Disclosure uses the editor's fold-reveal motion; the divider blur-fades. (C-1, C-2, C-3, C-3b, C-3c, C-6, F-3, F-4, G-2)
...
10. **Settings.** Show Footnotes By Default and Jump To Citation On Creation group under a Footnotes heading in the Pages & Editor leaf. (F-4, F-5)
...
**Steps:**
- [x] Add the item to the union, the id array, and the `page` default order.
- [x] Add the label to the shared two-state label module as one line beside the footer's — the module's whole point is that a control's wording is stated once. Note the List / Gallery item inlines its own ternary and is the counter-example, not the pattern.
- [x] Build the control as a text button and add its class to the List / Gallery item's existing CSS selector list — that rule is already the Subfield's text-control treatment, down to the hover state and the drag-region opt-out. No new block.
- [x] Take Enter and Space from the design system's activation primitive rather than a hand-rolled key handler.
- [x] Return null when the count is zero — the item registry's union already admits a null-returning item, as the add-menu item is on the wrong selection kind.
- [x] Resolve the shown state as override, then default; write through the override, clearing the row when the value matches the default. **Write the store slice optimistically and let the IPC be fire-and-forget** — the sixteen-times-copied persistence pattern — so the section moves within the frame instead of behind a round-trip.
- [x] Read the label's state from the **fold**, not the override, so a section opened by a marker jump or an outline reveal reports itself open.
- [x] Update both feature documents.
- [x] Run the gate — expect green.
- [x] Commit: `feat(subfield): a Show / Hide Footnotes control`
...
- [x] **Interaction pass handed to Nathan.** On a page with footnotes and at least one heading above them:
  ...
  7. Change Show Footnotes By Default in Settings — a page never toggled follows it; a page you have toggled keeps its own answer until you toggle it back to the default.
...
**Neutral verification.** The claim went to a verifier holding the decision log, the requirements and the range, and came back **NO** on four points. Three were the claim's own inaccuracy and one was a real defect.

- **The marker's ink is `--label-control`, not the accent.** Nathan retuned the KNOB by hand on 08-20 and it rode into `5aff1171` through the staging hook. The code is his and stays; the claim and `Features/MarkdownPM.md` were the false half and now read what the declaration holds.
- **The setting ships as "Show Footnotes By Default", not "Default Visibility".** F-4 and R10 named it the second way and Task 10 shipped it the first. The shipped label is the clearer one and is what `ConfigurationPM.md` has always documented, so the claim and the spec were corrected rather than the UI renamed.
- **"Cascades key to the range, never the gesture" was false.** `onForwardDelete` returned early on any non-empty selection, so sweeping exactly one citation row and pressing forward-Delete removed it with no marker cascade and no renumber, where Backspace over the identical range cascaded both. Two gestures, one range, two answers — the thing B-11 exists to prevent. Forward-Delete now asks the same `citationDeleteIntent`, and a caret against a marker's leading edge takes the whole marker the way Backspace against its trailing edge does. Four tests pin the two keys agreeing, including the negative control that a wide sweep still cascades neither way. **A replacement is left alone deliberately** — typing over a swept citation, or a cut, removes what was swept and cascades nothing, because silently deleting markers elsewhere in the document on a keystroke that reads as "replace this text" is the surprise the range rule exists to avoid. Stated in the claim rather than left implied.
- **The claim overstated what is unverified.** Phase 2's and Phase 3's interaction passes were run; the claim said Phases 3 through 6 were not.

**Cohesion Criteria, against the full range** — the two a per-phase pass structurally cannot answer.

- **C1 — one definition per thing, across phases.** The whole-range pass found five twins that formed between phases and none within one. The exclusion assembly was spelled in `docLineScan` and again in the counter's `citationBoundary`, whose own comment claimed the two could not disagree — `constructExclusions` is now the one assembly. "Is this the last marker bound to its citation" was spelled two ways on the two sides of the same interaction — what the menu row promises and what the click does — and is now `isLastReference`. `personalization.citationsShown ?? false` had four spellings across three phases and is now `citationsDefault` / `citationsVisible`. `coerceScale(personalization.embedScale, …)` was byte-identical in two files and is now `useEmbedScale`. A `HEADING_LINE` alias of `HEADING_FOLD_LINE` is gone. Verified after: one differ, one `citationScan`, one caller of `normalizeCitations`, one dispatch every gesture ends on in `commitCitation`, and one writer of the visibility row in `setCitationsShown`.
- **C5 — no residue, across phases.** Nothing survives from a replaced approach: `collapsedByDefault` returns zero against a control of 65 `docScan` hits, and `insertCitationChanges`, `withCitationsUnfolded`, `citationHostOf` and the briefly-added `jump` parameter all return nothing. `citationPointer`'s threaded `reveal` is gone rather than kept beside its facet.
- **C10 — Dead Vocabulary.** `collapsedByDefault` 0 (expected 0) · `footnoteDefinition` 1 (expected 1, the parser cross-check) · control `docScan` 65, non-zero, so the sweep ran.

**Total net line delta:** +1,277 code lines (comments, blanks and tests excluded) across 48 files, from +1,433 and −156. The raw range reads +1,439 across 75 files, but an unrelated embed-Scale arc ran through the same commits and that figure carries it; the 48-file figure is the feature's own.

**Adjudicated keeps, not to be re-proposed.**

- The `citationRef` skip in `activeTokenIndices` is inert — `tokenIntents` returns on that kind before consulting the active set, and the only other readers are gated on links. It stays as the invariant stated at the layer that owns token activity: a marker never reveals its syntax, and deleting the line would make the tokens layer silently depend on an early return in another module.
- `citeKey`'s encoder and parser stay in `Tables/widget.tsx` and `Tables/TableView.tsx` — the string genuinely crosses the widget boundary, so co-locating them is a move rather than a dedupe.
- `citationSeat` (where a row is written) and `citationSeatAt` (whether a marker may be written) keep their near-identical names; they answer different questions and renaming is not a simplification.

**Attack pass.** Dispatched after the verification, against the whole range. It returned one High, one Medium, one Low, one Unknown and ten killed candidates. Both real findings live where the breakage sweep structurally could not reach.

- **HIGH — the first footnote on a page could be written above its own marker.** `citationSeat` answered "where does the body end" when the question was "where does the body end at or after the marker". On any document ending in a newline the caret sits on an empty last line — where it lands after any Enter, and after a click in the blank area under the text — so the citation was seated at the last line holding content, *above* the caret, and the marker landed below the section it had just created. With one trailing newline the marker was swallowed as the citation's own lazy continuation; with two, `[^1]: ` stood as literal prose and every citation already on the page would have literalized with it. That is the catastrophic mode the feature exists to prevent, reachable on the first footnote of an ordinary page, by all three creation doors. The seat is now `Math.max(bodyEnd, at)`. Six tests pin it, all six red against the old seat.
- **MEDIUM — the tail guard's relocate dropped the range it moved text out of.** `rewrite` carried one range, and the relocate arm spent it on the destination — so replacing a selection with text that would break the run left the selection standing and wrote the text elsewhere. Pasting two paragraphs over a citation's own words kept the words and put the paste in the body. `GuardVerdict.rewrite` now carries a list of edits, because a repair that MOVES text is two disjoint edits; a sweep that began at or above the seat goes through as the plain replacement it is, since the guard stops text being stranded after the section rather than removed. Two more tests, both red before.
- **LOW, accepted — a gesture eats trailing blank lines below the section.** The rebuild ends the document at its last citation row. Recorded with the between-row blanks it already drops: a blank below the section belongs to no citation, and the reorder cannot carry it anywhere truthful.
- **UNKNOWN, closed.** The attacker could produce `[[^2]^1]` by seating a caret strictly inside a marker and inserting there, and could not determine whether that seat is reachable. It is not: `calloutAtomic` registers every resolved marker as an atomic range in a whole-document walk, so no cursor motion or deletion lands inside one. The repro used a programmatic dispatch, which that file's own header already states atomicity does not block — the reason the transaction-layer guard exists beside it.

**Killed by the attacker, recorded so they are not re-attacked:** the rename arithmetic across huge numeric labels, `0`, leading zeros, case-only pairs, ß/SS, regex metacharacters in labels, an orphan squatting on a wanted number, a duplicate-loser travelling with its winner, and word labels holding position · `ChangeSet.compose` and the differ's trim, which cannot overlap by construction · pure deletions that break the tail, which lazy continuation absorbs · a native menu held open while the document moves · per-document cache staleness, the cache being a WeakMap on an immutable `Text` · multi-range selections, which are not enabled · a control offered where pressing it does nothing, one predicate feeding both the row and the writer · an undo landing between two halves, every gesture being one dispatch · CRLF, normalized at state creation · counter-versus-editor boundary divergence, now one assembly.
...
- **Task 13 — `computeStats` is memoized on the body string.** Two footer items now mount on a page and both need the same figures on the same keystroke; the task's own Interfaces block forbids deriving the boundary twice on that path, and two `computeStats` calls would do exactly that. `pageStats(body)` holds one entry, which is all the footer needs — both items read the same string within one render.
...
- **Task 11 — the visibility is applied from outside the editor rather than by a field inside it.** A CM6 `StateField` cannot dispatch, and a `ViewPlugin` only wakes on a transaction, so neither shape can follow a prop change that arrives with no edit — which is the second half of what the task asks for. `applyCitationsVisibility(view, shown)` mirrors `applySavedFolds` in the same file: the view-creation effect calls it once at mount, and a `useEffect` keyed on the resolved value calls it after. The mount annotation, the single-writer property and the ordering against `applySavedFolds` are all unchanged.
...
- **`revealBar.ts` names `.footnotes-toggle` directly.** A design-system leaf reaching for a feature class — the lead zone's origin should be a selector the host names, so a second lead control can exist. The band's CSS in `reveal-bar.css` and `previewPane.css` names the class too, so the fix is the whole seam or none of it; with exactly one lead control today it buys nothing yet.
- **Two exported `CitationSubject` types** — `shared/citationMenu.ts`'s `'marker' | 'citation'` and `editor/citationActions.ts`'s union carrying the marker's span. Same name, same feature, different shapes; one of them wants a different name.
...
**Where it stands.** All 23 tasks landed across six phases, over the range `71fe5be2^..HEAD` — 68 commits, six of which belong to the unrelated embed-Scale arc that ran through the same window. Every gate ran its simplification alone and first, then the comment pass, then a correctness review; the closeout added a whole-range simplification, a neutral verification of the Delivery Claim and an attack pass. Four review rounds returned real defects — a renumber fusing two footnotes onto one label, forward-Delete disagreeing with Backspace over one range, a citation seated above its own marker on the first footnote of any document ending in a newline, and the two the closing review found below — and each was confirmed red before its fix and pinned after. Gates green: typecheck across both projects, Biome clean, 3,335 Vitest tests.

**Net:** +1,277 code lines across 48 files, comments, blanks and tests excluded.

**What is left, and it is Nathan's alone.** The Verification Checklist above. Both decisions the Log flagged — the relocate rule with the pasted footnote's shaping, and the setting's name — were accepted on 08-21 and are recorded in Rulings; nothing else in this arc waits on anything.

### Closing Review — duplication, bloat and errors across the whole arc

A last pass over `e4c5c04a..4c08a4f3`, holding the Log's adjudicated keeps and Sequenced After items so neither was re-reported. It found no duplication and no bloat: one `citationScan`, one `foldLabel`, one `commitCitation`, one writer of the visibility row, and every citation-named export with a live caller. It found two errors, both in the same blind spot — a question asked of the wrong offset — and both outside what the breakage sweep could reach, whose corpus always carries a blank line above the section and never drives a selection that spans the boundary.

- **HIGH — relocated text landed at the top of the document.** The tail guard seats what it rescues at `lineStarts[anchorLine]`, which is the end of the body only while that anchor line is the blank the section floats on. A file authored in Obsidian or on GitHub puts its citations straight under the last line of prose, and there the seat is that paragraph's own start — so a paste or a typed heading below the section was lifted out of the foot of the page and written in above the first paragraph. The seat now reads the anchor: a blank one keeps its gap and takes the text above it, a prose one takes it after its own end. Three tests, two red before, one the negative control that the blank-anchored document is unchanged.
