...
- **Gate 2 live check — the add→edit transition.** The panel now drives one `PropertyPicker` for both the Add chooser and the value popup; picking a property in Add closes it one frame and reopens the value picker at the new row next `requestAnimationFrame` (triggerRef `addRef → row`, origin `auto → right`). The anchor is correct; confirm it re-blooms cleanly at the row rather than visibly sliding from the Add button.
