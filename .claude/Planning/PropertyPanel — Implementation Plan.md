...
- **Gate 1 finding, needs Nathan — the multi-select/context add-drill regressed and the clean fix is a shared-contract change.** Adding a multi_select or context property through a card's add-chooser and drilling into it: the old `CardAddPicker` kept the drilled pane open and resolved `current` live via a `currentOf(picked)` callback, so more options could be toggled. The fold bakes `current` into `PickEntry.target` (a snapshot), and Cards reveals the column on first commit — which filters it out of the chooser array — so the drilled entry cannot be re-resolved live. Today's symptom is a **snap-back to the chooser root after the first toggle** (recoverable and non-corrupting: the value commits, the column reveals, further values go through the now-visible cell). The snap-back-only fix (stable effect key) would trade the eject for **silent value corruption** (a second toggle computes from stale `selected`), so it was NOT applied. The correct fix restores live drilled-target resolution — a design change to the shared `PickEntry`/chooser contract that Task 4's panel also uses. Left for Nathan's sign-off at the Gate 1 stop rather than redesigned unilaterally. This affects Task 4's chooser too.
