...
// the offset of the first word of label starting with the lowercased query; null when none; 0 for ''
export function matchAt(label: string, query: string): number | null
// keeps a row where matchAt is not null; drops a section left with no rows
...
**Now** — `—` (new file). Readers reused: `inCodeAt` and `lineIndexAt` (`Engine/docScan.ts:69, 81`), `docScan` (`docCache.ts:22`).
...
// null inside code, when the caret is not at its line's end, or when the line is not exactly /\S*
...
- [ ] Red first on the eleven cases above through `scanOf(text)`; expect module-not-found; then green.
- [ ] Full gate green, exit codes read directly.
...
// the query's length from matchAt(label, query) wrapped in .mdpm-autocomplete-match; the last live state held through the close as AutocompletePane does
```
...
<!-- :72 "…and where the grip menu lives." · :74 "The handle is also where the grip menu lives." -->
<!-- after the table: -->
...
- [ ] `rg -F "grip menu lives" .claude/Features/MarkdownPM.md` → 2; `rg -F "Block Menu:" .claude/Features/MarkdownPM.md` → 1. Control: `rg -F "Block Structure" .claude/Features/MarkdownPM.md` → 1.
...
- [ ] `/` mid-sentence, `> /`, `- /`, an indented `/`, and `/` inside a fence do not open it.
