...
- **B-2:** [confirmed] **Neither glyph opens a YAML flow collection, and that is why they were chosen.** Verified by execution: `(Projects):` and `<Status>:` parse clean unquoted, survive an ordinary write unchanged, and strip bijectively even when the name carries the closing glyph. The bracket and brace forms cannot do this — `[Projects]:` parses into `[ Projects ]` and `{Status}:` into `{ Status }`, neither matches, and the next write rewrites the line into explicit-key form permanently; forcing plain style does not help, the emitter re-quotes. So Pommora writes the key **plain**, what the app emits is byte-identical to what a person would type, and a hand-edit registers. There is no quoting invariant because there is nothing to quote.
- **B-2a:** [confirmed] **The strip is positional, so a title carrying a glyph round-trips.** `[Q3 [Draft]]` reads back as `Q3 [Draft]`. That is what retires the bracket ban rather than amending it, and it is why the sigil needs no character ban of its own.
...
- **D-5:** [confirmed] **The two *filesystem* validators become one basename core; the property-name validator stays its own and reads its bans from the syntax module.** Routing property names through the basename core would refuse a leading `_`, which `D-3` permits, and would newly refuse `README.md` as a property name. Property names go through a validator with **zero** character bans today; the file-basename validator that does carry bans never sees a property name; and the Context-title validator re-implements a subset of the basename rules while its own comment claims it shares them.
...
- **I-3:** [confirmed] **The tagged value shapes go, everywhere.** Both codec branches, the second hand-rolled decoder, and the `status` branch of the page-value writers — whose remaining `type` argument collapses to a multi-select-vs-scalar test rather than disappearing.
...
- Property titles unique nexus-wide, case-folded, normalized once at write — enforced on create and rename. Registration is a registry title match, never the sigil alone (`A-6a`). The `$` reservation; the two filesystem validators collapsed to one core (`D-5`).
...
- **Doubled delimiters** (`[[…]]` / `{{…}}`) — both open a YAML flow collection, so an unquoted hand-typed key silently becomes garbage the next write makes permanent; and `[[…]]` is needed elsewhere (`B-3`).
- **Bracket and brace singles** (`[…]` / `{…}`) — the shapes originally wanted. Rejected on the same mechanism: they parse only when quoted, so the file gains `"…"` on every key and a hand-edit does not register. The glyph pair is a one-line change in the syntax module if that trade is ever worth making.
