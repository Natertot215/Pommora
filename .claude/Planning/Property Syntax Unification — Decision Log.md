...
- **B-2:** [confirmed] **Both glyphs open a YAML flow collection, so a wrapped key is always written quoted — and the serializer does that itself.** Verified by execution: `"[Projects]":` and `"{Status}":` round-trip exactly, while the same keys hand-typed unquoted parse into `[ Projects ]` and `{ Status }` and never match — and one ordinary write then rewrites the line into explicit-key form, permanently. Forcing plain style does not help; the emitter re-quotes. No caller manages quotes, so the invariant costs nothing in code, and JSON quotes every key already, leaving agenda and sidecar roots unaffected. What it does cost is that a key typed by hand into an outside editor must be quoted to register — the trade Contexts have made since they shipped.
...
- **Doubled delimiters** (`[[…]]` / `{{…}}`) — no better than the single forms on quoting, and `[[…]]` is needed elsewhere (`B-3`).
- **Non-indicator glyphs** (`(…)` / `<…>`) — they write plain and unquoted, so a hand-typed key would register. Rejected: the bracket and brace shapes are the ones Pommora means, and the quoting they force is emitter-managed and invisible to every caller.
