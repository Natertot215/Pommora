...
1. `.nexus/versions.db` through `openDb`, an integrity check that quarantines a damaged file by renaming it — its journal is reclaimed by the handle's close, and a store that is merely locked or unreadable is left for the next launch — one table `snapshots(page_id, ts, source, blob)`, `CREATE TABLE IF NOT EXISTS` on open, no version row; add, latest, list, read, delete, clear, sweep. The watcher ignores it through one `.nexus`-scoped `.db*` clause in `ignoredUnder`.
...
- 09-03-2026 (mine, tests pass): the `PageDetail` literal three renderer tests build stays in each file — a test shares fixtures through its own helpers, and no cross-file test-helper module exists to hold one.
- 09-03-2026 (mine, secondary review): `capture`'s interval gate reads and writes `lastTs` with only synchronous statements between them — the config read awaits before the read, so two overlapping tails cannot both pass the gate; `readBodyEpoch` and `subscribeBodyEpoch` stay exported as the read half the tests observe through, as `readCache` and `captureCache` are.
- 09-03-2026 (mine, secondary review): `withStore` stays its own three lines — `contentIndex.ts`'s `guarded` and `queried` and `localState.ts`'s bare checks each encode a different answer to a missing handle (log and swallow, silent null, a bare fallback, a `Result`), and a factory parameterized on that policy is more mechanism than the guards it would replace; the history window's two nexus date-and-time selectors are the reads every surface makes through `useSession`, with `nexusDateFormat` already holding the derivation, and a hook for two lines earns nothing; `refresh` carries no liveness flag — React drops a set on an unmounted body, and two refreshes cannot interleave past the confirm dialog every action awaits.
...
- 09-03-2026, secondary review, final pass: the quarantine's damage test is `damagedStore` in `driver.ts` — SQLite's not-a-database and malformed-image codes both — since a truncated store reports the latter and would otherwise stay dead on every launch; the garbage-header test again couples the set-aside name to `ignoredUnder`. The review's six lenses, its tests pass, and this final pass are recorded above; nothing else regressed.
- 09-03-2026, secondary review, tests pass (every finding folded but one): four tests were proven green without their guard — the quarantine's journal siblings were asserted kept by a loop that never ran, since SQLite's own close reclaims a WAL and SHM before the rename, so `quarantine` is the one rename it performs and its test asserts the one set-aside file; `readPersonalization` now proves an explicit `fileHistory: true` is not held; the watcher's arming test asserts one call on a change event and none on a removal, without a write that reached nothing; the timer tests assert the timer count where their titles say disarm, and the deleted-page case keeps the file on disk so only the tree lookup can decline. The rest: `fileHistory.test.ts` derives its clock from `HISTORY_INTERVAL`, builds envelopes through one `page` helper, names `nextMs` for the millisecond that keeps two rows apart, separates the wait from the count, and asserts error codes; the cap, the refusals, the clear, the delete, the flush, and the reset are each their own case; the sweep reads the nexus's timeframe; the root switch pins its row count; `versionsDb`'s `opened` throws rather than asserts; `readNexus` pins rounding with `12.6` and reads the range from `HISTORY_DAYS`; `PageView`'s fixture is in `beforeEach` behind one `slot` helper with the debounce named and the timers narrowed; `replaceBody` asserts the detail alone and arms its save through one helper; `pageFile` names the errno; `page` asserts the envelope whole; `sessionDb` makes its root before the test; the card and cell menus locate the send block instead of counting from the end; `connMenu` asserts the action list; `pageMenu`'s separators are their own case; `useWindowWarm` and `previewSlice` title both halves; `TabState` reads the literal epoch.
- 09-03-2026, secondary review, pair 3 (long-term stability and debt): `openDb` answers the failure's SQLite code beside its null, and `openVersionsDb` sets a store aside only for a file that is not a database or fails its check — one that is locked, mid-sync, or unreadable is left intact for the next launch, as `nexus.db` is, which retires the Gate 1 ruling that the driver surfaced no code; `clearSnapshots` vacuums and checkpoints, so Clear History gives the file's bytes back rather than holding its high-water mark; the window's restore-signal comment sits on the state it describes; Context's `versions.db` Known Issue names the vault's own `.gitignore` as where it closes. Each carries a test.
- 09-03-2026, secondary review, pair 2 (correctness and duplication): `replaceBody` cancels the path's pending save as its first act, before the refetch it used to wait on — a refetch that failed or outlasted the autosave debounce let the pre-restore body land on the file main had just restored — and answers whether the slot was re-seeded, so `restoreSnapshot` reports a restore the editors could not follow; its test now holds the refetch open across the debounce and covers the failed refetch; the window's right-click batch rule is one line; `capture` takes the hash `writeBody` already holds; `readFileHistoryConfig` lives in `settings.ts` beside the other named readers of `readLivePersonalization`; `ancestryOf` has its own description back; `DesignSystem/Util/clamp` re-exports the shared `clamp` rather than restating it.
