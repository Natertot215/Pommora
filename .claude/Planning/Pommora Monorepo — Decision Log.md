...
- **B-3:** [proposed] Core is filed by domain, not by mechanism and not by process. One folder per thing Pommora has, holding everything about that thing; a future feature lands in the folder named for what it touches:
  ...
  ├── // Assets          | • Asset map, roots, write, migration, mime, crop geometry; the image component, the crop editor, the entity icon, the Nexus photo
...
- **C-1:** [confirmed] UIX is the design kit and nothing else. It imports nothing from Core and knows no Pommora entity, with one ruled exception: the icon-picker binding in `UIX/Pickers/IconPicker` reads the session's favorite icons and the native favorites menu (09-05-2026). The Showcase renders the rest alone. Its categories sit at the root rather than under a Primitives folder, by ruling:
