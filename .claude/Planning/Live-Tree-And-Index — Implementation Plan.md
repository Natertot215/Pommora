...
> **Status:** executed — closed. All five phases shipped and gated; the sole open item is the property-cascade journal named in Sequenced After.
...
**Steps:**
- [x] Tests per arm: patched tree matches a fresh walk of the mutated fixture (reuse the Task 4 must-agree harness); registry rename shows the *disk-normalized* name inside a collection's embedded defs; view save and `container:configure` show in the container node; `setContext` patches with zero walks; an option rename via `PropertiesPane` and a nexus rename via `Banner` each land without ⌘R *(the two UI paths land in Task 7’s manual sweep)*; the negative control both halves *(red-proven by routing the arm to refresh)*.
- [x] Remove the Task 3 write-invalidation bridge — the `writesSeen()` bracket in `ipc.ts`'s ask wrapper and the context-menu confirm's `invalidateLiveTree()` call in `index.ts` (keep `invalidateLiveTree` itself if a fallback arm still wants it; delete `writesSeen` if nothing else consumes it): with every write channel patching for itself, a blanket drop-on-write would defeat each patch it rides behind. *(All three deleted — nothing consumed them; the bridge tests went with them.)*
- [x] Full gates green.
- [x] Commit: `feat(main): every write channel confirms by patch and push`
...
**Steps:**
- [x] Re-run the derivation; adjudicate every site in the Log; drop the write-confirm calls; run full gates — green. *(34 raw hits, control ≥3 held. Dropped: `store.mutate`'s closing `load()` + `invisible` list, `submitPropertyRename`, ViewPane ×3, ViewDropdown, ViewSettings ×3, PropertiesPane ×2, SettingsPane, CardsView banner, Banner rename, and `viewMint`'s whole refetch machinery (the parameter died with `skipRefetch`; `useSaveView` narrowed). Survivors: App mount + reload-state, `openVia`, TrashLeaf's restore batching, and every non-tree `load()`. One renderer test inverted: PropertiesPane's assign now asserts NO reload.)*
- [x] Manual pass: rename, move, reorder, icon, banner, view save, property rename, option rename, a table cell edit — each lands visually with zero walks logged (the cell edit with zero pushes); then ⌘R and confirm `stabilize` reports the walked tree identical (the drift detector). *(See Gate 3 Evidence — the two cascade walks are Task 11's named kill.)*
- [x] Rewrite the five docs in this commit.
...
**Steps:**
- [x] Failing tests: both must-agree crossings, upsert/prune reconcile, null-Db signaling, the rename move, the upgrade-in-place control (both halves — red-proven by reverting the opener's re-apply: exactly the upgrade test failed). *(The stat gate carries its own two halves: a sabotaged row survives an unmoved file's re-seed, and a moved stat heals it.)*
- [x] Implement; full gates green.
- [x] Rewrite ArchitecturePM's "none exists" Pending entry in this commit *(plus the two sibling claims the index falsifies in the same doc: "one table of substance" and "every database action is one statement")*.
- [x] Commit: `feat(db): the content index — mentions and property values, open-reconciled`
...
**Steps:**
- [x] Run the acceptance criterion end-to-end on the dev build (including the pre-existing-db upgrade case); log the observations in the Log. *(See Evidence — every clause observed live except the two db-lifecycle cases, which are unit-covered.)*
- [x] Dispatch `comment-killer-agent` + `code-simplifier` over the full range; fold verified findings. *(Comment pass: zero removals — every comment in the range is a genuine why. Simplifier: five cross-phase folds, verified and committed at `81ee1651`; verdict saturated, with the four-site `excluded_folders` decode named as a deliberate keep.)*
- [x] Write the Delivery Claim; dispatch the neutral verifier (claim vs. the ratified requirements); on a clean yes, dispatch `build-breaking-agent` against the shipped range. *(Verifier round 1: R9 failed — fixed at `7bd6f153`, re-adjudicated CLEAN YES. Attack round: 10 findings, all verified and fixed at `ecb0eda8` — see Closeout.)*
- [x] Closing sweeps: Dead Vocabulary against control; every Made False row's rewrite verified landed in its named commit. *(`allCollectionFolders` 0; `readNexus` control exactly the three seam files; every stale Made False phrasing greps to zero; instruments and TODOs at zero.)*
- [x] Doc rewrites + History entry committed; lessons routed to `.claude/Guidelines`. *(PM-105; FrameworkPM + ContextPM; Build-Gotchas sandbox recipe + the new Data-Layer page.)*

#### Gate 5 — closed
- [x] Acceptance criterion observed and logged. *(See Evidence.)*
- [x] Neutral verification clean; attack round's findings fixed or ruled. *(CLEAN YES after the R9 fix; all 10 attack findings fixed, the symlink-root unknown accepted as a pre-existing shape.)*
- [x] Line-count delta reported (code only, comments/tests excluded). *(Net +1163 | +1417 / −254; raw incl. tests +3298 / −404, 66 files.)*
- [x] Progress hashes filled in; Closeout written.
...
- [x] **Phase 5** — Closeout · base `934fba96` · simplifier folds `81ee1651` · R9 fix `7bd6f153` · attack folds `ecb0eda8` · record commit closes the arc
  - [x] Task 12 — reconcile the record
...
### Deviations
- Task 6: the field-write arms (`setIcon`/`setBanner`/`setHeadingIconHidden`/`setContext`, the profile trio) confirm by **re-reading the one file the write changed through the walk's own readers** (the Phase-2 disk patchers, now shared confirmers) instead of patching request values — the same exactness argument the plan makes for the registry family, generalized. Creates follow their insert transform with one order re-read (`page_order`/`set_order`/`state.json`/`space_orders`), because a create's position derives from an order file the transform never saw. `patchForMutation` keeps the plan's sync signature for the transform arms; the routing lives in `confirmMutation(root, req, reply)` (async, returns the tree to push or null), with `confirmRegistry` and `confirmBy` beside it. The context menu's `onChanged` now carries `(req, reply)` so its mutations confirm identically. Remaining refresh arms: `restore` and a Context's `setIcon` (registry-structural) — both rare.
...
### Evidence
- **Task 12 acceptance run** (sandboxed dev instance — separate userData via the temporary env-gated line, temporary `[walk]` + `[cascade-open]` logs, all removed and grep-verified after; seeded nexus of 196 pages, 3 in-tree mentioners of P, an excluded `Hidden/` note linking P): open logged **exactly one walk**. Renaming P opened **exactly 4 files** (the 3 mentioners + the loose note) — the excluded note unopened and byte-untouched — with zero further walks. Renaming the **nexus itself**, then renaming P again: the cascade opened the **same 4 relative paths** under the new root and rewrote them (relative keys survived the switch; the root-switch reseed walk is the one deliberate walk). A **cell edit** cost zero walks and zero pushes. External **create, edit, delete** each surfaced within the settle (one push each, zero walks). An outside-the-tree note (`_drafts/`, hidden from the walk, inside the corpus) took an external **edit with zero walks and zero pushes**, and the next rename opened **exactly that one file** with its freshly-indexed mention. A **restore** (no patch transform) landed via the fallback refresh, the page back on disk and in the tree. **Drift capstone:** the served tree was captured after the full sequence, a structural no-op forced the verification walk, and the re-served tree was **byte-identical**. One fixture note: in a sidecar-mode nexus the open pass adopts bare top-level folders (the `Loose/` fixture folder was minted a sidecar and correctly became page-upsert territory), so the genuinely-outside-the-tree case rode `_drafts/` live plus the raw-mode unit coverage in `indexMaintenance.test.ts`. The pre-existing-db upgrade and delete-rebuilds cases are unit-covered (`contentIndex.test.ts` upgrade-in-place; the seed suite).
...
**Neutral verification, round 1:** eight of nine VERIFIED; R9 NOT VERIFIED — the Context cascade's nexus-wide sweep enumerated through raw `listMarkdownFiles`, honoring no exclusions, and the per-folder scans (`removeProperty`, `loadValues`) could reach an excluded folder nested inside a Collection. Fixed at the enumeration: the sweep's `'nexus'` scope now rides `nexusCorpus`, the per-folder paths ride a new `folderCorpus(root, folder)`, and the caller-less `'collections'` scope was deleted. Red/green-proven by a sweep test against an excluded folder's governed key (the file stays byte-identical; reverting the enumeration turns exactly that test red).

**Neutral verification, round 2:** R9 re-adjudicated at `7bd6f153` and confirmed — the one predicate now sits behind the walk, watcher, index corpus, rename cascade, key-holder queries and fallbacks, the governed sweep, and the per-folder scans. **Overall: CLEAN YES, all nine requirements verified at HEAD.**

**Attack round (build-breaking-agent, shipped range):** 10 findings — 1 High, 3 Medium, 6 Low — every one the same shape: a correct guard installed at n−1 of its n sites. All verified against the code and all fixed, none deferred: the walk-supersession epoch now also arms on the refresh arms (`noteDiskMoved` before `confirmBy`'s and the settle's walks — the High: a refresh could join a pre-write walk and install stale disk as canon); an exclusions edit re-arms the watcher, closing the stale-capture route back into the index; a rename/move over an adopted-id subtree degrades to the walk (a path-derived id can't be re-derived in shared code); the confirming push is deferred one macrotask behind the invoke reply, making the renderer's create ordering deterministic (and the two contradictory comments agree again); the Space-color clear and the icon clear both keep their keys (the factories' shape — `stabilize` counts keys); the transforms re-point and prune `tree.unreadable` across renames, moves, and removes; reorders pin their landed order from disk like creates do (`reorderChildren`/`reorderTop`/`reorderSpaces`); the seed re-checks the stat row before writing so a concurrent maintainer's fresher rows survive; the raw-mode gate moved inside `patchContainerFromDisk` so all its consumers answer identically; and the sentinel view adoption lands in the `activeViews` slice via a store-wired callback while the system-accent cache refreshes fire-and-forget per push. The round's two unknowns: the create-race visual check is moot under the deterministic ordering, and a symlinked nexus root degrades every watch event to a full refresh — correct, just uncached — accepted as a pre-existing shape.

**Delivery Claim** (adjudicated against the nine Requirements): (1) In-app mutations trigger zero full walks — every write channel confirms by patch/no-change/refresh against the live tree in `src/main`, observed live: a cell edit costs zero walks and zero pushes, tree-moving ops exactly one push. (2) External edits surface at per-path cost through `classifyEvent`'s targeted arms; everything unclassifiable falls back to `refreshTree`. (3) A page rename queries `mentions` and opened exactly its 4 mentioners on the acceptance nexus — over the cascade corpus, un-adopted folders included. (4) Property cascades (option rename/remove/clear, delete's snapshot + sweep, the rename sweep) query `page_values` at their call sites, scope-intersected with governing Collections; the Context cascade keeps its scan. (5) `mentions`/`page_values` (+ `indexed_files`) land via idempotent DDL the opener re-applies to existing databases, seeded/reconciled at open, maintained by every writer and watcher arm, disposable by construction. (6) The verification walk survives at open/⌘R, transform-built nodes carry the walk's key shape, and the live drift check returned a byte-identical tree. (7) Every `readNexus` caller rides the seam except `record.ts`'s stated seeder-exception; `trash:list` serves the held tree; `allCollectionFolders` is deleted. (8) Every Made False rewrite landed in its falsifying commit (one row's target text had already left the doc — recorded). (9) `excluded_folders` is total: one `excludedMatcher` behind the walk, watcher, corpus, index, and every pen — the excluded fixture note stayed byte-untouched through every cascade, live.
