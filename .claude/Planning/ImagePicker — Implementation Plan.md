...
**Steps:**
- [x] `git mv`; rewrite; the glyphs; the mounts; the Showcase; the docs; tests; `npm run build` 0 and gates green. `PhotoCropModal` → 0; `Move and Scale` → 0; `dataUrl` → 2 (WebpageEmbed). A-5 re-pick + paste built (see Deviations); `nexus:pasteImage` channel added.
...
- **A-5 built (my call, Task 7):** Re-pick and paste land through one seat callback, `onRepick(source: string)` on the picker — the trailing folder-open Button calls `pickFile()` then `onRepick(path)`; a document `paste` listener (gated on `open` + `onRepick`) calls a new `nexus:pasteImage` channel. That channel reads the OS clipboard image **main-side** (`clipboard.readImage()`, no bytes over the bridge), writes a temp PNG to the OS tmpdir, and returns its path, which `onRepick` adopts through the seat's existing `setProfileImage { source }` — the same source-adopt path a pick uses, so no byte-baking channel returns. The icon seats wire `onRepick`; banners get it in Task 8.
