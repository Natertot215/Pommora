...
**Ruling required at ratification — the frame is the viewport.** `coverStyle`'s percentages resolve against the painted element's own box, so the picker shows the seat's exact paint only if the frame *is* the element: `280 × round(280·boxAspect)` for `rect`, `280 × 280` for `circle`, the ring at inset 0, the circle cut by `border-radius: 50%`, and no blurred margin outside the frame — the margin's image could only be a second paint that cannot align with the first. A-4's "the current geometry" (a 220 circle inside a 280 viewport with a blurred surround) and B-2's "the result rather than a likeness" cannot both hold; the plan builds B-2. *(Nathan: yes / no.)*
...
**Survivors:** `backdrop`, `panel`, `viewport`, `grabbing`, `message`, `actions` — reused as they stand.
