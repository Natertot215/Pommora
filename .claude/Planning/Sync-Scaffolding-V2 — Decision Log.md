...
> **Standing:** Closed 09-14-2026 · [[NexusSyncPM]] describes what stands. Supersedes the retired [[Sync Groundwork — Decision Log]] and [[Mobile Companion & Pommora Sync — Decision Log]] (both recoverable from git at `7c7a68f47^` and `5f4b07c0b^`) for everything above the identity layer; the identity layer they produced stands and is extended here, never replaced.
...
- [[NexusSyncPM]] — what stands: the manifest rule, three identities, the keys, the device model, signed requests, the hub and its routes, the change log, the conflict rule, and the landing. Its "never a dependency" line stays true under this log.
- The `Sync/` folder on built-ins — `hub.ts`, `authority.ts`, `feed.ts`, `Routes/`, and `Store/` — which carries the roster, the change log, the blob store, and the waiter registry. The log was drafted against its one-file predecessor, `Sync/server.ts` (316).
- `Core/Sync/` — the wire types, the canonical string, the signed `call`, and the `sync:*` channels. The log was drafted against the four flat files that held them, `contract.ts` (62), `authority.ts` (17), `client.ts` (48), and `handlers.ts` (133); what stands is `Core/Sync/Contract/` and `Core/Sync/Client/` beside `Keys/`, `Arrival/`, and `handlers.ts`, carrying the same contents under H-1's structure.
...
- **D-2:** [confirmed] Request authority becomes one function on the hub that resolves a request to a device row and a role; that resolution was inline across two branches of the one-file server's `route()`, where `verifySigned` only checked a signature. Accounts, OAuth, or any other method later resolve a caller through that one function; no route reads a signature directly. This is the don't-foreclose for authentication methods.
...
- **E-2:** [confirmed] Verbs added to the route table: `store` (items with base version; per-item typed outcome in a 200; conflict outcome carries the hub's item) and `pull` (`{changes, cursor, hasMore}` since a cursor; typed resync error when the cursor predates retention). `pull` long-polls: a request carrying a wait budget parks in a waiter registry (`Sync/feed.ts`) until the Nexus's counter passes the cursor or the budget expires, which is E-2's one-directional intent without a stream encoding, so there is no separate feed route and no library. A `store` may mark an item as a capture, which the hub keeps as a non-head version under retention without advancing the item's head. A `store` is one SQLite transaction and carries a request id, so a retry after a dropped reply is idempotent. The client persists its cursor per change only after that change is applied.
...
- **E-4:** [confirmed] The wire stays JSON over the existing signed-request shape for verbs; bytes are content-addressed and travel on their own route, `PUT /blob/<nexusId>/<sha256>` and `GET /blob/<nexusId>/<sha256>`, with the signed body hash as the blob id, so `store` and `pull` stay small JSON bodies referencing hashes. The Nexus id rides in the byte path, so authority resolves before a byte is read. The transport member and `machine().sha256Hex` gain byte forms, the signing string hashes the raw body as today, the hub streams a body to a spool rather than buffering it, and the body cap and the request timeout are per-route (8 KiB and ten seconds for verbs; 50 MB and a transfer budget for bytes).
...
- **E-6:** [confirmed] A rename is one change-log entry carrying both paths, so no landing ever sees one `ID` at two paths; a move is the same entry. The rename and move operations report `{ from, to }` to the sync client once their cascades settle, and the client emits that one entry; a rename Pommora did not perform arrives as a delete and a write, pushed in that order. The hub keeps the item's record unchanged across a rename, since a blob is sealed under the path it was written at, so the record's path is the path every decrypt authenticates against while the change's path is where the file lives.
...
- **F-3:** [confirmed] A landing over an open page is absorbed by a three-way merge in the editor's host, on receipt of a `pages:changed` push from the watcher: base is the text the editor loaded, local is the buffer, remote is the landed file; the caret holds. A clean tab reloads silently as the degenerate case. This rules D-2 of the audit. `pages:changed` is declared as a Push in `bridge.ts`; the merge library is imported by the renderer only, behind one seam, so it never enters the engine graph; `Core/Sync/Arrival` never reaches the editor, it lands the file and the push does the rest. The merged transaction carries `Transaction.addToHistory.of(false)`, so a landing stays outside undo. A body write is compare-and-swap on the body: `page:updateBody` carries the hash of the body the editor last loaded or saved and is refused when the bytes on disk differ, and the refusal routes to the merge rather than to a retry; a caller with no hash writes unconditionally. The editor holds that base for as long as the page is open, a save sends what it holds rather than fetching, and a landing with no base is a conflict — the buffer is captured, the remote text lands, and the merged text is written back whenever it differs from disk.
...
- **F-5:** [confirmed] Offline: the client keeps writing locally; on reconnect it pushes first, so every dirty item meets the version precondition, then pulls; a landing never overwrites an unpushed edit. Before landing a change the client compares the file's hash to its base record, and a difference pushes first and re-evaluates. A Markdown file with no `ID` key waits until adoption stamps it, since the stamp is an app write the tap sees. Two devices editing one page offline resolve to the newer mtime with the older captured on both sides.
- **F-6:** [confirmed] JSON files named in C-3 land through a key-level three-way merge: base is the last synced bytes (kept in the `sync` table for these files, since they are small, and regenerable from the hub by version), local is the file on disk, remote is the landing; a key changed on one side takes that side, a key changed on both takes the newer mtime's. Arrays inside a key (`page_order`, `views`) are one key, and a per-file depth table carries the merge one level further into `personalization`, `order`, `navigation`, `defs`, and `byImage`. Merged bytes land under the current time rather than the writer's, and the base record holds the hub's plaintext and version, so the next push carries the merge against the right base and the client's stat comparison cannot mistake a merge for the remote. This is the last phase of Arrival, never cut; it shares its base machinery with F-3.
...
- **G-1:** [confirmed] The sync feed taps the watcher's `onEvent` above the write-echo check, so the app's own writes reach it; chokidar's ignore predicate admits tile bodies, and a predicate in `onEvent` hands those paths to the sync feed and returns before `batch.push`, so the tree never walks them. `.trash` reaches the feed through the write funnel (`Core/Files/writeEcho.ts`) instead of the watcher, and every Trash writer that removes or moves a bundle records its write first. The client never walks except at bind and on resync; the manifest walk runs once at bind and on resync. A base record per item (`path, mtime, size, hash, version`) lives in a `sync` table in `nexus.db`, since it is device-local and regenerable.
...
- **K-5:** [confirmed] [[NexusSyncPM]] and [[Cross-Device Mutation Checklist]] restate the manifest rule without thumbnails and journals.
### Core (must-have)
...
- Changing a Nexus password — why deferred: no channel carries the re-wrap; don't-foreclose: B′-1's ring re-wraps without re-encrypting content.
