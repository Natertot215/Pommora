...
### Closeout Attack Review (final)

Ran after both gates; adversarial frontmatter and trees came back clean under execution (symlink loop, missing folder, nested asset root, agenda subfolder, nexus-root exclusion, map/alias/quoted/empty-`<>` keys — all killed). Six findings; two fixed, the rest flagged or accepted.

- **#2 (Medium-Low) — fixed.** Rapid successive edits in the Manage pane raced: `commit` was fire-and-forget and the pane reads the list back through the tree, so a second edit computed from the stale list undid the first. Now one write is in flight at a time — `commit` no-ops while `busy`, and the remove/Add controls disable during the round-trip.
- **#3 (Low) — fixed.** `readExcludedLeaf` didn't dedup, so a hand-edited case/NFC-duplicate pair both survived and double-walked (over-counting sidecars). It now case-folds and dedups like `sanitizeExclusions`.
- **#1 (Medium) — flagged for the live pass, not folded.** A successful Clear shows nothing; the `refused` count reaches the renderer and is discarded, so a partial clear (Unknown files that retained data) looks identical to a full one. Surfacing it is a UX decision (a completion dialog/toast, and whether to add a failed-sidecar count) that belongs in Nathan's Phase 2 live pass rather than an invented dialog. Uncommon precondition (a malformed/misplaced file in an excluded folder).
- **#4 (Low) — flagged, out of scope.** A cleared plain-adopted page whose only frontmatter was `PageID` ends `---\n{}\n---`, an empty-map fence — `mergeFrontmatter` emits `{}` when every key is deleted. The fix lives in the shared `mergeFrontmatter` (drop the fence on an empty map) and carries its own blast radius across every delete-to-empty caller, so it is not an Exclusions edit. Re-reads fine; a legibility wart only.
- **#5 (Low) — accepted.** A mid-sweep `atomicWriteFile` failure (disk full) leaves a partial clear and a bare "operation failed"; atomic per-file writes + idempotent re-run + the startup re-seed heal it. Not worth a transactional restructure.
- **#6 (Latent) — accepted.** The walk skips `.`-dirs and `node_modules` but enters `_`-dirs, unlike the shared `hiddenName` (which also skips `_`). No producer (nexuses hold `_`-prefixed sidecar files, not dirs), and reusing `hiddenName` would make Clear miss `_`-dir content the corpus indexes — the current rule is the intended reach.
