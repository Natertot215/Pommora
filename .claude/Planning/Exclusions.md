...
- **#1 (Medium) — resolved (Nathan's call).** A successful Clear now flashes "Cleared" on the destructive button for 400ms, then reverts — the confirmation a silent success lacked. Nathan chose this over a dialog or surfacing the `refused`/failed-sidecar counts; the counts stay in the report unused, which is accepted.
