...
- **D-6:** [confirmed] **Seeded Contexts render pre-empty** — Areas, Topics, Projects show a row whether or not the page holds a value, so the slots read as ready to fill; `—` is the empty render. The inspector reveals a row only when the user picks it from `+ Add Property`, so what the leaf adds is narrow: it **seeds that reveal set with the seeded Context ids at mount**. A pre-empty row is display only and writes nothing — the reveal path shows a row without touching disk, and a value deletes its key when emptied rather than storing a blank, so no new guard is built for it.
...
- **Configurable default Contexts** — which Contexts pre-show as empty rows is fixed to the registry seeds for now; letting the user choose that set is deferred. Don't-foreclose: the pre-show decision reads from a single predicate over the registry, so widening it later is one function, not a re-shape.
