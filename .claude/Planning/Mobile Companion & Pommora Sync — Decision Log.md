...
- **H-3:** [confirmed] Settings › General gains two sections: Account (server address, email, password, Sign In, Sign Out, device name) and Sync (remote Nexus Create From This Nexus or Connect, Nexus password, status, Sync Now, Disconnect). Connect lists only remote Nexus' whose id matches this Nexus's `nexus.json` id and refuses a mismatch with the reason; Create against an id the server already holds becomes Connect. Every action has its inverse.
...
- **I-1:** [confirmed] First run: server address, sign in, pick a remote Nexus, Nexus password, pull, open. The local copy lives in the app's `Documents/<Nexus name>/`, exposed to the Files app as `On My iPhone › Pommora › <Nexus name>` by `UIFileSharingEnabled` and `LSSupportsOpeningDocumentsInPlace`, the shape Obsidian uses for the Nexus on Nathan's phone today (ruled 09-04-2026). Known limitation: other apps may then edit in place, which Apple says obliges file coordination that Capacitor's Filesystem does not perform; Obsidian ships without it, most-recent-wins governs the race, and a coordination plugin is a Prospect. Per-device state stays out of Documents under `Library/`. The Simulator's container is inspected with `simctl get_app_container` for verification.
...
- The Pommora Sync server with its change feed, packaged as one deployable container; the sync client in the engine with push-on-change and feed subscription; the desktop client in main; the phone client in the host; and the Settings › General rows to sign in and connect a remote Nexus.
...
- Argon2id or scrypt key derivation — why deferred: neither runs natively on both runtimes today; don't-foreclose: the KDF parameters live in the Nexus record, so a new KDF is a new Nexus.
...
- Multi-user Nexus' — why deferred: single-user by the PRD; don't-foreclose: F-1's wrapped-key list.
...
- Desktop as the v0 remote Nexus (LAN server in main) — rejected 09-04-2026: v0 must establish the long-term approach, which is a server.
...
- A password-derived Nexus key used directly on content — rejected: a password change would re-encrypt the whole Nexus; a random Nexus key wrapped by the derived key costs one record.
- A separate key-verifier record — rejected: the GCM tag on the wrapped Nexus key already answers a wrong password, and Obsidian documents no verifier of its own.
