...
- [x] `Windows/` — `PageWindow`, `WebWindow`, `NavWindow`, the tab strip, the warm cache, the morph; the chassis is `DesignSystem/Components/WindowBase` (`.window-*`), `SidePane` beside it; `Detail/DetailPane` → `Detail/ContentView`; `Tabs/NavView` → `Detail/NavView`; `NotchedPane` → `NotchedShell`.
