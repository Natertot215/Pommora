...
`renderer/SurfacePM/` became `renderer/Tiles/` — `Core/` unchanged, `TileGrid.tsx` (was `SurfaceView`), `TileHost.tsx` (was `TileSurface`), the four bodies under `Surfaces/`, `tile-base.css` (the `.tile-base` chassis) and `tile-grid.css` — and every tile-system identifier, IPC channel, CSS class, variable, and user string that said "block" says "tile": `shared/tiles.ts` carries `TileHostRef`, `TileEntry`, `TileDoc`, and `knownTile`; the channels are `tiles:*` under `window.nexus.tiles`; the classes are `.tile-host`, `.tile-grid`, `.tile`, `.tile-handle`, `.tile-edge`; the Scale ramp is one inline `--tile-zoom` set through `TileGridProps.tileStyle` from `zoomStyle` in `tileZoom.ts`, replacing seven class rules. MarkdownPM keeps "block" for its own CM6 blocks. The Features doc is [[SurfacePM]] with Tiles as its section.

**The engine:** the grid's edge and handle drags moved from a private rAF sensor (`Sensors/pointerDrag.ts`, deleted) onto `Interactions/gesture.ts` through `usePointerGesture`, and the embed tile's bottom-edge handle became a `useResizeFrame` consumer whose `rect` is measured at press. A frame release that never travelled, or that returns to its start by value, emits no drop; a drag back to its origin restores the start; the release after a cancelled drag is not a click; a move decided at a drop commits even when the grid unmounts mid-settle. Over CDP a sixty-move edge drag fired `onDragMove` once per move and half a call per frame, so the engine gained no coalescing. The grid's six never-passed props are module knobs and its feel is `GLIDE_FEEL`.

**The recipe:** the three tile kinds are declared in `TILE_KINDS` (`shared/tiles.ts`: schema, `fileBacked`, `menuRows`), `TILE_SURFACES` (`Tiles/tileKinds.tsx`: render, `sourceInfo`), and `TILE_COPY` (`main/tiles.ts`, the view re-mint, dispatched through `copyEntry`); `knownTile` parses through a union derived from the table; the host's render, `TileHandleMenu`, `tileMenuModel`, and main's lifecycle (`reviseTile` under remove and the two converts) read the tables, so no `.type === '<kind>'` comparison remains outside them. The in-app handle menu opens at its root, holds its entry through a waived delete, and refuses a Link row or a drill leaf exactly where the model does. A layout has one definition: the schema holds the split and ratio-count rules, and `validateLayout` keeps only what a schema cannot say.

**The document:** each host's layout, entries, and lock moved from a per-machine `local_state` row to `_tiles.json` in the host's folder, beside its tile bodies, so it syncs with `.nexus/`. `main/tileDoc.ts` reads it read-only (absent or corrupt reads empty) and writes it through `rmwJsonStrict` under the file's own lock, quarantining corrupt bytes to `_tiles.json.bad-<ulid>` before a write lands and returning its own failure; one JSON decode (`parseJsonText`) reads a BOM as encoding for every file another app may have written. The watcher names the file, exempts it from write-echo suppression the way navigation is, and pushes `tiles:changed` per host; `useTileDoc` answers by flushing its pending save, awaiting it, re-reading, and replacing its state — identical bytes under one stable serializer (`shared/stableJson.ts`) change nothing, and a push landing mid-gesture waits for the settle. Every `tiles:*` channel refuses while a nexus adopts, the Homepage host remounts per nexus, and a markdown body the read fails on renders inert rather than empty. NexusOS's legacy rows were moved into 11 files once (12 rows, none divergent) and the migration was then retired by Nathan's call; a machine that never ran it carries its rows over by hand. `MAX_INSPECTOR_TABS = 6` and the `state.json` `inspector` key are reserved with no reader.
...
MarkdownPM gained GFM reference footnotes: `[^label]` markers in a page's body and a trailing run of `[^label]: text` citations at its end, drawn as positional numbers over a section that hides by default, with plain GFM left on disk.

**The boundary:** `citationScan(d, excluded)` in `MarkdownPM/detect/index.ts` derives where the section begins, and the decoration pass, the block resolver, the heading-fold scan, the transaction guard, the Subfield counter and the fold region all read that one answer through the editor's cached per-document scan. `computeStats` excludes every section line through the same mask, and a marker scores its own characters and no word.

**The drawing:** citation rows emit as the `lineWidget` intent the codeblock line count uses, so the glyph is a computed ordinal over a hidden, atomic `[^label]:` prefix; a body marker draws from the scan rather than the token pass, which can only show the label its own text holds. Table cells take the same number through a serialized numbering key their equality compares, so a marker added elsewhere redraws a cell the edit never came near.

**Hiding and showing:** the section became a `'citations'` fold kind seeded from a per-page row in `nexus.db` and excluded from the persisted fold store. `md-foldable` split into a chevron class and `md-heading-fold`, having been simultaneously the chevron, the heading-drag gate and the grip menu's hit-test, and `collapsedByDefault` was retired rather than repaired.

**Guards and gestures:** `verdictFilter` was lifted out of the callout guard, and the citations guard relocates any change that would stop the section reading as a trailing run, carrying the range it moves text out of. `citationDeleteIntent` states once what deleting a range means, and both menus, Backspace and forward-Delete all ask it. A relocation reads its destination off the anchor line it lands on, since the line above the section is a blank only in a file the editor wrote itself, and the seat a marker may be written at answers for the end of the selection the marker follows.

**Creation and numbering:** `normalizeCitations` renumbers numeric labels to first-use order and sorts the rows to match, refusing any rename onto a number a row that keeps its own still holds; `commitCitation` is the one dispatch all six gestures end on, so each is one transaction one undo takes back whole. A citation is seated at or after the marker it belongs to — seated at the body's last line holding content, the first footnote written from the empty last line of a document ending in a newline had landed above its own marker.
...
The Link property type added the ability to resolve internal links. A value is now read for what it names: a connection, or a markdown link whose target resolves to a page, commits as the same `[[Title]]` every other surface writes, under that page's own capitalization and carrying any alias through, while a title no page answers to is still refused, a link nothing answers to not being a link. The value then reads as the connection it is — the connection color, and a click that opens the page rather than a browser — and the three link formats stand down, an address's forms having nothing to say about a page. The rename cascade was widened to match, because a reference the cascade cannot reach is a reference that rots: it rewrites the connections held in frontmatter alongside those in bodies, and the content index records what a page's frontmatter names, so a page whose only inbound reference is a property value is reachable at all rather than silently passed over. Travelling with it, the link right-click menu left main's inline template-building for one shared model, which the editor, table cells, card values and both inspector panes now pop in place of the parallel menus they each carried — an address gaining Open Preview and Open Browser, each naming its destination rather than deferring to the open-in preference; an open item dropping outright where its own surface already shows that page, so a link naming the page in hand offers nowhere to open it; and a property surface closing on Clear where the editor closes on Remove Link · Delete, with Remove left on the property itself rather than on the value it happens to hold. The editor's Paste As menu closed the same distance for embeds: a copied address and a copied connection each offer their lone-line embed — Embedded Link and Embedded Page — but only where the caret already sits on a blank line one can be written on, since both are line constructs and a form that had to make room for itself would be an insertion wearing a paste's name.

- **Diff:** Net +448 | +655 / −207

#### PM-109 || The Color Ramp
**DATE:** 08-18-2026

The chip palette stopped being ten flat solids and became a grid. `tokens/ramp.ts` is now the sole owner of what a color is — eight families of eight steps, running from dark to light, and greyscale reading the app's own surface tokens instead of computing anything. Every spectrum solid sits on an exact cell, so a bare `red` already on disk resolves to `red-3` through one accessor and nothing is migrated, rewritten, or read differently; `chip.css.ts` generates its variants from that table rather than listing them, which turns a returned cell into a compile-time concern instead of a chip left behind. The mix helpers consolidated onto one primitive — `tintAt` had only ever mixed toward transparent, and the ramp needed black, white, and oklch — and four call sites that indexed a table holding only the ten solids were converted before a stepped key could reach them and paint nothing at all. Main validates a stored color against that grammar at every write boundary it owns.
...
The editor learned to embed live websites the way it embeds Pages, and the app grew a web layer around them: one guest governor in the main process, one open-link adjudicator in the renderer, one persistent session, and live hover previews. A markdown link on its own line with an explicit HTTP(S) scheme now renders as a live website tile in the shared embed framework — the document bytes stay plain CommonMark — with formatting deferred until the selection leaves the line, so typing an address never pulls a tile in under the caret.

**The Menu & Built-In Formatting:** the context menu was reordered around an Embed submenu that holds both embedding types, and pasted-link auto-formatting graduated from a setting to built-in behavior, retiring its toggle, its key, and the conditional-disclosure mechanism that existed only to serve it. A destination guard keeps a paste or ⌘K inside a link's literal parentheses.

**The Guest Foundation:** a single main-process module owns everything a webview guest is allowed to be — the attach validator that denies a wrong-partition or hostile attach outright, the popup handler that opens no OS window and hands every address to the renderer's adjudicator, the navigation scheme gate, and the zoom sync that stamps every guest from its host's factor times the webpage-zoom preference, which offers the usual steps and takes any scale typed into it. All surfaces share one persistent partition wearing a cleaned user agent, so a sign-in anywhere authenticates everywhere, per machine, across restarts; the session simply remembers, and no management surface exists — an accounts section was built to the spec and dissolved the same night on the standing decision that persistence alone is the product.

**The Tile:** a guest is live only while its tile is fully visible — the visibility model is the rendering model, since a partially clipped webview paints outside its box. A clipped tile keeps its last captured frame on its face rather than going blank, a failed one shows its domain, and re-entry retries in front of the user. The live tile is inert until clicked in, disengages on click-out or Escape, and scrolled-out guests hide under a capped least-recently-hidden retention rather than unmounting. Retention spans tab switches: the detail pane holds a recent tab's full page surface parked off-screen instead of tearing it down, so its sites pause and resume their sessions rather than reloading, and the flip itself costs a quarter of what rebuilding the surface did. Titles are display-only and resolve through the same fetched-title path table cells use; the grip's Edit Link re-aims the whole line in place through the grammar's own round-trip gate, migrating the tile's remembered height.

**Links & The Browser:** every external open routes through one renderer adjudicator honoring the open-in preference — settings keep it with the webpage scale under Interface — guest popups included. The in-app browser is a flavor of the floating preview window — back and forward lead the toolbar, the centered title tracks the guest's current page and opens it in the system browser, and a summon while open retakes the window in place on a monotonically increasing sequence of closes. The strip holds its own band above the page and paints nothing.

**Hover Previews:** a dwell on a website link raises the shared hover card as a live render of the site that takes no clicks — the card opens wearing a quiet cover that fades once the page paints, and a site that fails or never paints closes the card whole. The guest mounts visible behind the cover because a guest mounting inside a hidden subtree may never attach; the card's guest fills the pane edge-to-edge, clips its own corners, and a shield above it keeps every pointer event on the card's own leave lifecycle — passing down the wheel alone, so the site reads past its first screen without becoming clickable.

- **Commits:** `f9ca6aa8..HEAD`'s webpage-integration line — 73 commits interleaved with the parallel settings and ramp work
- **Diff:** Net +1420 | +2842 / −1422
...
The schema cascades gained what the Context rename already had: a crash record. A property rename commits the registry and then sweeps pages; a delete snapshots, strips, unassigns, and removes the definition; the option ops move one value across every holder — and none of them recorded that the sweep was owed, so a crash partway stranded half the pages against the registry, permanently and silently. Now every op that writes to both the registry and pages states its intent in `.nexus/property-cascade.json` before the work and deletes it after, and a surviving record replays at the next open — forward-completing the interrupted cascade against current disk. 

**The Record:** One file, one record, intent only — the op's identity, the names or values it moves, never a snapshot, because a remembered page list goes stale the moment disk moves without the app while the idempotent re-sweep cannot. The slot protects what it holds: a write never displaces a record stranded by a faulted session, and a clear lands only for the record its caller staged, so an owed heal survives every later op until the replay disposes of it. The clear guards on the live session's root and echoes its own unlink past the watcher, a gap the Context journal's clear shared and lost in the same commit.

**The Replay:** One law, applied per op — act only on the state the record exactly maps, identity-checked by id, and clear on every other. A rename sweeps only when the journaled def itself wears the new name; a delete forward-completes when its def still stands under the journaled name (the registry commits last, so that is the crash state) or when the name has been freed, and stands down when the id lives under another name or the name was re-taken; an option rename acts only when the def holds the new value and not the old; an option removal only while the value still stands in the def. It rides the schema chain as one wrapped entry after the index seed — warm queries, warm tree — never blocks an open, and every acting state was proven byte-identical to an uninterrupted operation. 
...
**Writes Patch Instead Of Reload:** Every write channel confirms by patching the live tree and pushing once — field writes re-read the one file they changed through the walk's own readers, creates pin their position with one order re-read, the registry family re-reads `properties.json` and re-resolves the affected collections so disk-normalized values land in both of a def's homes reference-identically, and the rare transform-less ops degrade to a refresh rather than drift. The push lands before the invoke resolves, so the renderer's optimism became idempotent (inserts guard on presence) and captures precede their asks. With main confirming everything, the hottest walk triggers in the app are gone.
...
`PickerMenu` dropped its beak and became the one surface every picker, grid, calendar, hover card and fixed-option menu mounts; `MenuSurface` keeps a beak for the toolbar dropdown alone. The beak had forced `NotchedPane` to clip the frost to a hand-drawn curve and stroke it as an SVG, which meant suppressing `GlassPane`'s own border — and since that border, the top specular, the inner ring and the lower rim ride one `box-shadow`, every beaked menu had been three lighting layers short. `NotchedPane` fell from 204 lines to 109 once its sideways path, flip, inset props and resize publication had no caller. `GlassWindow` and `GlassSurface` were byte-identical, so they merged under `GlassSurface` for the app's menus, pickers, and panes; and `GlassWindow` was rebuilt as the in-app windows’ glass, with four spellings of the same fill collapsed onto `SOLID_FILL`. The material also gained `--glass-outline`, the tinted edge a tier wears while its surface is being driven.

**Pickers & Menus:** `MenuOption` and `PickerOption` split rows by what they hold, sharing one `rowShell` and one `chosenMark` where four definitions had stood. `MenuItem` reads its ramp from a variable the picker's pane sets, so a picker's rows take the control scale without restating it, and a pane's pinned bars became `ActionRow` in place of a density zoom local to one stylesheet. `PickerMenu` gained `header`, `footer`, `anchorHeight` and `bounds`, and holds its last rows through the Bloom-out — seven surfaces had mounted it conditionally, which tore the pane out before it could retract.
...
The `.trash` gained the surface that reads it. `listBundles` was complete, tested and callable by nothing; it now keeps the artifact basename it already computed and reaches the renderer through `trash:list` as rows `crud/trashRows.ts` shapes — kind, title, a breadcrumb resolved from the recorded parent id against the live tree, the deletion instant unreplaced from the bundle's own folder stamp, and whether that parent still resolves, which the menu needs before any restore is attempted and cannot learn by trying. `Settings/TrashLeaf.tsx` draws them on `navList.css`'s row under the table's own `.table-head`, with a date column that takes its format and its clock from a menu on its own heading. `restoreArtifact` gained an optional destination that `withDestination` substitutes into the record before `resolveRecord` runs, so placement, the schema a returning page is reconciled against, and a Space's membership reapply all follow the chosen home; `emptyBundle` spends a bundle the other way, behind a guard requiring a readable record because `.trash` mirrors the nexus and a chain folder can wear the bundle suffix.
...
**The Rename Fence:** `renamingPath` was a bare global two-surface families-mounted field, and the collision was already live — renaming a Set from a table band with its sidebar row visible double-mounted the field, the focus fight blur-committed empty, and both fields died. The slot gained an owner fence: hosts claim per path, one winner resolves (the gesture's declared surface first, detail over sidebar next, first-come last), the release defers its verdict a microtask so StrictMode's remount never kills a live session, a rename whose surface leaves is abandoned rather than teleported, and a session no surface ever claims self-heals after a beat — a newborn a filter hides can no longer strand the session and reopen an unprompted empty field later. Cards' naming also unified onto the fenced inline field, retiring its rename popover.

**Order Honesty:** Creation now settles every live order in its own act — a newborn absent from a stale manual array ranked last no matter what the disk said — and the same law reached the moves: a card dropped across location bands carries the destination's full-membership order with its landing spliced in, the store's `movePage` arm honors it optimistically through the new pages-side reorder, and the set-card drop holds a synchronous order override across the reply gap that used to snap it back. Cards' reorder now writes its local `viewOrders` copy, and a non-structural band-add settles its tiebreaker with the newborn ranked last in its band.
...
**The Stale Slot:** Three surfaces could aim a drop at geometry that had moved — `groupingDnd` never invalidated its activation snapshot inside its scroll-capped region, the GFM table's wrap-relative slot math kept a press-frozen origin through editor scrolls, and `tableDnd` re-measured on scroll but not on a mid-drag rows push. An invalidation followed by a release with no further move still committed the old slot, so the fix landed everywhere as one invariant — an invalidating event re-aims from the last pointer point, and the drop consults the snapshot's dirty flag before reading its slot — with each surface carrying a release-without-move regression test. `GroupingPane` and `NavList` memoize the row lists their hooks key invalidation on, since a caller rebuilding that list inline turned every move into a full re-measure.
...
Resting on a resolved `[[Connection]]` opens a compact read-only preview of the target page through the shared embed framework, without its banner or inline title. The card anchors to the live link with the pane's beak sliding to keep pointing at it, and holds glance-only interaction — content scrolls, headings fold on click, and the caret never enters. It resizes from its right edge, bottom edge, and corner to a single per-machine-remembered size, and the Settings ▸ Pages slider sets how long it lingers after hover-off. Resting table cells raise the same card through the intent delay the editor's own links use. `ConnectionHoverCard` became one app-level mount rather than one per link, a click consumes a pending intent, and navigation closes an open card.

- **Commits:** `71b423c1^..d6aba1d0`
- **Diff:** Net +510 | +594 / −84

#### PM-081 || Four Consolidations Off The Debt Ledger
...
#### PM-046 || Every Drag's Edge-Scroll Onto One Loop
...
#### PM-023 || The Hot Paths And The Inspector Teardown
**DATE:** 07-02-2026
...
#### PM-011 || Window Chrome — Toolbar, Inspector, Subfield
...
Drag and drop came in-house rather than rented, backed by a line-by-line dissection of dnd-kit's development source and proven in a standalone Interaction Lab that is the design-system harness rather than the app. The engine ships as a single-zone core with one Pointer-Events sensor using pointer capture, rects measured once at drag start, closest-center collision with hysteresis, and a decide-then-animate drop committing on the lifted item's `transitionend`, plus a cross-list board where the lifted card renders as a fixed-position portal overlay. Constraints, auto-scroll, and keyboard phases followed, and `@dnd-kit` was uninstalled at cutover with the shared vocabulary hoisted into one module. In parallel, the standalone DesignSystem page, Interaction Lab, and glass editor became one hash-routed showcase; colors gained system-grey, white, and black primitives with everything derived at an opacity through `color-mix`; and the primitive layer grew a `GlassWindow` third glass tier, the `Menu` family, a sidebar rebuilt on it, and a motion token layer with `Reveal`.
...
Pommora ran as a native SwiftUI app from 05-13-2026, after three days of specification in which React and Electron were the stated direction. That build defined the initial paradigm over roughly a month and was archived on 07-05 at `// The Studio // Archive // Pommora`; its commits remain in this repository's own ancestry.

- **Specification and the stack pivot** (05-10 → 05-15) — the repository opened on documents alone, inverted to SwiftUI on 05-13, and built the Nexus foundation: a value type with an inline ULID generator, security-scoped bookmarks, and a persisted identity.
- **The paradigm scaffolding** (05-16 → 05-18) — every entity in the locked paradigm received a Codable type, a validator, and an observable manager, with atomic I/O primitives and CRUD end to end.
- **The editor** (05-18 → 05-22) — a WKWebView CodeMirror fork and Milkdown were each wired and abandoned before native TextKit 2 shipped, carrying horizontal rules, styler-driven lists, folder adoption, and a renderer-drawn blockquote.
- **The layout refactor and its undo** (05-22 → 05-23) — ParadigmV2 wrapped entity folders under `Pages/`, `Items/`, and `Agenda/`; Flat-Layout dropped those wrappers one day later, classifying by sidecar filename instead of position.
- **Properties and the SQLite index** (05-23 → 05-27) — eleven lettered phases delivered the property system, a per-nexus GRDB index, and a working schema editor. A third Pages-side tier was built and reverted inside the same cycle.
- **Relations unified** (05-28 → 05-31) — tier tagging and relation properties collapsed onto one pipeline, a native icon picker replaced its third-party dependency, and a manager de-duplication removed roughly 590 lines with no behavior change.
- **Items built, then deleted** (06-01 → 06-10) — Items converted to Markdown, gained a template engine and a floating window across two rounds, and were removed outright nine days later. Item-named Swift files went from 75 to one in a single evening.
- **Late Swift** (06-10 → 06-19) — the three context tiers decoupled, Sets joined as a third operational tier, and a 19-task Views cluster shipped after its hand-built table was abandoned and salvaged onto `NSOutlineView`. An FSEvents watcher and a Nexus header closed it.
