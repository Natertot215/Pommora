...
The schema cascades gained what the Context rename already had: a crash record. A property rename commits the registry and then sweeps pages; a delete snapshots, strips, unassigns, and removes the definition; the option ops move one value across every holder — and none of them recorded that the sweep was owed, so a crash partway stranded half the pages against the registry, permanently and silently. Now every op that writes to both the registry and pages states its intent in `.nexus/property-cascade.json` before the work and deletes it after, and a surviving record replays at the next open — forward-completing the interrupted cascade against current disk. The arc closed the abstract-plumbing sequence: live tree, content index, crash journal.
...
**The Closing Pass:** The parent arc's final simplification round preceded the journal and yielded two real defects beyond its nine folds — the rename bookkeeping's case-sensitive `.md` check diverging from the walk's case-insensitive admit, and the per-folder corpus enumeration paying a whole-nexus readdir on every container open, fixed with a subtree-scoped form of the one corpus law.
...
The minimal render over the Part 1 seam became an actual table. Nexus label tiers became first-class `LabelPairs`, feeding a renderer-side column-label resolver and a render resolution context so cells could resolve contexts and chip colours. The shell landed with resolved names, a single header, and grouped rows; then type-aware cells over `Chip` and `ContextChip`, disclosure group headers with chevron and glyphs, a Compact density with column lines, full-width grid fill, column reorder through PommoraDND with a ghosted drag, zoom-aware clamped column resize, and column hide behind a header right-click menu with a collapse animation. A per-machine `viewOrders` cache joined as the lowest-priority sort tiebreaker, and row drag arrived through PommoraDND — gated off under multi-key sort, restricted to views whose orders are correct, and extended so a cross-group drop reassigns the grouped property.
