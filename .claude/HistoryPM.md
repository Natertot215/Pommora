...
The glass underneath it was named wrong. `GlassWindow` and `GlassSurface` were byte-identical, each carrying a comment promising divergence later, so three named tiers were really two and the one real distinction — 95% against 90% brightness — was invisible in the names. They merged under `GlassSurface` for the app's fixed chrome, and `GlassWindow` was rebuilt as the pane's chrome carrying a body, which is what a floating window needs and a menu does not. Four spellings of the same instruction — a boolean at 100%, `tintOpacity` at 85 and 90, `FrostParams.fill` at 78 — collapsed onto `SOLID_FILL`, ending a mismatch `NavWindow` and `PreviewWindow` carried while a comment between them asserted they matched. The material later gained `--glass-outline`, the tinted edge a tier wears while its surface is being driven, painted twice because a tint on a one-pixel border reads far fainter than the same colour on the thicker borders tiles wear.

**Pickers & Menus:** Rows split by what they hold rather than which shell they sit in. A fixed option set takes `MenuOption` — a leading label with a trailing accent mark on the chosen row, the mark slot laid out on every row so the pane cannot resize as the selection moves — while a user-authored value keeps `PickerOption`'s chip, whose fill already reads as chosen. The hover and keyboard-focus recipe had been stated twice and became `rowShell`; the chosen-row mark had four definitions and became `chosenMark`; `PickerControl` moved fourteen consumers at once by repointing its single menu. Rows then converged on two tiers: a menu row reads body text, a picker's rows the control ramp, stated once as a variable the picker's pane sets and every row inherits, which retired the block menu's hand-patched override. A pane's pinned header and footer became `ActionRow`, a full ramp under the rows they frame, replacing a density zoom that scaled a bar's type and glyphs together from inside one surface's stylesheet — and `PickerMenu` gained the header and footer slots that made those bars available to any picker. Its motion was half-finished: the Bloom opened from half scale and withdrew by eight percent, which reads as a cut, and seven surfaces mounted their picker conditionally with the pane hardcoded open, so a dismissal tore it out before it could retract at all. The pane now holds its last rows through the exit, since a caller's children are built from the state that opened it and closing clears that state in the same tick. The wikilink autocomplete rides the same shell, centred on the caret and sliding within the editor's nearest scrolling ancestor rather than the viewport — the detail pane in the main window, a floating window's body inside one, a tile's own box on a dashboard, none of the three named — which is what `anchorHeight` and `bounds` were added for.

**Native Option:** Menus that are plain lists can be drawn by the operating system instead, under a preference belonging to the machine rather than to the Nexus: how a menu is drawn is a property of the system in front of the user, so it lives in `nexus.db` and travels nowhere. The row shape ten shared modules had each restated became `ActionItem`, one generic model carrying a label, an action, a divider, a refusal, a confirm gate, a mark and a nested list — enough for either renderer, so a model no longer knows which one will draw it. Two vocabularies resolved against the code rather than by preference: `destructive` only ever pushed a separator, since main keys its dialog off the action string, and `confirm` already existed on the option menu, which is where the gate honestly belongs. A single channel serves every list menu, because the rows are the menu and a per-surface channel would carry nothing a row does not already say. Placement converts in exactly one place: a trigger's box crosses in renderer pixels and is scaled by the window's zoom before it becomes a popup coordinate, which is the difference between a menu hanging off its control and one landing half a control away from it. The fixed-option control routes all fourteen of its call sites through the preference at once, and the surface tile's menu reads the same model as its pane — its drills become submenus, its scale a marked set, and its source page's name an inert header, the one part with no system equivalent. Icons do not survive the crossing, which is the honest cost: a system menu draws its own marks, and the row in force reads as a checkmark instead.
