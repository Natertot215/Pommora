...
**The Editor's Duplication, And What It Was Hiding:** MarkdownPM derived the same document seven times over. The line split, the fence pass, the table pass, the block constructs and the per-line block predicates were each computed where they were needed, and the block resolver held a second full copy of all of it — which every grip hover and every drag frame paid for. One scan carries them all now, cached per document version, and the block resolver, the drag models, the table widget, the merge guard and the input keymap read it. Two second doors onto that derivation closed with it. The defect underneath: a fence still being typed claims every line to the document's end, so a viewport inside one had no line above where a token slice could safely resume — the walk ran off the end of the line table and returned an index with no position, which tokenized the whole document and shifted every token to NaN. The line table carries the document's end past its last line, and the empty slice that names is the right answer.

Four box constructs stated one geometry four times; they keep their own measures and project them onto one base, with the callout's border staying where it is because a quote nested inside it already spends that line's second pseudo-element. A code block nested in a box regained the head and foot padding it always specified — its zero gap was written without a unit, which voided the calc that added them. Both link syntaxes shared a gesture grammar written twice, and the claim on a press is derived from what there is to follow rather than restated: a link whose target names nothing is no longer claimed, so a caret seats in the label that needs repairing and the click cannot reach the opener with a malformed address. Where a link leads and what it previews came out as two editor-free functions, so a resting table cell reads the same answers the body does — its external links follow, preview and dismiss now — and an entered cell gained the wikilink path it was missing.

Both relocate drags snap to the nearest of two boundaries per candidate, in one function rather than two, and the block drag's candidate loop scopes to what is rendered. A table cell escaped every backslash it held rather than only the ones carrying a pipe, so a cell written by hand as `a \* b` came back out doubled for every other reader; and a row short of the columns its delimiter declares had a cell with a position in the model and no span in the source, so the typing vanished the moment the cell demoted. Both are fixed at the cause, and the round trip is pinned in the direction that was missing. `clamp` had five definitions and now has one.

Six stylesheet rules said the same thing twice — six heading weights, two grip anchors, one hanging numeric zone, a tone shared with two neighbours, and a code block declared twice forty-eight lines apart. Output is byte-identical, proved by rendering every specimen before and after rather than by a green suite. Finally, a fold became a region: a kind and the line its chevron sits on, with each kind saying where its regions are and what key persists one — the seam a document's footnotes section needs, and the reason three fold defects closed on the way. The clone of a folded body now rides its entry rather than a map beside it, which ends both the empty reveal after an edit above a collapsed section and the collision between two editors sharing one offset-keyed store.

- **Commits:** `d9765a55` · `b13af3de` · `9e47c1e5` · `3128133f` · `8a71f0d3` · `8b77d551` · `a3cd989e` · `c8c8cf3d` · `4532fc8e` · `f0c94fe0` · `8e5b2e7c` · `30c4fdc7` · `9fd6da98` · `ab6ac262` · `9c1ecf3d` · `6d047973` · `aea208e7` · `22c912e0` · `f796fc65` · `6af1ab4d` · `a8988710`
- **Diff:** Net +73 | +1726 / −1653
