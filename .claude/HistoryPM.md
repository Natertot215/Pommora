...
- **Diff:** Net −44 (source, comments and tests excluded)
...
**Surfaces:** sidebar rows, page tabs, nav-view rows, card rows, and table rows all arm through the facade — the three ghostCreate surfaces read `e.shiftKey` so Shift arbitrates with the create-ghost, while tabs and nav rows preview on plain hover. Trash and history raise nothing. An open preview stands the create-ghost down through `glanceShown`, and a right-click cancels a pending dwell so a preview never clobbers a context menu.
