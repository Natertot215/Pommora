...
**Mechanism Contracts:** `releaseRename`'s verdict became temporal — a survivor claimed before the release abandons the session, and only a claim minted after it, a remount, keeps it — and `createFromMenu` took the gesture's host from its sidebar and subfield call sites. `useGhostAnchor` returns identity-stable handlers, which retired the ref-and-memo stabilizers in `CardsView`, `TableView`, and `Sidebar`; it also gained a travel hold (entering a row-mate holds a standing ghost, the hold's expiry arming that anchor's own dwell), an exit watchdog that clears a `closing` ghost whose consumer never reports `closed()`, timer clearing in `take()`, and a menu count in place of the boolean. `GhostRow`'s New Page glyph moved from the first track cell to the title column's, `Reveal` took an `enterOnMount` prop in place of both ghost chromes' hand-rolled mount effects, and `CardsOptions`' leading separator moved to the ViewSettings flat door, which un-doubled the Layout leaf's header divider.
