...
**The Stale Slot:** Three surfaces could aim a drop at geometry that had moved — the grouping pane never invalidated its activation snapshot inside its scroll-capped region, the GFM table's wrap-relative slot math kept a press-frozen origin through editor scrolls, and the table row drag re-measured on scroll but not on a mid-drag rows push. The review over those fixes then found the class surviving on the path no test drove: an invalidation followed by a release with no further move still committed the old slot. The invariant landed everywhere as *resolve fresh before commit* — an invalidating event re-aims from the last pointer point and the drop consults the dirty flag before reading its slot — with release-without-move regressions pinned red-first on every surface, and the grouping callers memoized so the hook's own per-move re-renders stopped false-dirtying the cache they exist to serve.

**The Skeleton:** `gesture.ts` gained the guards its growing consumer count demanded: a throwing callback aborts its own gesture instead of committing a half-activated drop or stranding the module lock, a foreign pointer's events are ignored, and a lost release — window blur, a zero-buttons move — cancels rather than dragging a phantom press. The spec grew `onWindowScroll` with a `scrollTarget` relevance guard, so scroll-sensitive geometry rides the gesture's own listener set instead of six hand-paired window listeners, and the skeleton took the click rule whole: every activated release swallows its click, uniformly. The test harness holds the lock honest with a fresh module per test, and a release with no button held is now the pointer harness's default for up events.

**The Migrations:** The four lifecycles still hand-rolling the skeleton — the sidebar tree, both reorder hooks, and the table's column drag — became `usePointerGesture` consumers, deleting the pending/active machines, the window trios, the hand-rolled capture-phase Escapes, and the unmount cleanups whose remove-by-closure-identity leaked the scroll listener the historical tests counted as balanced. The column drag gained the Escape abort it never had and measures its geometry at activation, the column-width grip joined its GFM sibling with a cancel that reverts the exact pre-drag override instead of committing the scrub, and the Slider learned to cancel through a synchronous flag with an unmount reassert through its own `onInput` channel. The boundary went into the record as two deliberate families: window-listener drags on the skeleton, element-capture scrub controls self-cleaning by design.

**One Owner Each:** `useDragSnapshot` took the measure-once ritual for all seven insertion-line surfaces, the card engine's row model and column model went behind an identity-keyed cache the wholesale rects replacement invalidates for free, the floating ghost's three inline copies collapsed onto `DragGhost`, the sidebar's hand-frozen line literals moved onto the shared drop-line classes and tokens, the duplicated two-region scan became `regionScan` with both panes' policies untouched, and the ghost offset and editable-target core each got one spelling.

**The Adoption Gaps:** Five drags trapped inside real scrollers — options, statuses, the grouping pane, table columns along x, and the GFM table axis-matched — gained the edge auto-scroll loop, every product drag announces its pickup and drop with the engine's pointer path gaining the words its keyboard path always had, and spring-open reached its last three surfaces: the band drag brackets the service whose targets were already registered, and the grouping hierarchy and the sidebar's collapsed containers register their rows so a dwelling drag opens them, the disclose remeasure re-aiming against the rows the spring just moved.
