...
**Pickers & Menus:** With one shell in place, the rows inside it converged on two tiers. A menu row reads body text at the primary tone; a picker's rows read the control ramp, because they are a control's options rather than a menu's commands — stated once as a variable the picker's pane sets and every row inherits, which retired the block menu's hand-patched override and the tone it had been forcing onto eleven rows. A pane's pinned header and footer became `ActionRow`, a full ramp under the rows they frame at the secondary tone, replacing a density zoom that had scaled a bar's type and glyphs together from inside one surface's stylesheet; `PickerMenu` gained the header and footer slots that made those bars available to any picker. The retract deepened, since the Bloom opened from half scale and withdrew by eight percent, which reads as a cut rather than a withdrawal. Seven surfaces had been mounting their picker conditionally with the pane hardcoded open, so a dismissal tore the pane out before it could retract at all; `PickerMenu` now holds its last rows through the exit, which is what lets a caller clear the state that built them in the same tick. The glass gained `--glass-outline`, a tinted edge any tier can wear while it is being acted on, painted twice — once as the border, once inward — since a tint on a one-pixel border reads far fainter than the same colour on the thicker borders tiles wear, and widening the border would shift everything inside it.

**Native Option:** Menus that are plain lists can be drawn by the operating system instead, under a preference that belongs to the machine rather than to the Nexus: how a menu is drawn is a property of the system in front of the user, so it lives in `nexus.db` and travels nowhere. The row shape ten shared modules had each restated became `ActionItem`, one generic model carrying a label, an action, a divider, a refusal, a confirm gate, a mark and a nested list — enough for either renderer, so a model no longer knows which one will draw it. A single channel serves every list menu, since the rows are the menu and a per-surface channel would carry nothing a row does not already say. Placement converts in exactly one place: a trigger's box crosses in renderer pixels and is scaled by the window's zoom before it becomes a popup coordinate, which is the difference between a menu hanging off its control and one landing half a control away from it. The fixed-option control routes all fourteen of its call sites through the preference at once, and the surface tile's menu reads the same model as its pane — its drills become submenus, its scale a marked set, and its source page's name an inert header, the one part with no system equivalent. Icons do not survive the crossing, which is the honest cost: a system menu draws its own marks, and the row in force reads as a checkmark instead.
