...
3. Files and Code Sections:
   ...
   - `design-system/components/Slider.tsx` + `Slider.test.tsx` — `scrubbing` ref set in onPointerDown, cleared in onPointerUp; `revertRef`/`revertScrub`; unmount reassert via `onInput?.(clamp(value))`.
...
I want you to scope out a complete reconnaissance that would rewrite DesignPM, while also potentially retiring the guidelines. This document would rename DesignPM into PommoraUIX.
...
#### The PommoraUIX Reconnaissance — Scope

**What it becomes:** `DesignPM.md` renames to `PommoraUIX.md` and gains a complete token atlas — the existing narrative (materials, menu surfaces, primitives-and-aliases philosophy) stays as prose, and beneath it every family gets your H4 + table treatment: `Title | token | value`, raw hex where one exists, derivation notation where the value is derived (`system-white @ 65%`), and the CSS-var bridge name where one is emitted.
...
Once the feature-pocket agent reports (MarkdownPM's editor tokens, Carets, table-tokens, SurfacePM, the scope-hazard hunt), I write `PommoraUIX.md` with per-table **SOURCE:** lines, retire Design-Sources and the false README, amend the CLAUDE.md hard-rule exception, and wire the checker script.
...
Here's everything that exists, grouped by *type* — mark up what belongs in PommoraUIX and what doesn't:
...
My recommendation is PommoraUIX follows
...
Switches, Toggles, Chevrons, etc. get described in PommoraUIX.md rather than fully recited.

[CLAUDE]

Routing locked — the atlas distributes to where each family's consumers actually live, PommoraUIX keeps the core vocabulary, and the bridge names fold into each table as a column instead of a standalone section. Executing:

- BASH: Rename DesignPM to PommoraUIX
- WRITE: [~/The Studio/Projects/Project Pommora/.claude/Features/PommoraUIX.md]
...
- **[PommoraUIX.md](.claude/Features/PommoraUIX.md)** (renamed from DesignPM) — the charter naming itself the sanctioned docs-hold-exacts exception, the narrative carried over, and the core atlas: Primitives, Surfaces, Labels, States, Fills, Tints, Separators, Shadows, Spectrum, Geometry, Materials — every row `Title | token · --var | value`, every table under a **SOURCE:** line. Component chrome (switches, chevrons, grips, ActionBand, the dropdown shell) is described, not recited, per your cut.
...
- READ: [~/The Studio/Projects/Project Pommora/.claude/Features/PommoraUIX.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/ViewsPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/TableViewPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/CardViewPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/TypographyPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/SymbolsPM.md]
- WRITE: [~/The Studio/Projects/Project Pommora/.claude/Features/PommoraUIX.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/ViewsPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/TableViewPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/CardViewPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/TypographyPM.md] -> [~/The Studio/Projects/Project Pommora/.claude/Features/SymbolsPM.md]
...
- RETURN: The first residue reviewer's report — dead-weight and mechanism-narration prose flagged for removal across the rewritten Feature docs, including a lost PommoraUIX table of contents and dangling section pointers.

First residue report is in — the big catches: PommoraUIX lost its ToC (my script inserted it, but an outside save appears to have overwritten it), the `§charter` pointer dangles in five docs since the charter paragraph carries no heading, SurfacePM still points at the renamed `§II. Embeddings`, and NexusRecordPM references PropertiesPM's Remove cache with no antecedent. Several "residue" flags I'll adjudicate rather than fold blindly — a few hit things deliberately kept (the dnd-kit scope framing, philosophy copy). Verifying everything firsthand once the second reviewer lands.
...
- BASH: Apply the fold + capture edits with per-edit verification -> Check the current state of the three failed targets -> Verify which PommoraUIX edits landed -> Add the Pill Radius atlas row and gate
...
- **The ToC experiment** — every doc opens on a codemap-format tree of its own headings, script-derived so none drift. PommoraUIX's got overwritten once by an outside save and was re-added.
