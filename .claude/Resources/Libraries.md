...
- **PommoraDND** — the **in-house drag-and-drop engine** (behind the `interactions/drag.tsx` seam): measure-once, no mid-drag array churn, pointer-capture single sensor, closest-centre + hysteresis, decide-then-animate; constraints, auto-scroll, keyboard + ARIA. **Decided + shipped** — built, reviewed, and Lab-approved (06-18-2026); replaced `@dnd-kit` entirely. Spec → `Features/PommoraDND.md`.
- **@dnd-kit** — the reference engine PommoraDND was dissected from. **Replaced + uninstalled** (06-18-2026) — no longer a dependency or import. Kept here only as a historical anchor for the dissection.
