...
Two things followed from the removal. New nexuses are minted at the current schema version instead of the old one and relying on a migration to catch up, and `reconcileWriteRoot` — a wrapper whose only outside consumer was the migration — collapsed into the shared reconcile it had been forwarding to. The reserved `_tier1`/`_tier2`/`_tier3` ids stay: they are the seeded Contexts' live identifiers, referenced by saved views and space orders, not legacy residue.
