...
**Nothing is mid-flight.** One thing wants an eye before the next thing starts: every gate is green and the disk state is confirmed, but nothing was rendered and looked at, so a table drawing the new keys is asserted rather than seen. Past that the next focus is open, and the list below is what came to mind rather than a mandate — if something else matters more, take that instead.
...
- **Look at the converted nexus in the running app.** The syntax change is green on every gate and correct on disk, but no one has watched a table render the new keys. Cheap, and it's the only part of that work taken on trust.
