...
**The two audits run beside it.** The architecture-audit cleanup ([[Architecture Audit — Full-Codebase Report]], session-sized work at [[Codebase-Cleanup-Checklist]]) landed bundles 1–3 on 08-21 with 6a → 6b next, and runs its structural bundles through before design work joins it — with one standing constraint: `ColorPicker` moves into the design system before the `Components/Detail` rehome. The design audit ([[Design-Coherence-Report]]) has landed its reorganization (08-24): `DesignSystem/` mirrors [[DesignSystemPM]] one folder per section, motion lives in `Animation/`, only `Tokens/` and `Theming/` reach `@shared`, and `Components/Controls/Button/` is the one button — five types, four sizes (`button-inline` for row affordances), `Segmented` as its run — worn by every toolbar button and every former ghost; ActionBand keeps its own tab-style segment. [[Buttons-Spec]] holds the recipe; the Figma parity pass and an eyeball of the swept row affordances are next.
