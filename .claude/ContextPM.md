...
**The two audits run beside it.** The architecture-audit cleanup ([[Architecture Audit — Full-Codebase Report]], session-sized work at [[Codebase-Cleanup-Checklist]]) landed bundles 1–3 on 08-21 with 6a → 6b next, and runs its structural bundles through before design work joins it — with one standing constraint: `ColorPicker` moves into the design system before the `Components/Detail` rehome. The design audit ([[Design-Coherence-Report]]) has landed its first focused arm: `DesignSystem/Components/Fields/` holds the input, search, rename and path fields on one chrome — `InputField`, one `bare` reset, one transparent search look, the `--field-ring` channel with rest/focus/error presets, and the editing behaviors beside it — with `--input-field` gone; the report's remaining slices (the boundary lint rule next, the root pile, the reference rewrite, the twin extraction) queue behind it.
