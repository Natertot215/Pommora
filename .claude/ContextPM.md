...
- [ ] **Where do link / connection actions and surfaces go in `/renderer`?** The hover pane (`ConnectionPane` + its presenter and size) is a floating surface serving page and webpage hovers alike and may split into a PagePane and a WebPane; the right-click menu (`connectionMenu`) and the click resolvers (root `linkResolve`, `openWebLink`) are link actions with no UI. `Embeds/` dissolves under the Tiles work, so all four need a home — `Windows/` for the pane, `Connections/` or `Core/` for the actions — decided together, not by default.
...
- [ ] **The remaining filing rows** — `Detail` → `Interface`, `Core/`, `Navigation/` absorbing `Tabs/`, the Showcase out, the casing renames — each a session or a few folded into one. `SurfacePM` keeps its name; `Blocks/` becomes `Tiles/` under [[Tiles — Decision Log]].
