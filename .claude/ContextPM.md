...
**Compatible Properties landed 09-01-2026.** Property values are written under bare frontmatter keys named exactly as the registered property, Context keys as `<Title>:`, and Select/Status values as one-element lists, so a page reads the same in Pommora and in any other frontmatter editor; ownership of a key is the registry name, a reserved-name rule keeps Pommora's own keys out of the namespace, and one reconcile (`reconcileGovernedRoot`) repairs shape, adopts unknown Multi-Select options, and settles Context titles on every governed write, every restore, and the opt-in on-open sweep. Values reach an open view live through `values:changed`, fed by the watcher and by every main-side writer, with overrides retired by page id. Two Settings toggles sit in the Properties leaf's Metadata section. The NexusOS vault was converted in place (backup beside it); five Ideas pages still hold `Status: Open`, a value outside the registered six, and read as no value until Nathan maps them. The sustainability judge answered for the pre-arc tree; its reasoning sits in the plan's Closeout.
...
- [ ] **Map `Status: Open` on the five Ideas pages** to one of the six registered options (or register Open) — until then they read as no value.
- [ ] **Answer the Projects `.slate` question** — the vault pass kept the bare `Projects:` key beside the new `<Projects>:`; delete or keep is Nathan's call.
