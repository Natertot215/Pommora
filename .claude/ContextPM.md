...
- [ ] **The in-app two-host lost update.** Two editors holding one page — the content pane and the Page Window, or a page and its embed — each save their own body with no lock between them, so the later keystroke writes over the earlier host's text; a keystroke landing between a restore's pre-flush and main's write re-saves the old body the same way. The watcher-driven reload through `replaceBody` is the mechanism that closes it.
