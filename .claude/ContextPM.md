...
**MarkdownPM's cleanup is finished, and it is the Footnotes greenlight.** Eight phases against [[MarkdownPM-Plan]] closed six live defects and the duplication each of them sat inside — the editor derived the same document seven times over, both link syntaxes spelled one gesture grammar twice, four box constructs stated one geometry four times, and a fold was a heading offset rather than a region. Every seam the [[Footnotes — Decision Log]] named as a prerequisite is now in place: the fold model takes a kind, the document scan is one derivation a footnotes scan can join, the pointer path is a factory a marker registers with, the block resolver reads that scan, and the two device-row holes footnotes would have inherited are closed. What is left of the MarkdownPM arc is four items the plan deferred *because* footnotes is what makes them worth doing, and they are the immediate work.
...
- [ ] Finish the MarkdownPM cleanup arc — the four items [[MarkdownPM-Plan]] deferred because footnotes is what makes them worth doing: `setList`/`setListKind`, `embedWidget.tsx`'s split, the three `Embeds/` import cycles, and `PageHeader`'s threaded props. The last carries a design question and needs Nathan before it needs code.
- [ ] Fold the [[Cohesion-Audit]]'s §One Definition Per Thing easy wins — `parentOf` and the smaller twins, landing immediately before the Table session so `TableView.tsx` and `CardsView.tsx` are opened once.
- [ ] MarkdownPM Footnotes → Plan & Execution. The decision log at [[Footnotes — Decision Log]] is the settled contract, one `[assumed]` entry (C-3's clear-on-default) pending Nathan's word. Every seam it named is in place, so this can run beside the cleanup arc wherever the two don't touch the same file.
...
The cohesion arc's dusting, its main-process costs, and MarkdownPM's own cleanup are closed (→ PM-110); the catalog and where every finding stands are in [[Cohesion-Audit]]. Three efforts remain, and none of them blocks Footnotes.
...
- [ ] **§II's helper deduplication.** `parentOf`, `persistViewOrder`, and move-an-item-in-an-array each exist in several spellings. Half of them live in `TableView.tsx` and `CardsView.tsx`, so this lands immediately before the Table session rather than on its own — otherwise those two files get opened twice. `clamp` is done: one definition in the design system, read by five surfaces.

- [ ] **The design-splits, parked.** The renderer-lifting and view-host arcs across Tables, Cards and their siblings are recorded here rather than built. Nothing is half-started; they wait behind Table hoisting.
