...
- [ ] **`Carried` is `unknown`.** The two tab rows cast the carried item to `PageTarget` and the family string is the only agreement on its shape; a generic `SortableZone<T>` would type it.
