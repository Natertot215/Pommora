...
**Sidebar + DND Consolidation & Bug Fixes.**
...
- The Set-Card drag flash wants one live drag to confirm it. The optimistic `moveSet` order patch is in, and the Known Issue stands until a drag proves it.

### Pending Focuses

#### The Boring Work

Architectural work with no user-visible payoff and permanent editing payoff. Nothing here is broken; each is a shape that taxes every future edit.
...
- [ ] **The tree reload's escape path.** Every change re-reads the nexus from disk with `stabilize` keeping unchanged parts identical, which is correctness by brute force and fast enough at current sizes. It is also the future ceiling — at several thousand pages, every mutation paying a full re-read becomes the felt lag. The replacement arrives with the content index, so this is a constraint handed to the Pages-in-DB session rather than a task of its own.
- [ ] **`mutate.ts` organization.** Every change funnels through one dispatcher in the file-owning process, which is deliberate: one entry point means one place for safety policy. Early operations got tidy `crud//` modules where later ones were written inline, and each arm moves as its file is next touched.

#### The Identity Arc

- [ ] **The seed slot's second writer.** Seeding runs only at nexus creation and refuses a folder name already on disk, which correctly protects a user's own `Tasks/` of notes. Filling that slot afterwards needs a second writer to the registration record plus a product call on whether to adopt the user's folder or disambiguate. Belongs to the Agenda work.
...
- [ ] NOR filters are hand-authoring only — the mode lives on disk and in the evaluator while the pane offers All and Any.
...
- [ ] Mobile iOS companion — parked, spec at `.claude/Mobile/MobileSpec.md`.
...
- [ ] On menu rows where property values are expected to be positioned horizontally rather than stacked vertically, there isn't currently a constraint on how far indented relative to its properties label itself; this makes multi-value property rows have its values land its left-side padding tight against the property label; its right-side overflow scroll is properly done, however the lack of left-side padding against the value itself makes the menues cramped. Mutliple CSS tries have been applied and reverted; a pane-width-relative max-width that these values can take on the left-side of its field needs to be figured out.
...
#### PM-089 || The Write Path Converges On One Lock (08-08)
...
#### PM-088 || Dropdown Shell & Menu Consolidation (08-08)
...
#### PM-087 || One Shared Pass Over Fenced Code (08-07 → 08-08)
...
#### PM-085 || The PageMenu And The In-App Picker (08-06 → 08-07)
...
- Nathan writes into Pending Focuses and Important Information directly; what he leaves there is authoritative rather than a draft to be reworded.
