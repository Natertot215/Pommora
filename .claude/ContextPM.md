...
- [ ] **The value-picker fold's write layer.** The options and dispatch leftovers folded: `isOptionsKind` states the options-kind test once, one type-keyed `optionsOf` replaced the two that disagreed — correcting a stale-options bug across the Settings frames — and the two context-click bypasses route through `sharedValueClickAction`. What remains sits in the write and undo layers: three value-write paths (`PropertyPanel.commitValue`, `useViewHost.setProperty`, `useViewHost.commitValue`), four number parsers, and single-cell undo that today covers only the mass path. Unifying the writers onto one assignment — abstracting the optimistic-apply step so the panel's `setFm` and the views' `setValueOverride` share it — is the seat for single-cell undo across all three surfaces; a group boundary lets a batch coalesce into one revert, dissolving `MassPropertyPicker`'s exemption. That undo reach is a feature, so this arc carries a live check rather than running unattended. Left by decision: `FilterFrame`'s `ChipsField` renders `PropertyOptionRows`' rows to write filter rules, not property values, so its commit path stays its own; the value right-click menu's per-surface arms and `openAddPicker`'s number/file routing are genuinely surface-specific.
