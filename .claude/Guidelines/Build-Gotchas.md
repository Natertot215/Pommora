...
- **`src/main` and `src/preload` need a full dev-process restart.** Neither HMR nor ⌘R reaches them, so a new native menu item, IPC channel, or main-side label change is simply absent until the dev process is restarted. The signature is a change that appears to have done nothing while the code is already correct — or, where the renderer's half DID hot-reload, a stale main answering the renderer's new request: a menu built from the old model against a new context renders neither shape and reads as a fresh regression. It has cost two rounds on a single feature and one phantom bug report.
