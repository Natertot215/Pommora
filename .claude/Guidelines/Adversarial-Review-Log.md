...
A running catalog of every way shipped work broke **after** Claude called it solid, robust, or "bulletproof." Most of it is MarkdownPM under Nathan's hands — **Callouts** and **Tables**, each fuzzed until it stopped breaking — plus one data-layer class that generalizes past the editor. Kept for two reasons: to make the lesson un-ignorable, and to seed a reusable skill whose only job is finding ways to break things.

This is the running-UI companion to the review → revise loop the brainstorming and planning skills carry — that loop governs *docs* ("never call a spec bulletproof before an adversarial review proves it"); this Log is the same failure mode at the *functional* layer.
...
### The Data Layer — Defects Live in the Seam, Not in the Mechanism

The one non-editor entry, kept because its class generalizes hardest. The content-identity subsystem shipped gated green, was walked by hand across every user-reachable path, simplified twice, and then handed to a reviewer briefed that it had been settled for a while and merely needed a last look before release. That framing is the technique: a reviewer expecting a finished implementation reports what contradicts "finished," rather than grading a work-in-progress charitably.

**Every real defect it found was in the seam between two mechanisms that were each correct alone.**

- **Re-homing reached deeper than contest detection looked.** A registered agenda folder found nested is carried back to the root — matched by id, at any depth. The check for "two folders claiming one record" counted only the root's own children. A duplicate filed inside a Collection was therefore invisible to one and fully visible to the other: it relocated itself out of the folder its owner put it in, and on the next open the two contested the slot and de-registered the real singleton. Both behaviors had passing tests. Nobody had crossed them.
- **The walk and the sweeps disagreed about what a Markdown file is.** The read walk and adoption matched the extension case-insensitively; the sweep enumerator and the watcher did not. An upper-case `.MD` was a full member of the tree — visible, adopted, holding a real id — that no sweep would ever rewrite, so its links survived renames and its property cells read empty. One fact, stated six times, in two versions.

**The tells.** Both bugs were compositions of individually-passing tests, so no single test could have caught either; both were duplications of a *fact* rather than of code, so neither looked like duplication at a glance — one was a hand-written `try/catch` whose fallback happened to equal the shared helper's empty-list path, the other a predicate spelled two ways in six places. **A fact stated twice is one edit away from disagreeing with itself, and the disagreement surfaces as a user-visible defect long before anyone thinks to read both copies.** When two passes must agree about something, the durable fix is one owner they both call — not two correct implementations.

**Method notes.** Verify every reported `file:line` before folding: of five findings, three were real, one was already an adjudicated ruling the reviewer suspected as much and asked about, and one was right in outcome but imprecise in mechanism. Reproduce a defect as a failing test *before* fixing it, and negative-control the fix by reverting it and watching the test go red — one fix here initially "failed" for a reason that was in the test fixture, not the code (a ULID containing `U`, which Crockford base32 excludes, so the file read as malformed and never reached the assertion under test).

---

### The Color Ramp — The Migration Was Safe; The New Surfaces Were Not

Ten flat chip colors became a 64-cell ramp, with every legacy key on disk normalizing onto an anchor cell so nothing needed migrating. The whole plan was written around that migration: which stored value might render differently, which indexer would return `undefined` once a new key shape was storable, which validator would reject data it used to accept. **That fear was misplaced in an instructive way.** An adversarial round computed the old and new output for all ten legacy names across every channel and found them byte-identical but for one knowingly-accepted case. Every defect it did find was in a surface the change had newly *built*.

**A comment claiming to be the single source is a claim, not a fact.** A shared helper's docstring read "one source for the link cell/editor AND the checkbox cell/editor." It was true of three of those four: the checkbox *cell* called it only when a color was set and branched to the app accent itself when one wasn't. A new control trusted the sentence, called the helper for its empty case, and inherited the *link* path's OS accent — so one unset value claimed one color in the settings pane and a different one in the table, with the picker's ring quietly claiming a third. The helper was never wrong; the sentence describing it was, and the sentence is what got read. **Before reusing a helper on the strength of what its comment claims, open its callers and check the claim holds for the branch you need** — especially the empty/default branch, which is exactly where a "shared" resolver most often turns out to be shared by only some of its consumers.

**Hiding an option can delete a gesture.** One row of the grid is withheld from the two surfaces that paint the raw color, since its dark end is the window substrate. Clearing, meanwhile, is clicking the already-selected cell — there is no separate clear affordance. The two rules were each deliberate and together they stranded any value already stored in the hidden row: still painted in the table, no longer clearable in its own picker, and showing as though nothing were selected. **When an interaction is bound to an element's presence, every filter over those elements is also a filter over that interaction.** The fix isn't to restore the row but to keep it exactly when the stored value lives there.

**A size change is a probe for latent clamps.** The picker went from about 36px wide to 181px. That surfaced a placement branch that had clamped only its near edge for as long as it had existed — its mirror branch one line above clamped both. Nothing had ever been wide enough to fall off the far side. A bug that needs a magnitude to become reachable will sit unbothered through every review that doesn't change the magnitude, so **when a component's dimensions move by an order of magnitude, re-derive the geometry it was always assumed to satisfy** rather than assuming the surrounding machinery is size-agnostic.

**Method note.** Two dispatches, not one: a neutral verifier asked only "is this claim true?", then an attacker asked to break it. The verifier caught something the attacker would have waved through as cosmetic — chips had silently stopped indirecting through their `--color-solid-*` vars, because the new token module read the raw shared constants for its chromatic anchors while reading design-system tokens for its greys. Same painted color, so nothing failed; but one module was sourcing one kind of value two ways, which is the duplication that eventually disagrees. Adjudication and attack find different classes, and merging them into one brief loses the first.

---

### File Properties — A Capability Can Ship Correct and Unreachable

The last of the ten property types, built as reconciliation rather than as new ground: nine types already shipped with their cell renderer, column style, editor pane, filter operators, sort branch and value menu, so the work was dominated by deletion. It gated green at every phase. The three defects that survived to the closing passes are all the same class — **a mechanism was built correctly and the thing that reaches it was not**.

**A sort nobody can select is not a sort.** The task named one file: add the `file` arm to `sortText`'s switch, which returned a constant for every file value. Done, tested, green — and unreachable, because the sorting pane keeps its own allow-list of sortable types and no column-header menu offers a sort at all. The engine ranked correctly and the UI never offered the column. **When a capability is added to an engine, find every gate between a user and that engine before calling it delivered** — an allow-list is a second door, and a passing engine test says nothing about whether anyone can open the first.

**A hit test that walks up needs the node that was clicked, not the node it is anchored to.** Four surfaces shared one click router, which was the whole point of the arm. Two of them passed `e.currentTarget` — the value row — where the other two passed `e.target`. `closest()` walks *up*, so from the row it could only ever answer null, and clicking a file label in either inspector pane silently ADDED a second file instead of replacing the one named. Both readings are ordinary React; the two facts are simply different, and a single parameter carrying both hid that. **When one element anchors what opens and another says what was hit, they are two arguments** — collapsing them reads as tidy and is a defect that no type gate and no test on the router itself can see.

**A prop kept for a consumer that never arrived.** A component's click prop was held through a phase gate on the reasoning that the spec named it and the next task would consume it. The next task shipped, took the click through DOM hit-testing instead, and the prop stayed — declared, forwarded through a second layer, reachable by nothing. **A ruling that defers a deletion on a prediction expires when the prediction resolves**; revisit it at that gate rather than at the end, when it reads as settled.

**Method note.** The correctness reviewer returned clean on all five questions it was given, and the simplification pass — briefed only to find duplication and dead code — is what surfaced the pane hit-test defect, by noticing that a comment claimed something the call sites defeated. **A comment that describes behavior is a testable claim**, and reading it against its callers found what an inspection aimed at correctness had already passed over.

**The verification round found two more of the same class, and one of them was inside the fix that closed the last one.** A helper written for the panes' value menu answered a boolean its docstring said a caller would fall through on — and both callers decided the type before calling, discarding the promise, because a context-menu handler must answer `preventDefault` synchronously and nothing async can route for it. A test pinned the dead arm, which is what kept it reading as live. Alongside it, the component's `unresolved` prop was dead in exactly the way its `onClick` sibling had been: deleted once by name, never re-censused, so the twin survived one layer down. **When a dead interface is removed, census the rest of that component's interface in the same pass** — the reason one prop went dead is rarely unique to it.

**The showcase is a consumer, and a shape that never lands there is a shape nobody can find.** Two chip shapes shipped, documented in the atlas's own chip table, and appeared nowhere on the deployed component library — where a developer looks for the house rendering of a named file before writing their own. That is the drift a design system exists to prevent, arriving through the one surface a feature's Made False table never lists. **A new design-system primitive is not delivered until the roster that advertises it carries it.**

**Containment is not reachability.** A picked file inside the asset root, in a folder the index never walks, passed every boundary check and minted a reference that resolved to nothing forever. Two prior passes had verified the boundary itself and both were right; the question was simply the wrong one. **Where a path is admitted because it is inside a root, ask the indexer too** — "may it be written here" and "will anything ever find it" are separate predicates, and the second one fails silently.
