...
const REG = { agenda: { tasks: TASKS, events: EVENTS }, sidecarMode: true }
...
describe('resolveFolderKind', () => {
  ...
  it('classifies a sidecar-less root folder as a Collection in raw mode', async () => {
    const d = await dir('Stray')
    expect(await resolveFolderKind(d, 'root', { agenda: {}, sidecarMode: false })).toBe('collection')
  })
  ...
  it('still refuses an agenda config in raw mode when nothing registers it', async () => {
    ...
    expect(await resolveFolderKind(d, 'root', { agenda: {}, sidecarMode: false })).toBe('unknown')
  })
})
