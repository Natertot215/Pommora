// The one-time tierN → registry migration. Idempotent and resumable: every step
// re-runs safely (folder moves skip the already-moved, rewrites skip the already-
// bracketed) and the version bump commits LAST, so a crash mid-run re-runs the
// remainder on the next open instead of sealing a partial state. No verification
// re-scan gates the bump — a straggler file isn't data loss: legacy tierN stays
// read-recognized and heals on that file's next governed write.

import { mkdir, readdir, rename, rm } from 'node:fs/promises'
import { join } from 'node:path'
import { contextKey, seededRegistry } from '@shared/contexts'
import { LEGACY_CHIP_COLOR_MAP } from '@shared/types'
import { mutateJson, pathExists, readJsonObject, writeJson } from './io/atomicWrite'
...
export async function migrateContexts(root: string): Promise<void> {
  ...
  let hasTiers = false
  for (const t of TIERS) if (await pathExists(contextTierDir(root, t.dir))) hasTiers = true
  ...
  if (hasTiers) {
    ...
    for (const t of TIERS) {
      ...
      for (const e of await readdir(parent, { withFileTypes: true })) {
        ...
        const raw = await readJsonObject(oldSidecar)
        if (raw && !(await pathExists(newSidecar))) {
          const next = { ...raw }
          delete next.tier
          const mapped =
        ...
        if (await pathExists(oldSidecar)) await rm(oldSidecar, { force: true })
      }
    }
  ...
  // 7. Version bump LAST — a crash anywhere above re-runs the remainder on reopen.
