 ...
 *  the codec. */
export function coerceTileDoc(raw: Record<string, unknown>): TileDoc {
  return {
    layout: raw.layout,
...
// A corrupt document is adjudicated here, by its one writer under the lock: the bytes move aside
...
const quarantine = async (absPath: string): Promise<void> => {
  await rename(absPath, `${absPath}.bad-${newId()}`)
}
