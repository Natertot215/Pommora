// Per-nexus index open with the version handshake: an existing index at the current
// SCHEMA_VERSION is reused; any other version (or a corrupt/unreadable/absent one) is deleted
// and recreated fresh, signalling needsRebuild so the caller runs a cold build then stamps.
// Returns null only when even a fresh DB can't be opened — the caller degrades to file-only reads.
