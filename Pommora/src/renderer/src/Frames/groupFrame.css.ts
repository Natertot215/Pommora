// The Grouping frame's row-tier knobs. Primary rows (Group By / Sub-Group / Date By) read the
 ...
 *  LARGER than the menus' Footnote detail so the frame's lead value reads at full strength. */
export const groupByValue = style([
  text.control.standard,
  { color: c.label.control, display: 'inline-flex', alignItems: 'center', gap: '4px' },
])

/** Scope class for the frame's rows: every grouping picker's value reads a step brighter than the
...
/** The frame's own gutter for the shared `drop-line` — wider than the token default so the
