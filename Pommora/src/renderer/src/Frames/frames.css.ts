...
const COLOR = {
  ...
  iconHover: c.state.hover, // the shared fill behind any frame icon-button on hover (not a glyph shift)
...
/** — ICONS — which ladder step each glyph in this frame names, consumed by PropertyFrame/ViewFrame
 *  TSX. The TopRow's own ‹ chevron is the shared MenuTopRow's — not a frame-local knob. */
export const ICON = {
 ...
 *  the menu's own beak tip owns the Bloom origin. */
...
// (assigned rows carry no frame-local style — they're menu.css items + flushTrailing)
// ═══════════════════════════════════════════════════════════════════════════

/** The elastic gap above the All Properties block: closed it absorbs the frame floor's slack
 *  (the block reads bottom-pinned); open it collapses on the frame's beat, so the heading RISES
...
export const allHeadingRow = style({
  // The frame's beat, not the disclosure beat: this row's chevron, its Reveal unfold and the elastic
...
/** The hidden zone sits directly below the shown rows and grows into the frame's slack (rows
 ...
 *  Switches' on-track tints to it — that frame only. */
 ...
 *  frame sits at control scale, but a label is what the row says, so it reads at full strength while
 *  its glyph and its value stay a step under. */
export const configLabel = style([text.control.emphasized, { color: c.label.primary }])

/** The scoped-frame footer breadcrumb — the embed's source path, `(icon) Collection › (icon) Set`;
