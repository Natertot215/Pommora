...
globalStyle(`${body} .cards-view .group-band-row`, {
  paddingLeft: `calc(var(--inset-detail) / (var(--zoom, 1) * var(--block-zoom, 1)) - (12px + var(--cell-icon-gap, 6px)))`,
})
...
globalStyle(`${body} .col-header:first-child::before`, {
  ...
  left: `calc((${HEAD_PAD_L} / (var(--zoom, 1) * var(--block-zoom, 1))) - var(--inset-detail))`,
})
