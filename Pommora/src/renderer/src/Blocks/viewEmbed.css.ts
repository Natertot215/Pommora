...
// The table gutter (row grips + group chevrons strip) resolves from the root --detail-inset token,
...
globalStyle(`${body} .cards-view .group-band-row`, {
  paddingLeft: `calc(var(--detail-inset) / (var(--zoom, 1) * var(--block-zoom, 1)) - (12px + var(--cell-icon-gap, 6px)))`,
})
 ...
 *  it sits --detail-inset in — but the embed header insets at HEAD_PAD_L. Pull ONLY the cap out to the
...
globalStyle(`${body} .col-header:first-child::before`, {
  ...
  left: `calc((${HEAD_PAD_L} / (var(--zoom, 1) * var(--block-zoom, 1))) - var(--detail-inset))`,
})
