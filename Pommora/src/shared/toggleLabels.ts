...
//
// Every one of these reads in more than one place — a native menu row, an in-app menu model, and
// the button or aria-label for the same act — and each site would otherwise carry its own ternary
// over its own predicate. Two of them already had: one spelled the icon toggle over `iconShown`
// and the other over `iconHidden`, which agree only for as long as nobody edits one of them.
//
