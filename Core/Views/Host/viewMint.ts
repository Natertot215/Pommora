// `ensureContainerView` is the SOLE place a container's default view is born (an in-flight map keyed by container id guards a re-select from double-firing); every other writer routes through `saveViewAdopting`, which awaits the in-flight mint rather than minting its own.
...
export async function saveViewAdopting(
  ...
  if (view.id === DEFAULT_VIEW_ID) {
    const minted = await inFlight.get(source.id)?.catch(() => undefined)
