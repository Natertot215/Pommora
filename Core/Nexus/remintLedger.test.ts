...
describe('latchBaseline', () => {
  ...
  it('a duplicated id keeps the prior entry while its path still claims', () => {
    const prior: Baseline = {
      'page-dup': {
        id: 'page-dup',
        kind: 'page',
  ...
  it('an unusable registry carries every prior context and space through', () => {
    const prior: Baseline = {
      'ctx-areas': {
        id: 'ctx-areas',
        kind: 'context',
...
describe('the record rows', () => {
  ...
  it('round-trips the baseline', () => {
    const b: Baseline = { 'page-a': entry }
...
describe('runOpenLedger — the open sequence', () => {
