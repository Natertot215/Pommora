...
describe('readNexus — the asset root leaves the tree and the corpus together', () => {
  ...
  const build = (asset_directory?: string): string => {
    ...
    for (const folder of ['file-assets', 'Archive', '_drafts', 'Notes']) {
      mkdirSync(join(root, folder), { recursive: true })
      writeFileSync(join(root, folder, '_pagecollection.json'), JSON.stringify({ id: folder }))
      writeFileSync(join(root, folder, 'note.md'), 'text\n')
    }
    return root
  }
