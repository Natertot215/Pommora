...
quadtree hit-testing — and then written from scratch: **no code was copied from any of
them**. Two requirements drove the loop's own shape rather than a library's:
...
- **A local reheat.** A Page created elsewhere joins at its Folder's centroid while the rest
  of the picture holds still, which needs the moving set to be a first-class input to the
  wake rather than a global alpha bump.
...
| `matrixGraph.ts` | The reply shape of `matrix:graph` |
| `matrixConfig.ts` | The four sections, their patch, and the per-key merge |
| `matrixFile.ts`, `handlers.ts` | `.nexus/matrix.json` and the channels over it |
