...
describe('normalizeTitle as the Context value normalizer', () => {
  it('string-coerces YAML scalars', () => {
    expect(normalizeTitle(2024)).toBe('2024')
    expect(normalizeTitle(true)).toBe('true')
  })

  it('trims, lowercases, and NFC-normalizes', () => {
    const nfd = 'Café'
    expect(normalizeTitle(nfd)).toBe('café')
    expect(normalizeTitle('  Pommora ')).toBe('pommora')
  })
})
