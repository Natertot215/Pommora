...
describe('readPageDetail', () => {
  it('reads frontmatter + body, derives title + path from the rel path', async () => {
    const p = await readPageDetail(root, 'Vault A/Collection A/Page A.md')
  ...
  it('adopts an id and returns whole file as body when no frontmatter', async () => {
    const p = await readPageDetail(root, 'Vault A/Collection A/Plain.md')
  ...
  it('handles an empty file', async () => {
    const p = await readPageDetail(root, 'Vault A/Collection A/Empty.md')
    expect(p.frontmatter).toEqual({})
    expect(p.body).toBe('')
  })

  it('treats an unterminated fence as all-body', async () => {
    const p = await readPageDetail(root, 'Vault A/Collection A/Unterminated.md')
    expect(p.frontmatter).toEqual({})
    expect(p.body).toBe('---\nid: x\nno close fence')
  })

  it('throws when the file does not exist', async () => {
    await expect(readPageDetail(root, 'Vault A/Collection A/Nope.md')).rejects.toThrow()
  })
})
