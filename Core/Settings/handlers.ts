...
import { excludedFolderRefusal } from './codec'
...
export const settingsHandlers = {
  ...
  'exclusions:count': async () => {
    const root = sessionRoot()
    if (root === null) return NO_NEXUS
    return ok((await readWatchScope(root)).excluded.length)
  },

  'personalization:set': async (ctx, key: unknown, value: unknown) => {
    const root = sessionRoot()
    if (root === null) return NO_NEXUS
  ...
  'subfield:set': async (_ctx, config: unknown) => {
    const root = sessionRoot()
    if (root === null) return NO_NEXUS
  ...
  'navViewModes:set': async (_ctx, modes: unknown) => {
    const root = sessionRoot()
    if (root === null) return NO_NEXUS
