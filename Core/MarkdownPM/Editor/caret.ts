// A drawn caret via a CM `layer` (shape + fade are CSS; this file is geometry only). Only the
// native caret is hidden (`caret-color: transparent`), not the selection — unlike drawSelection's
...
// Measuring those seats yields the tile box itself, parking a caret at the tile's top on the
// widget's own x. Draw instead where the seat's insertion will land: one line's worth above or
// below the tile, at the indent.
function tileEdgeMarker(view: EditorView, head: number): RectangleMarker | null {
  ...
  // Marker coords are document-relative (client minus scroll-adjusted origin), matching forRange.
  const sr = view.scrollDOM.getBoundingClientRect()
  const cr = view.contentDOM.getBoundingClientRect()
  const cs = getComputedStyle(view.contentDOM)
  // Body height, not defaultLineHeight — CM measures the latter off its default font, taller than
  // body lines actually render.
...
// A cursor placed against a block widget (e.g. a table) makes forRange return a marker spanning
// the whole widget — a giant, mis-placed measurement. Anything far taller than a text line comes
// back one line tall.
export function clampToLine(view: EditorView, cls: string, m: RectangleMarker): RectangleMarker {
  ...
  const floor = 4 // under this a marker is a collapsed line's sliver, not a caret
...
function caretMarkers(view: EditorView): RectangleMarker[] {
  ...
  for (const r of view.state.selection.ranges) {
    ...
    // A seat whose assoc side faces a replaced range has no coords there and forRange returns
    // nothing — flip to the surviving side, which always measures.
...
export const customCaret = layer({
  ...
  update(update, dom) {
    ...
    // the instant it moves, rather than mid-fade.
